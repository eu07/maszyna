...
namespace audio
{
	...
	void openal_source::update(double const Deltatime, glm::vec3 const& Listenervelocity)
	{
	...
	void openal_source::sync_with(sound_properties const& State)
	{
		...
		// velocity
		if ((update_deltatime > 0.0) && (sound_range >= 0) && (properties.location != glm::dvec3())) {
		...
		// gain
		if ((State.soundproofing_stamp != properties.soundproofing_stamp) || (State.gain != properties.gain)) {
		...
		if (sound_range > 0) {
			...
			if ((distancesquared > rangesquared) || (false == is_in_range)) {
				...
				auto const rangefactor{ interpolate(1.f, 0.f, clamp<float>((distancesquared - rangesquared) / (fadedistance * fadedistance), 0.f, 1.f)) };
	...
	void openal_source::range(float const Range)
	{
		...
		auto const range{ (Range >= 0 ? Range : 5) }; // range of -1 means sound of unlimited range, positioned at the listener
	...
	audio::buffer_handle openal_renderer::fetch_buffer(std::string const& Filename)
	{
		return m_buffers.create(Filename);
	}

	// provides direct access to a specified buffer
	audio::openal_buffer const& openal_renderer::buffer(audio::buffer_handle const Buffer) const
	{
	...
	void openal_renderer::update(double const Deltatime)
	{
		...
		glm::vec3 const orientation[] = { glm::vec3{ 0, 0, -1 } * rotationmatrix, glm::vec3{ 0, 1, 0 } * rotationmatrix };
		...
		if (Deltatime > 0) {
			...
			m_listenervelocity = (glm::length(listenermovement) < 1000.0 ? // large jumps are typically camera changes
	...
	audio::openal_source openal_renderer::fetch_source()
	{
		...
		if (newsource.id == audio::null_resource) {
			...
			for (auto source{ std::begin(m_sources) }; source != std::cend(m_sources); ++source) {
				if ((source->id == audio::null_resource) || (true == source->is_multipart) || (false == source->is_playing)) {
					continue;
				}
				auto const sourceweight{ (source->sound_range > 0 ? (source->sound_range * source->sound_range) / (glm::length2(source->sound_distance) + 1) : std::numeric_limits<float>::max()) };
				if (sourceweight < leastimportantweight) {
					leastimportantsource = source;
					leastimportantweight = sourceweight;
				}
			}
			if ((leastimportantsource != std::end(m_sources)) && (leastimportantweight < 1.f)) {
	...
	bool openal_renderer::init_caps()
	{
		...
		WriteLog("Audio Renderer: " + std::string{ (char*)::alcGetString(m_device, ALC_DEVICE_SPECIFIER) } + " OpenAL Version: " + oalversion);
