...
namespace scene
{
	...
	void basic_cell::update_traction(TDynamicObject* Vehicle, int const Pantographindex)
	{
		...
		for (auto* traction : m_directories.traction) {
			...
			auto const fRaParam = -(glm::dot(traction->pPoint1, vFront) - glm::dot(pantographposition, vFront)) / (paramfrontdot != 0.0 ? paramfrontdot : 0.001); // div0 trap
			...
			if (fVertical >= 0.0) {
				...
				if ((Global.bEnableTraction) && (fVertical < pantograph->PantWys - 0.15)) {
	...
	void basic_cell::update_events()
	{
		...
		for (auto* launcher : m_eventlaunchers) {
			if ((true == launcher->check_conditions()) && (SquareMagnitude(launcher->location() - Global.pCameraPosition) < launcher->dRadius)) {
				WriteLog("Eventlauncher " + launcher->name());
				if ((true == Global.shiftState) && (launcher->Event2 != nullptr)) {
	...
	void basic_cell::deserialize(std::istream& Input)
	{
		...
		m_active = ((false == m_shapesopaque.empty()) || (false == m_shapestranslucent.empty()) || (false == m_lines.empty()));
	}
	...
	void basic_cell::insert(shape_node Shape)
	{
		...
		m_area.radius = std::max<float>(m_area.radius, glm::length(m_area.center - Shape.data().area.center) + Shape.data().area.radius);
	...
	void basic_cell::insert(TTrack* Path)
	{
		...
		m_area.radius = std::max(m_area.radius,
	...
	void basic_cell::insert(TAnimModel* Instance)
	{
		...
		auto alpha = (Instance->material() != nullptr ? Instance->material()->textures_alpha : 0x30300030);
	...
	void basic_cell::register_end(TTrack* Path)
	{
		...
		m_directories.paths.erase(std::unique(std::begin(m_directories.paths), std::end(m_directories.paths)), std::end(m_directories.paths));
	}
	...
	void basic_cell::register_end(TTraction* Traction)
	{
		...
		// eliminate potential duplicates
		m_directories.traction.erase(std::unique(std::begin(m_directories.traction), std::end(m_directories.traction)), std::end(m_directories.traction));
	}
	...
	std::tuple<TDynamicObject*, float> basic_cell::find(glm::dvec3 const& Point, float const Radius, bool const Onlycontrolled, bool const Findbycoupler) const
	{
		...
		for (auto* path : m_paths) {
			for (auto* vehicle : path->Dynamics) {
				...
				if (false == Findbycoupler) {
					...
					distance = std::min(glm::length2(glm::dvec3{ vehicle->HeadPosition() } - Point), glm::length2(glm::dvec3{ vehicle->RearPosition() } - Point));
				}
	...
	std::tuple<TTrack*, int> basic_cell::find(glm::dvec3 const& Point, TTrack const* Exclude) const
	{
	...
	std::tuple<TTraction*, int> basic_cell::find(glm::dvec3 const& Point, TTraction const* Exclude) const
	{
		int endpointid;
	...
	std::tuple<TTraction*, int, float> basic_cell::find(glm::dvec3 const& Point, TTraction const* Other, int const Currentdirection) const
	{
		...
		for (auto* traction : m_directories.traction) {
			if ((traction == Other) || (traction->psSection != Other->psSection) || (traction == Other->hvNext[0]) || (traction == Other->hvNext[1])) {
			...
			if ((traction->psPower[endpoint] == nullptr) || (traction->fResistance[endpoint] < 0.0)) {
				continue;
			}
	...
	{
		m_area.radius = std::max(m_area.radius, static_cast<float>(glm::length(m_area.center - Node->location()) + Node->radius()));
	}

	// legacy method, finds and assigns traction piece(s) to pantographs of provided vehicle
	void basic_section::update_traction(TDynamicObject* Vehicle, int const Pantographindex)
	{
		auto const vFront = glm::make_vec3(Vehicle->VectorFront().getArray()); // wektor normalny dla płaszczyzny ruchu pantografu
		auto const vUp = glm::make_vec3(Vehicle->VectorUp().getArray()); // wektor pionu pudła (pochylony od pionu na przechyłce)
		auto const vLeft = glm::make_vec3(Vehicle->VectorLeft().getArray()); // wektor odległości w bok (odchylony od poziomu na przechyłce)
	...
	void basic_section::update_events(glm::dvec3 const& Location, float const Radius)
	{
		for (auto& cell : m_cells) {
	...
	void basic_section::update_sounds(glm::dvec3 const& Location, float const Radius)
	{
		for (auto& cell : m_cells) {
			if (glm::length2(cell.area().center - Location) < ((cell.area().radius + Radius) * (cell.area().radius + Radius))) {
				// we reject cells which aren't within our area of interest
	...
	void basic_section::insert(shape_node Shape)
	{
		...
		m_area.radius = std::max<float>(m_area.radius, static_cast<float>(glm::length(m_area.center - shapedata.area.center) + shapedata.area.radius));
	...
	// find a vehicle located nearest to specified point, within specified radius, optionally ignoring vehicles without drivers. reurns: located vehicle and distance
	std::tuple<TDynamicObject*, float> basic_section::find(glm::dvec3 const& Point, float const Radius, bool const Onlycontrolled, bool const Findbycoupler)
	{
	...
	// finds a path with one of its ends located in specified point. returns: located path and id of the matching endpoint
	std::tuple<TTrack*, int> basic_section::find(glm::dvec3 const& Point, TTrack const* Exclude)
	{
		return cell(Point).find(Point, Exclude);
	}

	// finds a traction piece with one of its ends located in specified point. returns: located traction piece and id of the matching endpoint
	std::tuple<TTraction*, int> basic_section::find(glm::dvec3 const& Point, TTraction const* Exclude)
	{
		return cell(Point).find(Point, Exclude);
	}

	// finds a traction piece located nearest to specified point, sharing section with specified other piece and powered in specified direction. returns: located traction piece
	std::tuple<TTraction*, int, float> basic_section::find(glm::dvec3 const& Point, TTraction const* Other, int const Currentdirection)
	{
		// go through sections within radius of interest, and pick the nearest candidate
	...
	basic_cell& basic_section::cell(glm::dvec3 const& Location)
	{
		...
		return m_cells[clamp(row, 0, (EU07_SECTIONSIZE / EU07_CELLSIZE) - 1) * (EU07_SECTIONSIZE / EU07_CELLSIZE) + clamp(column, 0, (EU07_SECTIONSIZE / EU07_CELLSIZE) - 1)];
	}
	...
	// legacy method, finds and assigns traction piece(s) to pantographs of provided vehicle
	void basic_region::update_traction(TDynamicObject* Vehicle, int const Pantographindex)
	{
	...
	bool basic_region::deserialize(std::string const& Scenariofile)
	{
		...
		if ((headermain != MAKE_ID4('E', 'U', '0', '7') || (headertype != EU07_FILEVERSION_REGION))) {
			// wrong file type
	...
	void basic_region::insert_shape(shape_node Shape, scratch_data& Scratchpad, bool const Transform)
	{
		...
		if (true == Transform) {
			...
			if ((false == Scratchpad.location.offset.empty()) && (Scratchpad.location.offset.top() != glm::dvec3(0, 0, 0))) {
				// ...and move
		...
		for (auto& shape : shapes) {
			...
			if (point_inside(shape.m_data.area.center)) {
				...
				ErrorLog(
				"Bad scenario: shape node" + (shape.m_name.empty() ? "" : " \"" + shape.m_name + "\"") + " placed in location outside region bounds (" + to_string(shape.m_data.area.center) + ")");
			}
		}
	}

	// inserts provided lines in the region
	void basic_region::insert_lines(lines_node Lines, scratch_data& Scratchpad)
	{
		...
		if (point_inside(Lines.m_data.area.center)) {
			...
			ErrorLog(
			"Bad scenario: lines node" + (Lines.m_name.empty() ? "" : " \"" + Lines.m_name + "\"") + " placed in location outside region bounds (" + to_string(Lines.m_data.area.center) + ")");
		}
	}

	// inserts provided track in the region
	void basic_region::insert_path(TTrack* Path, const scratch_data& Scratchpad)
	{
	...
	// inserts provided track in the region
	void basic_region::insert_traction(TTraction* Traction, scratch_data& Scratchpad)
	{
		// NOTE: bounding area isn't present/filled until track class and wrapper refactoring is done
	...
	void basic_region::insert_instance(TAnimModel* Instance, scratch_data& Scratchpad)
	{
		// NOTE: bounding area isn't present/filled until track class and wrapper refactoring is done
	...
	void basic_region::insert_sound(sound_source* Sound, scratch_data& Scratchpad)
	{
		// NOTE: bounding area isn't present/filled until track class and wrapper refactoring is done
	...
	void basic_region::insert_launcher(TEventLauncher* Launcher, scratch_data& Scratchpad)
	{
		// NOTE: bounding area isn't present/filled until track class and wrapper refactoring is done
	...
	std::tuple<TDynamicObject*, float> basic_region::find_vehicle(glm::dvec3 const& Point, float const Radius, bool const Onlycontrolled, bool const Findbycoupler)
	{
	...
	// finds a path with one of its ends located in specified point. returns: located path and id of the matching endpoint
	std::tuple<TTrack*, int> basic_region::find_path(glm::dvec3 const& Point, TTrack const* Exclude)
	{
	...
	// finds a traction piece with one of its ends located in specified point. returns: located traction piece and id of the matching endpoint
	std::tuple<TTraction*, int> basic_region::find_traction(glm::dvec3 const& Point, TTraction const* Exclude)
	{
		// TBD: throw out of bounds exception instead of checks all over the place..?
		if (point_inside(Point)) {
			return section(Point).find(Point, Exclude);
	...
	// finds a traction piece located nearest to specified point, sharing section with specified other piece and powered in specified direction. returns: located traction piece
	std::tuple<TTraction*, int> basic_region::find_traction(glm::dvec3 const& Point, TTraction const* Other, int const Currentdirection)
	{
	...
	std::vector<basic_section*> const& basic_region::sections(glm::dvec3 const& Point, float const Radius)
	{
		...
		for (int row = originz; row <= originz + sectioncount; ++row) {
			...
			for (int column = originx; column <= originx + sectioncount; ++column) {
				...
				if ((section != nullptr) && (glm::length2(section->area().center - Point) <= ((section->area().radius + padding + Radius) * (section->area().radius + padding + Radius)))) {
	...
	void basic_region::register_path(TTrack* Path, glm::dvec3 const& Point)
	{
		if (point_inside(Point)) {
	...
	void basic_region::register_traction(TTraction* Traction, glm::dvec3 const& Point)
	{
		if (point_inside(Point)) {
	...
	bool basic_region::RaTriangleDivider(shape_node& Shape, std::deque<shape_node>& Shapes)
	{
		...
		if ((Shape.m_data.vertices[0].position.x >= x0) && (Shape.m_data.vertices[0].position.x <= x1) && (Shape.m_data.vertices[0].position.z >= z0) && (Shape.m_data.vertices[0].position.z <= z1) &&
		    (Shape.m_data.vertices[1].position.x >= x0) && (Shape.m_data.vertices[1].position.x <= x1) && (Shape.m_data.vertices[1].position.z >= z0) && (Shape.m_data.vertices[1].position.z <= z1) &&
		    (Shape.m_data.vertices[2].position.x >= x0) && (Shape.m_data.vertices[2].position.x <= x1) && (Shape.m_data.vertices[2].position.z >= z0) && (Shape.m_data.vertices[2].position.z <= z1)) {
		...
		switch (divide & 3) {
			...
			case 0: {
				...
				if (divide & 4) {
					Shape.m_data.vertices[1].set_from_z(Shape.m_data.vertices[0], Shape.m_data.vertices[1], ((divide & 8) ? z1 : z0));
				} else {
					Shape.m_data.vertices[1].set_from_x(Shape.m_data.vertices[0], Shape.m_data.vertices[1], ((divide & 8) ? x1 : x0));
				}
			...
			case 1: {
				...
				if (divide & 4) {
					Shape.m_data.vertices[2].set_from_z(Shape.m_data.vertices[1], Shape.m_data.vertices[2], ((divide & 8) ? z1 : z0));
				} else {
					Shape.m_data.vertices[2].set_from_x(Shape.m_data.vertices[1], Shape.m_data.vertices[2], ((divide & 8) ? x1 : x0));
				}
			...
			case 2: {
				...
				newshape.m_data.vertices[2] = Shape.m_data.vertices[2]; // wierzchołek C przechodzi do nowego
				if (divide & 4) {
					Shape.m_data.vertices[2].set_from_z(Shape.m_data.vertices[2], Shape.m_data.vertices[0], ((divide & 8) ? z1 : z0));
				} else {
					Shape.m_data.vertices[2].set_from_x(Shape.m_data.vertices[2], Shape.m_data.vertices[0], ((divide & 8) ? x1 : x0));
				}
	...
	basic_section& basic_region::section(glm::dvec3 const& Location)
	{
		...
		auto& section = m_sections[clamp(row, 0, EU07_REGIONSIDESECTIONCOUNT - 1) * EU07_REGIONSIDESECTIONCOUNT + clamp(column, 0, EU07_REGIONSIDESECTIONCOUNT - 1)];
