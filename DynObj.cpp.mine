...
{
 if (Global::bQueuedAdvLog) WriteLog("TDynamicObject::GetLastDynamic(int cpl_type)");
...
AnsiString X;
double CONSISTMASS, CONSISTCARS, CONSISTLEN, ML;
...
TDynamicObject* TDynamicObject::GetConsist_f(int cpl_type, TDynamicObject *lok)
{
 ...
 Global::CONSISTF->Clear();
 Global::CONSISTB->Clear();
 Global::CONSISTA->Clear();
 ...
 for (int LOOP=0; LOOP<Global::CONSISTF->Count; LOOP++) {Global::CONSISTA->Add(Global::CONSISTF->Strings[LOOP]); }
                                                         Global::CONSISTA->Add(lok->MoverParameters->Name + ", " + lok->MyTrack->asTrackName + ", " + lok->asBaseDir + ", " + lok->MoverParameters->TypeName + ", " );
 for (int LOOP=0; LOOP<Global::CONSISTB->Count; LOOP++) {Global::CONSISTA->Add(Global::CONSISTB->Strings[LOOP]); }

 Global::consistlen = CONSISTLEN;
 Global::consistmass = CONSISTMASS/1000;
 Global::consistcars = CONSISTCARS;
}


// *****************************************************************************
// FUNKCJA SZUKAJACA POJAZDY DOCZEPIONE Z TYLU LOKA, WYWOLYWANA Z OnUpdate() ^^^
// *****************************************************************************

TDynamicObject* TDynamicObject::GetConsist_b(int cpl_type, TDynamicObject *lok)
{
 if (Global::bQueuedAdvLog) WriteLog("TDynamicObject::GetConsist_b(int cpl_type, TDynamicObject *lok)");
...
//              "," + FloatToStr(temp->QueuedMoveLen) +                  // MOVE LEN
//              "," + FloatToStr(temp->MoverParameters->Vel) +           // VELOCITY
//              "," + temp->GetFirstDynamic(1)->MoverParameters->Name +  // FIRST DYN
//              "," + temp->GetLastDynamic(1)->MoverParameters->Name +   // LAST DYN
//              "," + temp->MoverParameters->filename +
//              "," + temp->MoverParameters->TrainType +
//              "," + temp->TrainParams->TrainName +
                "," + temp->MyTrack->asTrackName +
                "," + temp->asBaseDir +
...
//                "," + temp->bodytype +
                ...
                //temp->InitializeCab(1, temp->asBaseDir + temp->MoverParameters->TypeName + ".mmd");
                Global::CONSISTB->Add(X);
         }
      else
         {
...
{
   if (Global::bQueuedAdvLog) WriteLog("TDynamicObject::GetConsist_a(int cpl_type, TDynamicObject *lok)");
...
//            "," + temp->TrainParams->TrainName +
              ...
              "," + temp->asBaseDir +
              "," + temp->MoverParameters->TypeName +                           // TYPE
//              "," + temp->texturefile +                                         // TEXTURE
//              "," + temp->bodytype +                                            // BODY
              "," + FloatToStr(temp->MoverParameters->BrakePress) +             // BRAKE PRESS
              "," + FloatToStr(temp->MoverParameters->Mass) +                   // MASS
              "," + FloatToStr(temp->MoverParameters->Dim.L) +                  // LENGTH
              "," + temp->asBaseDir + "," ;                                     // BASE DIR
 ...
 btnOn=false;
  ...
  if (ObjSqrDist<160000) //gdy bli¿ej ni¿ 400m
  {
   if (ObjSqrDist<2500) //gdy bli¿ej ni¿ 50m
   {
    ...
    if (mdLoad) mdLoad->GetSMRoot()->SetTranslate(modelShake);
...
// sygnaly czola pociagu
    if (TestFlag(iLights[0],1)) {btHeadSignals11.TurnOn(); btnOn=true; headlA_L=true;}
    else
     {btHeadSignals11.TurnOff(); headlA_L=false;}

    if (TestFlag(iLights[0],4)) {btHeadSignals12.TurnOn(); btnOn=true; headlA_U=true;}
    else
     {btHeadSignals12.TurnOff(); headlA_U=false;}

    if (TestFlag(iLights[0],16)) {btHeadSignals13.TurnOn(); btnOn=true; headlA_R=true;}
    else
     {btHeadSignals13.TurnOff(); headlA_R=false;}

    if (TestFlag(iLights[1],1)) {btHeadSignals21.TurnOn(); btnOn=true; headlB_L=true;}
    else
     {btHeadSignals21.TurnOff(); headlB_L=false;}

    if (TestFlag(iLights[1],4)) {btHeadSignals22.TurnOn(); btnOn=true; headlB_U=true;}
    else
     {btHeadSignals22.TurnOff(); headlB_U=false;}

    if (TestFlag(iLights[1],16)) {btHeadSignals23.TurnOn(); btnOn=true; headlB_R=true;}
    else
     {btHeadSignals23.TurnOff(); headlB_R=false; }
...
{//ustawienie przechy³ki pojazdu i jego zawartoœci
 ...
 {//jak nie ma przechy³ki, to nie ma po co przechylaæ modeli
  ...
  if (mdLoad) mdLoad->GetSMRoot()->SetRotateXYZ(vector3(0,modelRoll,0));
  if (mdLowPolyInt) mdLowPolyInt->GetSMRoot()->SetRotateXYZ(vector3(0,modelRoll,0));
  if (mdPrzedsionek) mdPrzedsionek->GetSMRoot()->SetRotateXYZ(vector3(0,modelRoll,0));
 }
}
...
//McZapkie: parametry powinny byc pobierane z toru
    ...
    ts.dHtrack=Axle1.pPosition.y-Axle0.pPosition.y; //wektor miêdzy skrajnymi osiami (!!!odwrotny)
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// POBIERANIE INFORMACJI O POJEZDZIE PRZY KTORYM STOI OBSERWATOR ^^^^^^^^^^^^^^^
// WYKORZYSTYWANE DO ZMIANY POJAZDU,

AnsiString  dynname;
vector3 vechicleA = GetPosition();
float vector_lenght  = 0;
float ObjSqrDist= SquareMagnitude(Global::pCameraPosition-vechicleA);

Global::flnearestdyndist = ObjSqrDist;

  if ((ObjSqrDist < 31 && Global::bnearestengaged == false))  // JEZELI W ODLEGLOSCI NIE WIEKSZEJ NIZ 29
      {
       dynname = LowerCase( MoverParameters->Name );
       Global::asnearestdyn = dynname;
       Global::bnearestengaged = true;
      }
...
// RENDEROWANIE MODELI ZDEFINIOWANYCH W PLIKU ADD ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

    if (bcab1light) glEnable(GL_LIGHT2); else glDisable(GL_LIGHT2);
#ifdef USE_VBO
  if (Global::bUseVBO)
  {
   if (mdClock1 != NULL) mdClock1->RaRender(ObjSqrDist, ReplacableSkinID,iAlpha);
   if (mdClock2 != NULL) mdClock2->RaRender(ObjSqrDist, ReplacableSkinID,iAlpha);
   if (mdVentilator1 != NULL) mdVentilator1->RaRender(ObjSqrDist, ReplacableSkinID,iAlpha);
   if (mdVentilator2 != NULL) mdVentilator2->RaRender(ObjSqrDist, ReplacableSkinID,iAlpha);
  }
  else
#endif
  {
...
// RENDEROWANIE MODELI ZDEFINIOWANYCH W PLIKU ADD ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  if (Global::bUseVBO)
  {
   if (mdVentilator1 != NULL) mdVentilator1->RaRenderAlpha(ObjSqrDist, ReplacableSkinID,iAlpha);
   if (mdVentilator2 != NULL) mdVentilator2->RaRenderAlpha(ObjSqrDist, ReplacableSkinID,iAlpha);
  }
  else
#endif
  {
...
// KONIEC OBSLUGI MODELI Z PLIKU ADD -------------------------------------------
