	...
	switch (Key) {
		case GLFW_KEY_F1: {
			...
			if (Global.iTextMode == Key) {
				++Global.iScreenMode[Key - GLFW_KEY_F1];
			}
		...
		case GLFW_KEY_F10: {
			// quit
			if (Global.iTextMode == Key) {
				Global.iTextMode = (Global.iPause && (Key != GLFW_KEY_F1) ? GLFW_KEY_F1 : 0); // wyłączenie napisów, chyba że pauza
			} else {
				Global.iTextMode = Key;
			}
			return true;
		...
		case GLFW_KEY_Y: {
			// potentially quit
			if (Global.iTextMode != GLFW_KEY_F10) {
				return false;
			} // not in quit mode
	...
	switch (Global.iTextMode) {
			...
			if (Global.iScreenMode[Global.iTextMode - GLFW_KEY_F1] == 1) {
				if (0 == table->StationCount) {
					...
					for (int i = owner->iStationStart; i <= std::min(owner->iStationStart + 10, table->StationCount); ++i) {
						...
						std::string const station = (Bezogonkow(tableline->StationName, true) + "                                  ").substr(0, 34);
						...
						((owner->iStationStart < table->StationIndex) && (i < table->StationIndex) && ((time.wHour * 60 + time.wMinute) >= (tableline->Dh * 60 + tableline->Dm)));
						auto traveltime =
						"   " + (i < 2 ? "" :
						...
						UITable->text_lines.emplace_back(("| " + vmax + " | " + station + " | " + arrival + " | " + traveltime + " |"),
						(candeparture ? glm::vec4(0.0f, 1.0f, 0.0f, 1.0f) : // czas minął i odjazd był, to nazwa stacji będzie na zielono
						 Global.UITextColor));
						UITable->text_lines.emplace_back(("|     | " + location + tableline->StationWare + " | " + departure + " |     |"),
						(candeparture ? glm::vec4(0.0f, 1.0f, 0.0f, 1.0f) : // czas minął i odjazd był, to nazwa stacji będzie na zielono
						 Global.UITextColor));
						// divider/footer
						UITable->text_lines.emplace_back("+-----+------------------------------------+-------+-----+", Global.UITextColor);
		...
		case (GLFW_KEY_F3): {
			...
			if (vehicle != nullptr) {
				...
				uitextline2 +=
				(true == vehicle->MoverParameters->WaterHeater.is_damaged ?
				 "!" :
				 (vehicle->MoverParameters->WaterHeater.is_active ? "H" :
				...
				uitextline2 +=
				(false == vehicle->MoverParameters->ConverterAllowLocal ? "-" : (vehicle->MoverParameters->ConverterAllow ? (vehicle->MoverParameters->ConverterFlag ? "X" : "x") : "."));
				...
				(vehicle->MoverParameters->CompressorFlag ? "C" :
				                                            (false == vehicle->MoverParameters->CompressorAllowLocal ?
				                                             "-" :
				                                             ((vehicle->MoverParameters->CompressorAllow || vehicle->MoverParameters->CompressorStart == start::automatic) ? "c" : ".")));
...
*/
				if (vehicle->Mechanik) {
					...
					uitextline4 +=
					"Driver: Vd=" + to_string(vehicle->Mechanik->VelDesired, 0) + " Ad=" + to_string(vehicle->Mechanik->AccDesired, 2) + " Ah=" + to_string(vehicle->Mechanik->fAccThreshold, 2) + "@" +
					to_string(vehicle->Mechanik->fBrake_a0[0], 2) + "+" + to_string(vehicle->Mechanik->fBrake_a1[0], 2) + " Bd=" + to_string(vehicle->Mechanik->fBrakeDist, 0) +
					" Pd=" + to_string(vehicle->Mechanik->ActualProximityDist, 0) + " Vn=" + to_string(vehicle->Mechanik->VelNext, 0) + " VSl=" + to_string(vehicle->Mechanik->VelSignalLast, 0) +
					" VLl=" + to_string(vehicle->Mechanik->VelLimitLast, 0) + " VRd=" + to_string(vehicle->Mechanik->VelRoad, 0) + " VRst=" + to_string(vehicle->Mechanik->VelRestricted, 0);
		...
		case (GLFW_KEY_F11): {
			...
			while ((event != nullptr) && (eventtableindex < 30)) {
		...
		default: {
			...
			if (DebugModeFlag) {
				...
				uitextline2 =
				"HamZ=" + to_string(vehicle->MoverParameters->fBrakeCtrlPos, 2) + "; HamP=" + std::to_string(vehicle->MoverParameters->LocalBrakePos) + "/" +
				to_string(vehicle->MoverParameters->LocalBrakePosA, 2) + "; NasJ=" + std::to_string(vehicle->MoverParameters->MainCtrlPos) + "(" +
				std::to_string(vehicle->MoverParameters->MainCtrlActualPos) + ")" +
...
*/
				...
				if ((vehicle->MoverParameters->EnginePowerSource.SourceType == CurrentCollector) || (vehicle->MoverParameters->TrainType == dt_EZT)) {
				...
				if ((vehicle->Mechanik) && (vehicle->Mechanik->AIControllFlag == AIdriver)) {
					uitextline4 += "AI: Vd=" + to_string(vehicle->Mechanik->VelDesired, 0) + " ad=" + to_string(vehicle->Mechanik->AccDesired, 2) + "/" +
					               ...
					               to_string(vehicle->Mechanik->fBrake_a1[0], 2) + " aS=" + to_string(vehicle->Mechanik->AbsAccS_pub, 2) +
					               " Pd=" + to_string(vehicle->Mechanik->ActualProximityDist, 0) + " Vn=" + to_string(vehicle->Mechanik->VelNext, 0);
				}
...
void ui_layer::render_progress()
{
	...
	if (m_subtaskprogress) {
		quad(glm::vec4(origin.x, origin.y, origin.x + size.x * m_subtaskprogress, origin.y + size.y), glm::vec4(8.0f / 255.0f, 160.0f / 255.0f, 8.0f / 255.0f, 0.35f));
	}
	// primary bar
	if (m_progress) {
		quad(glm::vec4(origin.x, origin.y, origin.x + size.x * m_progress, origin.y + size.y), glm::vec4(8.0f / 255.0f, 160.0f / 255.0f, 8.0f / 255.0f, 1.0f));
	}

	if (false == m_progresstext.empty()) {
		float const screenratio = static_cast<float>(Global.iWindowWidth) / Global.iWindowHeight;
		float const width = (screenratio >= (4.0f / 3.0f) ? (4.0f / 3.0f) * Global.iWindowHeight : Global.iWindowWidth);
		float const heightratio = (screenratio >= (4.0f / 3.0f) ? Global.iWindowHeight / 768.f : Global.iWindowHeight / 768.f * screenratio / (4.0f / 3.0f));
		...
		::glRasterPos2f((0.5f * (Global.iWindowWidth - width) + origin.x * heightratio) + ((size.x * heightratio - textwidth) * 0.5f * heightratio),
...
void ui_layer::render_panels()
{
	...
	for (auto const& panel : m_panels) {
		...
		for (auto const& line : panel->text_lines) {
			...
			::glRasterPos2f(0.5f * (Global.iWindowWidth - width) + panel->origin_x * height, panel->origin_y * height + 20.f * lineidx);
			print(line.data);
			++lineidx;
		}
	}
...
void ui_layer::render_background()
{
	...
	auto const& texture = GfxRenderer.Texture(m_background);
	float const width = (texture.width() == texture.height() ? 1024.0f : // legacy mode, square texture displayed as 4:3 image
	                     texture.width() / (texture.height() / 768.0f));
	quad(glm::vec4((1024.0f * 0.5f) - (width * 0.5f), (768.0f * 0.5f) - (height * 0.5f), (1024.0f * 0.5f) - (width * 0.5f) + width, (768.0f * 0.5f) - (height * 0.5f) + height), colors::white);
}

void ui_layer::render_texture()
{
	if (m_texture != 0) {
		::glColor4fv(glm::value_ptr(colors::white));
		...
		glMultiTexCoord2f(m_textureunit, 0.f, 1.f);
		glVertex2f(offset, Global.iWindowHeight - offset - size);
		glMultiTexCoord2f(m_textureunit, 0.f, 0.f);
		glVertex2f(offset, Global.iWindowHeight - offset);
		glMultiTexCoord2f(m_textureunit, 1.f, 1.f);
		glVertex2f(offset + size, Global.iWindowHeight - offset - size);
		glMultiTexCoord2f(m_textureunit, 1.f, 0.f);
		glVertex2f(offset + size, Global.iWindowHeight - offset);
...
void ui_layer::print(std::string const& Text)
{
	if ((false == Global.DLFont) || (true == Text.empty()))
		return;
...
void ui_layer::quad(glm::vec4 const& Coordinates, glm::vec4 const& Color)
{
	float const screenratio = static_cast<float>(Global.iWindowWidth) / Global.iWindowHeight;
	float const width = (screenratio >= (4.f / 3.f) ? (4.f / 3.f) * Global.iWindowHeight : Global.iWindowWidth);
	float const heightratio = (screenratio >= (4.f / 3.f) ? Global.iWindowHeight / 768.f : Global.iWindowHeight / 768.f * screenratio / (4.f / 3.f));
	...
	glMultiTexCoord2f(m_textureunit, 0.f, 1.f);
	glVertex2f(0.5f * (Global.iWindowWidth - width) + Coordinates.x * heightratio, 0.5f * (Global.iWindowHeight - height) + Coordinates.y * heightratio);
	glMultiTexCoord2f(m_textureunit, 0.f, 0.f);
	glVertex2f(0.5f * (Global.iWindowWidth - width) + Coordinates.x * heightratio, 0.5f * (Global.iWindowHeight - height) + Coordinates.w * heightratio);
	glMultiTexCoord2f(m_textureunit, 1.f, 1.f);
	glVertex2f(0.5f * (Global.iWindowWidth - width) + Coordinates.z * heightratio, 0.5f * (Global.iWindowHeight - height) + Coordinates.y * heightratio);
	glMultiTexCoord2f(m_textureunit, 1.f, 0.f);
	glVertex2f(0.5f * (Global.iWindowWidth - width) + Coordinates.z * heightratio, 0.5f * (Global.iWindowHeight - height) + Coordinates.w * heightratio);
