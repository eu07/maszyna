...
static ddspp_constexpr unsigned int DDS_RGBA =
    0x00000041;  // DDPF_RGB | DDPF_ALPHAPIXELS
static ddspp_constexpr unsigned int DDS_YUV = 0x00000200;  // DDPF_YUV
static ddspp_constexpr unsigned int DDS_LUMINANCE =
    0x00020000;  // DDPF_LUMINANCE
static ddspp_constexpr unsigned int DDS_LUMINANCEA =
    0x00020001;  // DDPF_LUMINANCE | DDPF_ALPHAPIXELS

static ddspp_constexpr unsigned int DDS_PAL8 =
    0x00000020;  // DDPF_PALETTEINDEXED8
...
static ddspp_constexpr unsigned int DDS_HEADER_FLAGS_HEIGHT =
    0x00000002;  // DDSD_HEIGHT
static ddspp_constexpr unsigned int DDS_HEADER_FLAGS_WIDTH =
    0x00000004;  // DDSD_WIDTH
static ddspp_constexpr unsigned int DDS_HEADER_FLAGS_PITCH =
    0x00000008;  // DDSD_PITCH
static ddspp_constexpr unsigned int DDS_HEADER_FLAGS_PIXELFORMAT =
    0x00001000;  // DDSD_PIXELFORMAT
static ddspp_constexpr unsigned int DDS_HEADER_FLAGS_MIPMAP =
    0x00020000;  // DDSD_MIPMAPCOUNT
static ddspp_constexpr unsigned int DDS_HEADER_FLAGS_LINEARSIZE =
    0x00080000;  // DDSD_LINEARSIZE
static ddspp_constexpr unsigned int DDS_HEADER_FLAGS_VOLUME =
    0x00800000;  // DDSD_DEPTH

static ddspp_constexpr unsigned int DDS_HEADER_CAPS_COMPLEX =
    0x00000008;  // DDSCAPS_COMPLEX
static ddspp_constexpr unsigned int DDS_HEADER_CAPS_MIPMAP =
    0x00400000;  // DDSCAPS_MIPMAP
static ddspp_constexpr unsigned int DDS_HEADER_CAPS_TEXTURE =
    0x00001000;  // DDSCAPS_TEXTURE

static ddspp_constexpr unsigned int DDS_HEADER_CAPS2_CUBEMAP =
...
static ddspp_constexpr unsigned int DDS_HEADER_CAPS2_VOLUME =
    0x00200000;  // DDSCAPS2_VOLUME
static ddspp_constexpr unsigned int DDS_HEADER_CAPS2_CUBEMAP_ALLFACES =
    DDS_HEADER_CAPS2_CUBEMAP_POSITIVEX | DDS_HEADER_CAPS2_CUBEMAP_NEGATIVEX |
    DDS_HEADER_CAPS2_CUBEMAP_POSITIVEY | DDS_HEADER_CAPS2_CUBEMAP_NEGATIVEY |
...
static ddspp_constexpr unsigned int DXGI_MISC_FLAG_CUBEMAP =
    0x2;  // https://learn.microsoft.com/en-us/windows/win32/api/d3d11/ne-d3d11-d3d11_resource_misc_flag
static ddspp_constexpr unsigned int DDS_MISC_FLAGS2_ALPHA_MODE_MASK = 0x7;

#define ddspp_make_fourcc(a, b, c, d) \
  ((a) + ((b) << 8) + ((c) << 16) + ((d) << 24))

// FOURCC constants
static ddspp_constexpr unsigned int FOURCC_DXT1 =
    ddspp_make_fourcc('D', 'X', 'T', '1');  // BC1_UNORM
static ddspp_constexpr unsigned int FOURCC_DXT2 =
    ddspp_make_fourcc('D', 'X', 'T', '2');  // BC2_UNORM
static ddspp_constexpr unsigned int FOURCC_DXT3 =
    ddspp_make_fourcc('D', 'X', 'T', '3');  // BC2_UNORM
static ddspp_constexpr unsigned int FOURCC_DXT4 =
    ddspp_make_fourcc('D', 'X', 'T', '4');  // BC3_UNORM
static ddspp_constexpr unsigned int FOURCC_DXT5 =
    ddspp_make_fourcc('D', 'X', 'T', '5');  // BC3_UNORM
static ddspp_constexpr unsigned int FOURCC_ATI1 =
    ddspp_make_fourcc('A', 'T', 'I', '1');  // BC4_UNORM
static ddspp_constexpr unsigned int FOURCC_BC4U =
    ddspp_make_fourcc('B', 'C', '4', 'U');  // BC4_UNORM
static ddspp_constexpr unsigned int FOURCC_BC4S =
    ddspp_make_fourcc('B', 'C', '4', 'S');  // BC4_SNORM
static ddspp_constexpr unsigned int FOURCC_ATI2 =
    ddspp_make_fourcc('A', 'T', 'I', '2');  // BC5_UNORM
static ddspp_constexpr unsigned int FOURCC_BC5U =
    ddspp_make_fourcc('B', 'C', '5', 'U');  // BC5_UNORM
static ddspp_constexpr unsigned int FOURCC_BC5S =
    ddspp_make_fourcc('B', 'C', '5', 'S');  // BC5_SNORM
static ddspp_constexpr unsigned int FOURCC_RGBG =
    ddspp_make_fourcc('R', 'G', 'B', 'G');  // R8G8_B8G8_UNORM
static ddspp_constexpr unsigned int FOURCC_GRBG =
    ddspp_make_fourcc('G', 'R', 'G', 'B');  // G8R8_G8B8_UNORM
static ddspp_constexpr unsigned int FOURCC_YUY2 =
    ddspp_make_fourcc('Y', 'U', 'Y', '2');  // YUY2
static ddspp_constexpr unsigned int FOURCC_DXT10 =
    ddspp_make_fourcc('D', 'X', '1', '0');  // DDS extension header
...
static ddspp_constexpr unsigned int FOURCC_R5G6B5 =
    23;  // B5G6R5_UNORM   (needs swizzling)
static ddspp_constexpr unsigned int FOURCC_X1R5G5B5 = 24;
static ddspp_constexpr unsigned int FOURCC_RGB5A1 =
    25;  // B5G5R5A1_UNORM (needs swizzling)
...
inline ddspp_constexpr bool is_rgba_mask(const PixelFormat& ddspf,
                                         unsigned int rmask, unsigned int gmask,
                                         unsigned int bmask,
                                         unsigned int amask) {
  return (ddspf.RBitMask == rmask) && (ddspf.GBitMask == gmask) &&
         (ddspf.BBitMask == bmask) && (ddspf.ABitMask == amask);
}

inline ddspp_constexpr bool is_rgb_mask(const PixelFormat& ddspf,
                                        unsigned int rmask, unsigned int gmask,
                                        unsigned int bmask) {
  return (ddspf.RBitMask == rmask) && (ddspf.GBitMask == gmask) &&
         (ddspf.BBitMask == bmask);
}
}  // namespace internal
...
static_assert(sizeof(HeaderDXT10) == 20,
              "DDS DX10 Extended Header size mismatch");

// Maximum possible size of header. Use this to read in only the header, decode,
// seek to the real header size, then read in the rest of the image data
ddspp_constexpr int MAX_HEADER_SIZE =
    sizeof(DDS_MAGIC) + sizeof(Header) + sizeof(HeaderDXT10);
...
struct Descriptor {
  ...
  unsigned int
      bitsPerPixelOrBlock;  // If compressed bits per block, else bits per pixel
  ...
  bool srgb;
  unsigned int
      headerSize;  // Actual size of header, use this to get to image data
};
...
inline ddspp_constexpr unsigned int get_bits_per_pixel_or_block(
    DXGIFormat format) {
...
inline ddspp_constexpr void get_block_size(DXGIFormat format,
...
inline ddspp_constexpr unsigned int get_row_pitch(
    unsigned int width, unsigned int bitsPerPixelOrBlock,
    unsigned int blockWidth, unsigned int mip) {
  // Shift width by mipmap index, round to next block size and round to next
  // byte (for the rare less than 1 byte per pixel formats) E.g. width = 119,
  // mip = 3, BC1 compression
  // ((((119 >> 2) + 4 - 1) / 4) * 64) / 8 = 64 bytes
  return ((((((width >> mip) > 1) ? (width >> mip) : 1) + blockWidth - 1) /
           blockWidth) *
              bitsPerPixelOrBlock +
          7) /
         8;
...
// Returns number of bytes for each row of a given mip. Valid range is [0,
// desc.numMips)
inline ddspp_constexpr unsigned int get_row_pitch(const Descriptor& desc,
                                                  unsigned int mip) {
  return get_row_pitch(desc.width, desc.bitsPerPixelOrBlock, desc.blockWidth,
                       mip);
}

inline Result decode_header(unsigned char* sourceData, Descriptor& desc) {
  unsigned int magic = *reinterpret_cast<unsigned int*>(
      sourceData);  // First 4 bytes are the magic DDS number
  ...
  if (dxt10Extension) {
    ...
    if (ddspf.flags & DDS_FOURCC) {
      ...
      switch (ddspf.RGBBitCount) {
        case 32:
          if (is_rgba_mask(ddspf, 0x000000ff, 0x0000ff00, 0x00ff0000,
                           0xff000000)) {
            desc.format = R8G8B8A8_UNORM;
          } else if (is_rgba_mask(ddspf, 0x00ff0000, 0x0000ff00, 0x000000ff,
                                  0xff000000)) {
          ...
          // No DXGI format maps to (0x000000ff, 0x0000ff00, 0x00ff0000,
          // 0x00000000) aka D3DFMT_X8B8G8R8 No DXGI format maps to (0x000003ff,
          // 0x000ffc00, 0x3ff00000, 0xc0000000) aka D3DFMT_A2R10G10B10

          // Note that many common DDS reader/writers (including D3DX) swap the
          // the RED/BLUE masks for 10:10:10:2 formats. We assume
          // below that the 'backwards' header mask is being used since it is
          // most likely written by D3DX. The more robust solution is to use the
          ...
          else if (is_rgba_mask(ddspf, 0x3ff00000, 0x000ffc00, 0x000003ff,
                                0xc0000000)) {
            desc.format = R10G10B10A2_UNORM;
          } else if (is_rgba_mask(ddspf, 0x0000ffff, 0xffff0000, 0x00000000,
                                  0x00000000)) {
            desc.format = R16G16_UNORM;
          } else if (is_rgba_mask(ddspf, 0xffffffff, 0x00000000, 0x00000000,
                                  0x00000000)) {
        ...
        case 16:
          ...
          // No DXGI format maps to (0x7c00, 0x03e0, 0x001f, 0x0000) aka
          // D3DFMT_X1R5G5B5. No DXGI format maps to (0x0f00, 0x00f0, 0x000f,
          // 0x0000) aka D3DFMT_X4R4G4B4. No 3:3:2, 3:3:2:8, or paletted DXGI
          // formats aka D3DFMT_A8R3G3B2, D3DFMT_R3G3B2, D3DFMT_P8, D3DFMT_A8P8,
          // etc.
          break;
        default:
          break;
      ...
      switch (ddspf.RGBBitCount) {
        case 16:
          if (is_rgba_mask(ddspf, 0x0000ffff, 0x00000000, 0x00000000,
                           0x00000000)) {
            desc.format =
                R16_UNORM;  // D3DX10/11 writes this out as DX10 extension.
          }
          if (is_rgba_mask(ddspf, 0x000000ff, 0x00000000, 0x00000000,
                           0x0000ff00)) {
            desc.format =
                R8G8_UNORM;  // D3DX10/11 writes this out as DX10 extension.
          }
          break;
        case 8:
          if (is_rgba_mask(ddspf, 0x000000ff, 0x00000000, 0x00000000,
                           0x00000000)) {
            desc.format =
                R8_UNORM;  // D3DX10/11 writes this out as DX10 extension
          }
          ...
          if (is_rgba_mask(ddspf, 0x000000ff, 0x00000000, 0x00000000,
                           0x0000ff00)) {
            desc.format = R8G8_UNORM;  // Some DDS writers assume the bitcount
                                       // should be 8 instead of 16
          }
          break;
      }
      ...
      if (ddspf.RGBBitCount == 32) {
        if (is_rgba_mask(ddspf, 0x000000ff, 0x0000ff00, 0x00ff0000,
                         0xff000000)) {
          desc.format =
              R8G8B8A8_SNORM;  // D3DX10/11 writes this out as DX10 extension
        }
        if (is_rgba_mask(ddspf, 0x0000ffff, 0xffff0000, 0x00000000,
                         0x00000000)) {
          desc.format =
              R16G16_SNORM;  // D3DX10/11 writes this out as DX10 extension
        }

        // No DXGI format maps to (0x3ff00000, 0x000ffc00, 0x000003ff,
        // 0xc0000000) aka D3DFMT_A2W10V10U10
      } else if (ddspf.RGBBitCount == 16) {
        if (is_rgba_mask(ddspf, 0x000000ff, 0x0000ff00, 0x00000000,
                         0x00000000)) {
          desc.format =
              R8G8_SNORM;  // D3DX10/11 writes this out as DX10 extension
        }
      }
    }
    ...
    if ((header.flags & DDS_HEADER_FLAGS_VOLUME) ||
      ...
      if ((header.caps2 & DDS_HEADER_CAPS2_CUBEMAP_ALLFACES) !=
          DDS_HEADER_CAPS2_CUBEMAP_ALLFACES) {
        return ddspp::Error;
      }
  ...
  desc.rowPitch =
      get_row_pitch(desc.width, desc.bitsPerPixelOrBlock, desc.blockWidth, 0);
  desc.depthPitch = desc.rowPitch * desc.height / desc.blockHeight;
  desc.headerSize = sizeof(DDS_MAGIC) + sizeof(Header) +
                    (dxt10Extension ? sizeof(HeaderDXT10) : 0);

  return ddspp::Success;
}

inline void encode_header(const DXGIFormat format, const unsigned int width,
                          const unsigned int height, const unsigned int depth,
                          const TextureType type, const unsigned int mipCount,
                          const unsigned int arraySize, Header& header,
                          HeaderDXT10& dxt10Header) {
  header.size = sizeof(Header);

  // Fill in header flags
  header.flags = DDS_HEADER_FLAGS_CAPS | DDS_HEADER_FLAGS_HEIGHT |
  ...
  if (is_compressed(format)) {
    ...
    header.pitchOrLinearSize =
        width * height * bitsPerPixelOrBlock / (8 * blockWidth * blockHeight);
  } else {
...
// Slice is either a texture from an array, a face from a cubemap, or a 2D slice
// of a volume texture
inline ddspp_constexpr unsigned int get_offset(const Descriptor& desc,
                                               const unsigned int mip,
                                               const unsigned int slice) {
  // The mip/slice arrangement is different between texture arrays and volume
  // textures
  //
  // Arrays
  ...
  if (desc.type == Texture3D) {
    ...
    for (unsigned int m = 0; m < desc.numMips; ++m) {
      unsigned long long mipSize =
          mip0Size >> 2 * m;  // Divide by 2 in width and height
      mipChainSize += mipSize > desc.bitsPerPixelOrBlock
                          ? mipSize
    ...
    for (unsigned int m = 0; m < mip; ++m) {
      unsigned long long mipSize =
          mip0Size >> 2 * m;  // Divide by 2 in width and height
      offset += mipSize > desc.bitsPerPixelOrBlock ? mipSize
                                                   : desc.bitsPerPixelOrBlock;
    }
  }
