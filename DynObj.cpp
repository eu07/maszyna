...
std::string const TDynamicObject::MED_labels[] = { "masa: ", "amax: ", "Fzad: ", "FmPN: ", "FmED: ", "FrED: ", "FzPN: ", "nPrF: " };
...
{
	auto const lookup{ FileExists({ Global.asCurrentTexturePath + Name, Name, szTexturePath + Name }, { ".mat", ".dds", ".tga", ".bmp" }) };
...
{ // ustawienie typu animacji i zależnej od niego ilości animowanych submodeli
	...
	switch (i) { // maska 0x000F: ile używa wskaźników na submodele (0 gdy jeden,
		// wtedy bez tablicy)
		// maska 0x00F0:
...
TAnim::~TAnim()
{ // usuwanie animacji
	switch (iFlags & 0xF0) { // usuwanie struktur, zależnie ile zostało stworzonych
...
{ // animacja drzwi - przesuw
	if (pAnim->smAnimated) {
		if (pAnim->iNumber & 1) {
			pAnim->smAnimated->SetTranslate(Math3D::vector3{ 0.0, 0.0, dDoorMoveR });
		} else {
			pAnim->smAnimated->SetTranslate(Math3D::vector3{ 0.0, 0.0, dDoorMoveL });
		}
	}
};
...
{ // animacja drzwi - odskokprzesuw
	if (pAnim->smAnimated) {
		if (pAnim->iNumber & 1) {
			pAnim->smAnimated->SetTranslate(Math3D::vector3{ std::min(dDoorMoveR * 2, MoverParameters->DoorMaxPlugShift), 0.0, std::max(0.0, dDoorMoveR - MoverParameters->DoorMaxPlugShift * 0.5f) });
		} else {
			pAnim->smAnimated->SetTranslate(Math3D::vector3{ std::min(dDoorMoveL * 2, MoverParameters->DoorMaxPlugShift), 0.0, std::max(0.0, dDoorMoveL - MoverParameters->DoorMaxPlugShift * 0.5f) });
		}
	}
}
...
void TDynamicObject::UpdatePlatformTranslate(TAnim* pAnim)
{
	...
	if (pAnim->iNumber & 1) {
		pAnim->smAnimated->SetTranslate(Math3D::vector3{ interpolate(0.0, MoverParameters->PlatformMaxShift, dDoorstepMoveR), 0.0, 0.0 });
	} else {
		pAnim->smAnimated->SetTranslate(Math3D::vector3{ interpolate(0.0, MoverParameters->PlatformMaxShift, dDoorstepMoveL), 0.0, 0.0 });
	}
}
...
void TDynamicObject::UpdatePlatformRotate(TAnim* pAnim)
{
	...
	if (pAnim->iNumber & 1)
		pAnim->smAnimated->SetRotate(float3(0, 1, 0), interpolate(0.0, MoverParameters->PlatformMaxShift, dDoorstepMoveR));
	else
		pAnim->smAnimated->SetRotate(float3(0, 1, 0), interpolate(0.0, MoverParameters->PlatformMaxShift, dDoorstepMoveL));
}
...
void TDynamicObject::UpdateMirror(TAnim* pAnim)
{
	...
	auto const isactive{ (
	MoverParameters->ActiveCab > 0 ? ((pAnim->iNumber >> 4) == side::front ? 1.0 : 0.0) : MoverParameters->ActiveCab < 0 ? ((pAnim->iNumber >> 4) == side::rear ? 1.0 : 0.0) : 0.0) };

	if (pAnim->iNumber & 1)
		pAnim->smAnimated->SetRotate(float3(0, 1, 0), interpolate(0.0, MoverParameters->MirrorMaxShift, dMirrorMoveR * isactive));
	else
		pAnim->smAnimated->SetRotate(float3(0, 1, 0), interpolate(0.0, MoverParameters->MirrorMaxShift, dMirrorMoveL * isactive));
}
...
void TDynamicObject::toggle_lights()
{
	if (true == SectionLightsActive) {
		...
		for (auto& sectionlight : SectionLightLevels) {
			...
			if ((compartmentname.find("corridor") == 0) || (compartmentname.find("korytarz") == 0)) {
				// corridors are lit 100% of time
				sectionlight.level = 0.75f;
			} else if ((compartmentname.find("compartment") == 0) || (compartmentname.find("przedzial") == 0)) {
...
{ // ABu290105: pozbierane i uporzadkowane powtarzajace
	...
	if (MoverParameters->Battery || MoverParameters->ConverterFlag) { // sygnały czoła pociagu //Ra: wyświetlamy bez
...
{ // skanowanie toru w poszukiwaniu obiektu najblizszego kamerze
	...
	if (FoundedObj == NULL) {
		...
		if (ScanDir >= 0)
			ActDist = Track->Length() - RaTranslationGet(); //???-przesunięcie wózka względem Point1 toru
		else
...
TDynamicObject* TDynamicObject::ABuFindObject(int& Foundcoupler, double& Distance, TTrack const* Track, int const Direction, int const Mycoupler)
...
{ // rozłączenie sprzęgów rzeczywistych od
	...
	if (ctOwner) { // jeśli pojazd ma przypisany obiekt nadzorujący skład, to póki
		// są wskaźniki
...
{
	if (MoverParameters->Couplers[Side].Connected == nullptr) {
		return;
	}

	if (MoverParameters->Couplers[Side].CouplingFlag == coupling::faux) {
		// najpierw hak
		if ((MoverParameters->Couplers[Side].Connected->Couplers[Side].AllowedFlag & MoverParameters->Couplers[Side].AllowedFlag & coupling::coupler) == coupling::coupler) {
			if (MoverParameters->Attach(Side, 2, MoverParameters->Couplers[Side].Connected, coupling::coupler)) {
	...
	if (false == TestFlag(MoverParameters->Couplers[Side].CouplingFlag, coupling::brakehose)) {
		// pneumatyka
		if ((MoverParameters->Couplers[Side].Connected->Couplers[Side].AllowedFlag & MoverParameters->Couplers[Side].AllowedFlag & coupling::brakehose) == coupling::brakehose) {
			if (MoverParameters->Attach(Side, 2, MoverParameters->Couplers[Side].Connected, (MoverParameters->Couplers[Side].CouplingFlag | coupling::brakehose))) {
	...
	if (false == TestFlag(MoverParameters->Couplers[Side].CouplingFlag, coupling::mainhose)) {
		// zasilajacy
		if ((MoverParameters->Couplers[Side].Connected->Couplers[Side].AllowedFlag & MoverParameters->Couplers[Side].AllowedFlag & coupling::mainhose) == coupling::mainhose) {
			if (MoverParameters->Attach(Side, 2, MoverParameters->Couplers[Side].Connected, (MoverParameters->Couplers[Side].CouplingFlag | coupling::mainhose))) {
				// TODO: dedicated sound for connecting cable-type connections
				m_couplersounds[Side].dsbCouplerDetach.play();
	...
	if (false == TestFlag(MoverParameters->Couplers[Side].CouplingFlag, coupling::control)) {
		// ukrotnionko
		if ((MoverParameters->Couplers[Side].Connected->Couplers[Side].AllowedFlag & MoverParameters->Couplers[Side].AllowedFlag & coupling::control) == coupling::control) {
			if (MoverParameters->Attach(Side, 2, MoverParameters->Couplers[Side].Connected, (MoverParameters->Couplers[Side].CouplingFlag | coupling::control))) {
				// TODO: dedicated sound for connecting cable-type connections
				m_couplersounds[Side].dsbCouplerAttach.play();
				// one coupling type per key press
				return;
	...
	if (false == TestFlag(MoverParameters->Couplers[Side].CouplingFlag, coupling::gangway)) {
		// mostek
		if ((MoverParameters->Couplers[Side].Connected->Couplers[Side].AllowedFlag & MoverParameters->Couplers[Side].AllowedFlag & coupling::gangway) == coupling::gangway) {
			if (MoverParameters->Attach(Side, 2, MoverParameters->Couplers[Side].Connected, (MoverParameters->Couplers[Side].CouplingFlag | coupling::gangway))) {
	...
	if (false == TestFlag(MoverParameters->Couplers[Side].CouplingFlag, coupling::heating)) {
		// heating
		if ((MoverParameters->Couplers[Side].Connected->Couplers[Side].AllowedFlag & MoverParameters->Couplers[Side].AllowedFlag & coupling::heating) == coupling::heating) {
			if (MoverParameters->Attach(Side, 2, MoverParameters->Couplers[Side].Connected, (MoverParameters->Couplers[Side].CouplingFlag | coupling::heating))) {
...
{
	if ((DettachStatus(Side) >= 0) || (true == TestFlag(MoverParameters->Couplers[Side].CouplingFlag, coupling::permanent))) {
...
void TDynamicObject::CouplersDettach(double MinDist, int MyScanDir)
{
	...
	if (MyScanDir > 0) {
		...
		if ((PrevConnected != nullptr) && (MoverParameters->Couplers[side::front].CoupleDist > MinDist)) {
			// sprzęgi wirtualne zawsze przekraczają
			if ((PrevConnectedNo == side::front ? PrevConnected->PrevConnected : PrevConnected->NextConnected) == this) {
		...
		if ((NextConnected != nullptr) && (MoverParameters->Couplers[side::rear].CoupleDist > MinDist)) {
			// sprzęgi wirtualne zawsze przekraczają
			if ((NextConnectedNo == side::front ? NextConnected->PrevConnected : NextConnected->NextConnected) == this) {
				// Ra: nie rozłączamy znalezionego, jeżeli nie do nas podłączony
				// (może jechać w innym kierunku)
...
{ // skanowanie toru w poszukiwaniu kolidujących pojazdów
	...
	auto connectedobject = (initialdirection > 0 ? PrevConnected : NextConnected);
	if ((connectedobject != nullptr) && (connectedobject != foundobject)) {
	...
	if (foundobject) {
		...
		if (foundobject->MoverParameters->Couplers[foundcoupler].CouplingFlag == coupling::faux) {
			...
			if ((foundcoupler == side::front ? foundobject->PrevConnected : foundobject->NextConnected) != this) {
				...
				foundobject->CouplersDettach(1.0, (foundcoupler == side::front ? 1 : -1));
			}
			...
			if (foundcoupler == side::front) {
				...
				if ((DebugModeFlag) && (foundobject->PrevConnected) && (foundobject->PrevConnected != this)) {
					WriteLog("ScanObjects(): formed virtual link between \"" + asName + "\" (coupler " + to_string(mycoupler) + ") and \"" + foundobject->asName + "\" (coupler " +
					         to_string(foundcoupler) + ")");
				}
				foundobject->PrevConnected = this;
				foundobject->PrevConnectedNo = mycoupler;
			} else {
				// jeśli widoczny sprzęg 1 znalezionego
				if ((DebugModeFlag) && (foundobject->NextConnected) && (foundobject->NextConnected != this)) {
					WriteLog("ScanObjects(): formed virtual link between \"" + asName + "\" (coupler " + to_string(mycoupler) + ") and \"" + foundobject->asName + "\" (coupler " +
					         to_string(foundcoupler) + ")");
				}
		...
		if ((false == TestFlag(track->iCategoryFlag, 1)) && (distance > 50.0)) {
	...
	if ((PrevConnected != nullptr) && (PrevConnected == NextConnected)) {
...
double TDynamicObject::Init(std::string Name, // nazwa pojazdu, np. "EU07-424"
...
*/
	...
	if (!MoverParameters->LoadFIZ(asBaseDir)) { // jak wczytanie CHK się nie uda, to błąd
	...
	if (!MoverParameters->CheckLocomotiveParameters(driveractive,
	    (fVel > 0 ? 1 : -1) * Cab * (iDirection ? 1 : -1))) // jak jedzie lub obsadzony to gotowy do drogi
	{
	...
	if (driveractive) {
		if (Cab == 0)
			MoverParameters->BrakeCtrlPos = static_cast<int>(std::floor(MoverParameters->Handle->GetPos(bh_NP)));
		else
			MoverParameters->BrakeCtrlPos = static_cast<int>(std::floor(MoverParameters->Handle->GetPos(bh_RP)));
	} else
		MoverParameters->BrakeCtrlPos = static_cast<int>(std::floor(MoverParameters->Handle->GetPos(bh_NP)));

	MoverParameters->BrakeLevelSet(MoverParameters->BrakeCtrlPos); // poprawienie hamulca po ewentualnym
	...
	while (kropka != std::string::npos) // jesli sa kropki jeszcze
	{
		...
		else if ((ActPar.size() >= 3) && (ActPar[0] == 'W')) {
			// wheel
			ActPar.erase(0, 1);
		...
		else if ((ActPar.size() >= 2) && (ActPar[0] == 'T')) {
			// temperature
			ActPar.erase(0, 1);
...
*/
	...
	{ // ustawianie samochodow na poboczu albo na środku drogi
		...
		// ruchu
			...
			MoverParameters->OffsetTrackH = (lanefreespace > 1.5 ? -0.5 * MoverParameters->Dim.W - 0.75 : // wide road, keep near centre with safe margin
			                                 (lanefreespace > 0.1 ? -0.25 * Track->fTrackWidth : // narrow fit, stay on the middle
...
*/
	...
	if (mdLowPolyInt != nullptr) {
		...
		if ((submodel = mdLowPolyInt->GetFromName("cab1")) != nullptr) {
			SectionLightLevels.push_back({ submodel, nullptr, 0.0f });
		}
		...
		for (auto const& nameprefix : nameprefixes) {
			compartmentindex = 0;
			do {
				compartmentname = nameprefix + (compartmentindex < 10 ? "0" + std::to_string(compartmentindex) : std::to_string(compartmentindex));
				submodel = mdLowPolyInt->GetFromName(compartmentname);
				if (submodel != nullptr) {
					SectionLightLevels.push_back({ submodel,
					nullptr, // pointers to load sections are generated afterwards
					0.0f });
				}
				++compartmentindex;
			} while ((submodel != nullptr) || (compartmentindex < 2)); // chain can start from prefix00 or prefix01
		}
	...
	for (auto& axle : m_axlesounds) {
		...
		axle.distance = (Reversed ? -axle.offset : (axle.offset + MoverParameters->Dim.L)) + fDist;
	}
	// McZapkie-250202 end.
...
void TDynamicObject::create_controller(std::string const Type, bool const Trainset)
{
	if (Type == "") {
		return;
	}
	...
	if ((Type == "1") || (Type == "2")) {
		...
		if (false == Trainset) {
			...
			Mechanik->PutCommand("Timetable:", MoverParameters->V * 3.6 * (iDirection ? -1.0 : 1.0), 0, nullptr);
		}
	} else if (Type == "p") {
...
{ // przesuwanie pojazdu po
	...
	if (fDistance > 0.0) { // gdy ruch w stronę sprzęgu 0, doliczyć korektę do osi 1
	...
	} else if (fDistance < 0.0) { // gdy ruch w stronę sprzęgu 1, doliczyć korektę do osi 0
	...
	{ // liczenie pozycji pojazdu tutaj, bo jest używane w wielu miejscach
		...
		if (a != 0.0) { // wyznaczanie przechylenia tylko jeśli jest przechyłka
...
{
	if ((MoverParameters->DoorOpenCtrl == control::passenger) || (MoverParameters->DoorOpenCtrl == control::mixed)) {
	...
	m_exchange.speed_factor = (Platform == 3 ? 2.0 : 1.0);
	m_exchange.time = 0.0;
}
...
void TDynamicObject::update_exchange(double const Deltatime)
{
	...
	if ((MoverParameters->Vel < 2.0) && ((true == MoverParameters->DoorLeftOpened) || (true == MoverParameters->DoorRightOpened))) {
		m_exchange.time += Deltatime;
		while ((m_exchange.unload_count > 0.01) && (m_exchange.time >= 1.0)) {
			m_exchange.time -= 1.0;
		...
		if (m_exchange.unload_count < 0.01) {
			...
			while ((m_exchange.load_count > 0.01) && (m_exchange.time >= 1.0)) {
				m_exchange.time -= 1.0;
	...
	if ((m_exchange.unload_count < 0.01) && (m_exchange.load_count < 0.01)) {
		...
		if ((MoverParameters->DoorCloseCtrl == control::passenger) || (MoverParameters->DoorCloseCtrl == control::mixed)) {
			if ((MoverParameters->Vel > 2.0) || (Random() < (
			                                                // remotely controlled door are more likely to be left open
			                                                MoverParameters->DoorCloseCtrl == control::passenger ? 0.75 : 0.50))) {
...
void TDynamicObject::LoadUpdate()
{
	...
	if ((mdLoad == nullptr) && (MoverParameters->Load > 0)) {
...
void TDynamicObject::update_load_sections()
{
	...
	for (auto& section : SectionLightLevels) {
		section.load = (mdLoad != nullptr ? mdLoad->GetFromName(section.compartment->pName) : nullptr);

		if ((section.load != nullptr) && (section.load->count_children() > 0)) {
...
*/
	auto loadpercentage{ (MoverParameters->MaxLoad == 0.0 ? 0.0 : 100.0 * MoverParameters->Load / MoverParameters->MaxLoad) };
	auto const sectionloadpercentage{ (SectionLoadVisibility.empty() ? 0.0 : 100.0 / SectionLoadVisibility.size()) };
	// set as many sections as we can, given overall load percentage and how much of full percentage is covered by each chunk
	std::for_each(std::begin(SectionLoadVisibility), std::end(SectionLoadVisibility), [&](section_visibility& section) {
		section.visible = (loadpercentage > 0.0);
		section.visible_chunks = 0;
		auto const sectionchunkcount{ section.submodel->count_children() };
		auto const sectionchunkloadpercentage{ (sectionchunkcount == 0 ? 0.0 : sectionloadpercentage / sectionchunkcount) };
		auto* sectionchunk{ section.submodel->ChildGet() };
		while (sectionchunk != nullptr) {
...
void TDynamicObject::shuffle_load_sections()
{
	...
	std::stable_partition(std::begin(SectionLoadVisibility), std::end(SectionLoadVisibility),
	[](section_visibility const& section) { return ((section.submodel->pName.find("compartment") == 0) || (section.submodel->pName.find("przedzial") == 0)); });
}
...
*/
	...
	//    wczytywaniu
	if (Axle0.vAngles.z != Axle1.vAngles.z) { // wyliczenie promienia z obrotów osi - modyfikację zgłosił youBy
	...
	// couplers
	if ((MoverParameters->Couplers[0].CouplingFlag != coupling::faux) && (MoverParameters->Couplers[1].CouplingFlag != coupling::faux)) {
	...
	if (MoverParameters->EnginePowerSource.SourceType == CurrentCollector) { // dla EZT tylko silnikowy
				...
				if (NoVoltTime > 0.2) {
					...
					if (MoverParameters->Vel > 0.5) {
						...
						if (MoverParameters->PantFrontUp || MoverParameters->PantRearUp) {
							if ((MoverParameters->Mains) && (MoverParameters->GetTrainsetVoltage() < 0.1f)) {
								...
								ErrorLog("Bad traction: " + MoverParameters->Name + " lost power for " + to_string(NoVoltTime, 2) + " sec. at " + to_string(glm::dvec3{ vPosition }));
							}
						}
					}
	...
	if (Mechanik) { // Ra 2F3F: do Driver.cpp to przenieść?
		...
		if ((Mechanik->Primary()) && (MoverParameters->EngineType == ElectricInductionMotor)) {
			...
			for (TDynamicObject* p = GetFirstDynamic(MoverParameters->ActiveCab < 0 ? 1 : 0, 4); p; (kier ? p = p->NextC(4) : p = p->PrevC(4))) {
				++np;
				masamax += p->MoverParameters->MBPM + (p->MoverParameters->MBPM > 1.0 ? 0.0 : p->MoverParameters->Mass) + p->MoverParameters->Mred;
				auto const Nmax =
				((p->MoverParameters->P2FTrans * p->MoverParameters->MaxBrakePress[0] - p->MoverParameters->BrakeCylSpring) * p->MoverParameters->BrakeCylMult[0] - p->MoverParameters->BrakeSlckAdj) *
				p->MoverParameters->BrakeCylNo * p->MoverParameters->BrakeRigEff;
				FmaxPN += Nmax * p->MoverParameters->Hamulec->GetFC(Nmax / (p->MoverParameters->NAxles * p->MoverParameters->NBpA),
				                 p->MoverParameters->MED_Vref) * 1000; // sila hamowania pn
				FmaxED += ((p->MoverParameters->Mains) && (p->MoverParameters->ActiveDir != 0) && (p->MoverParameters->eimc[eimc_p_Fh] * p->MoverParameters->NPoweredAxles > 0) ?
				           p->MoverParameters->eimc[eimc_p_Fh] * 1000 :
				           0); // chwilowy max ED -> do rozdzialu sil
				FED -= std::min(p->MoverParameters->eimv[eimv_Fmax], 0.0) * 1000; // chwilowy max ED -> do rozdzialu sil
				FfulED = std::min(p->MoverParameters->eimv[eimv_Fful], 0.0) * 1000; // chwilowy max ED -> do rozdzialu sil
				FrED -= std::min(p->MoverParameters->eimv[eimv_Fmax], 0.0) * 1000; // chwilowo realizowane ED -> do pneumatyki
				Frj += std::max(p->MoverParameters->eimv[eimv_Fmax], 0.0) * 1000; // chwilowo realizowany napęd -> do utrzymującego
				masa += p->MoverParameters->TotalMass;
				osie += p->MoverParameters->NAxles;
			}
			double RapidMult = 1.0;
			if (((MoverParameters->BrakeDelays & (bdelay_P + bdelay_R)) == (bdelay_P + bdelay_R)) && (MoverParameters->BrakeDelayFlag & bdelay_P))
				RapidMult = MoverParameters->RapidMult;

			auto const amax = RapidMult * std::min(FmaxPN / masamax, MoverParameters->MED_amax);

			if ((MoverParameters->Vel < 0.5) && (MoverParameters->BrakePress > 0.2) || (dDoorMoveL > 0.001) || (dDoorMoveR > 0.001)) {
				MoverParameters->ShuntMode = true;
			}
			if (MoverParameters->ShuntMode) {
				MoverParameters->ShuntModeAllow = (dDoorMoveL < 0.001) && (dDoorMoveR < 0.001) && (MoverParameters->LocalBrakeRatio() < 0.01);
			}
			if ((MoverParameters->Vel > 1) && (dDoorMoveL < 0.001) && (dDoorMoveR < 0.001)) {
				MoverParameters->ShuntMode = false;
				MoverParameters->ShuntModeAllow = (MoverParameters->BrakePress > 0.2) && (MoverParameters->LocalBrakeRatio() < 0.01);
			}
			auto Fzad = amax * MoverParameters->LocalBrakeRatio() * masa;
			if ((MoverParameters->ScndS) && (MoverParameters->Vel > MoverParameters->eimc[eimc_p_Vh1]) && (FmaxED > 0)) {
				Fzad = std::min(MoverParameters->LocalBrakeRatio() * FmaxED, FfulED);
			}
			if (((MoverParameters->ShuntMode) && (Frj < 0.0015 * masa)) || (MoverParameters->V * MoverParameters->DirAbsolute < -0.2)) {
				Fzad = std::max(MoverParameters->StopBrakeDecc * masa, Fzad);
			}
			if ((Fzad > 1) && (!MEDLogFile.is_open()) && (MoverParameters->Vel > 1)) {
				MEDLogFile.open(std::string("MEDLOGS/" + MoverParameters->Name + "_" + to_string(++MEDLogCount) + ".csv"), std::ios::in | std::ios::out | std::ios::trunc);
			...
			auto FzadED{ 0.0 };
			if ((MoverParameters->EpFuse && (MoverParameters->BrakeHandle != MHZ_EN57)) || ((MoverParameters->BrakeHandle == MHZ_EN57) && (MoverParameters->BrakeOpModeFlag & bom_MED))) {
			...
			int i = 0;
			for (TDynamicObject* p = GetFirstDynamic(MoverParameters->ActiveCab < 0 ? 1 : 0, 4); p; p = (kier == true ? p->NextC(4) : p->PrevC(4))) {
				auto const Nmax =
				((p->MoverParameters->P2FTrans * p->MoverParameters->MaxBrakePress[0] - p->MoverParameters->BrakeCylSpring) * p->MoverParameters->BrakeCylMult[0] - p->MoverParameters->BrakeSlckAdj) *
				p->MoverParameters->BrakeCylNo * p->MoverParameters->BrakeRigEff;
				FmaxEP[i] = Nmax * p->MoverParameters->Hamulec->GetFC(Nmax / (p->MoverParameters->NAxles * p->MoverParameters->NBpA),
				                   p->MoverParameters->MED_Vref) * 1000; // sila hamowania pn

				PrzekrF[i] = false;
				FzED[i] = (FmaxED > 0 ? FzadED / FmaxED : 0);
				p->MoverParameters->AnPos = (MoverParameters->ScndS ? MoverParameters->LocalBrakeRatio() : FzED[i]);
			...
			while (test) {
				...
				float przek = 0;
				for (TDynamicObject* p = GetFirstDynamic(MoverParameters->ActiveCab < 0 ? 1 : 0, 4); p; p = (kier == true ? p->NextC(4) : p->PrevC(4))) {
					if ((FzEP[i] > 0.01) && (FzEP[i] > p->MoverParameters->TotalMass * p->MoverParameters->eimc[eimc_p_eped] + Min0R(p->MoverParameters->eimv[eimv_Fmax], 0) * 1000) && (!PrzekrF[i])) {
						float przek1 = -Min0R(p->MoverParameters->eimv[eimv_Fmax], 0) * 1000 + FzEP[i] - p->MoverParameters->TotalMass * p->MoverParameters->eimc[eimc_p_eped] * 0.999;
						PrzekrF[i] = true;
						test = true;
						nPrzekrF++;
				...
				for (TDynamicObject* p = GetFirstDynamic(MoverParameters->ActiveCab < 0 ? 1 : 0, 4); p; (true == kier ? p = p->NextC(4) : p = p->PrevC(4))) {
					if (!PrzekrF[i]) {
						FzEP[i] += przek;
					}
					++i;
			...
			for (TDynamicObject* p = GetFirstDynamic(MoverParameters->ActiveCab < 0 ? 1 : 0, 4); p; (true == kier ? p = p->NextC(4) : p = p->PrevC(4))) {
				...
				((p->MoverParameters->P2FTrans * p->MoverParameters->MaxBrakePress[0] - p->MoverParameters->BrakeCylSpring) * p->MoverParameters->BrakeCylMult[0] - p->MoverParameters->BrakeSlckAdj) *
				p->MoverParameters->BrakeCylNo * p->MoverParameters->BrakeRigEff;
				float VelC = ((FrED > 0.1) || p->MoverParameters->MED_EPVC ? clamp(p->MoverParameters->Vel, p->MoverParameters->MED_Vmin, p->MoverParameters->MED_Vmax) :
				                                                             p->MoverParameters->MED_Vref); //korekcja EP po prędkości
				float FmaxPoj = Nmax * p->MoverParameters->Hamulec->GetFC(Nmax / (p->MoverParameters->NAxles * p->MoverParameters->NBpA), VelC) * 1000; // sila hamowania pn
				...
				if (p->MoverParameters->LocalBrakePosA > 0.009)
					if (p->MoverParameters->P2FTrans * p->MoverParameters->BrakeCylMult[0] * p->MoverParameters->MaxBrakePress[0] != 0) {
						float x = (p->MoverParameters->BrakeSlckAdj / p->MoverParameters->BrakeCylMult[0] + p->MoverParameters->BrakeCylSpring) /
			...
			if (MEDLogFile.is_open()) {
				MEDLogFile << MEDLogTime << "\t" << MoverParameters->Vel << "\t" << masa * 0.001 << "\t" << osie << "\t" << FmaxPN * 0.001 << "\t" << FmaxED * 0.001 << "\t" << FfulED * 0.001 << "\t"
				           << FrED * 0.001 << "\t" << Fzad * 0.001 << "\t" << FzadED * 0.001 << "\t" << FzadPN * 0.001;
				for (TDynamicObject* p = GetFirstDynamic(MoverParameters->ActiveCab < 0 ? 1 : 0, 4); p; (true == kier ? p = p->NextC(4) : p = p->PrevC(4))) {
	...
	if (MoverParameters->Mains) // nie wchodzić w funkcję bez potrzeby
        ...
        // NOTE: disabled on account of multi-unit setups, where the unmanned unit wouldn't be affected
          && ( Controller == Humandriver )
*/
		    && (MoverParameters->EngineType != DieselEngine) && (MoverParameters->EngineType != WheelsDriven)) { // jeśli bateria wyłączona, a nie diesel ani drezyna reczna
...
*/
	...
	if (MyTrack->fSoundDistance != -1) {
		...
		if (dRailLength != -1) {
			if (MoverParameters->Vel > 0) {
				...
				auto volume = interpolate(0.8, 1.2, clamp(MyTrack->iQualityFlag / 20.0, 0.0, 1.0));
				...
				for (auto& axle : m_axlesounds) {
					...
					if (axle.distance < 0) {
						...
						if (MoverParameters->Vel > 2.5) {
							...
							axle.clatter.pitch(frequency).gain(volume).play();
							// crude bump simulation, drop down on even axles, move back up on the odd ones
							MoverParameters->AccVert += interpolate(0.01, 0.05, clamp(GetVelocity() / (1 + MoverParameters->Vmax), 0.0, 1.0)) * ((axleindex % 2) != 0 ? 1 : -1);
						}
					}
					++axleindex;
				}
	...
	{ // przeliczanie kątów dla pantografów
		...
		double fCurrent = ((MoverParameters->DynamicBrakeFlag && MoverParameters->ResistorsFlag) ? 0 : MoverParameters->Itot) + MoverParameters->TotalCurrent; // prąd pobierany przez pojazd - bez
		...
		for (int i = 0; i < iAnimType[ANIM_PANTS]; ++i) { // pętla po wszystkich pantografach
			p = pants[i].fParamPants;
			if (p->PantWys < 0) { // patograf został połamany, liczony nie będzie
			...
			{ // trzeba usunąć to rozróżnienie
				case 0:
					if ((Global.bLiveTraction == false) && (p->hvPowerWire == nullptr)) {
					...
					} else if ((true == MoverParameters->PantFrontUp) && (PantDiff < 0.01)) // tolerancja niedolegania
					{
						if (p->hvPowerWire) {
							...
							if ((lastvoltage == 0.0) && (MoverParameters->PantFrontVolt > 0.0)) {
					...
					break;
				case 1:
					if ((false == Global.bLiveTraction) && (nullptr == p->hvPowerWire)) {
						// jeśli nie ma drutu, może pooszukiwać
						MoverParameters->PantRearVolt = (p->PantWys >= 1.2) ? 0.95 * MoverParameters->EnginePowerSource.MaxVoltage : 0.0;
					} else if ((true == MoverParameters->PantRearUp) && (PantDiff < 0.01)) {
						if (p->hvPowerWire) {
							...
							fCurrent -= fPantCurrent; // taki prąd płynie przez powyższy pantograf
							// TODO: refactor reaction to voltage change to mover as sound event for specific pantograph
							if ((lastvoltage == 0.0) && (MoverParameters->PantRearVolt > 0.0)) {
								for (auto& pantograph : m_pantographsounds) {
									if (pantograph.sPantUp.offset().z < 0) {
			...
			if (MoverParameters->PantPress > (MoverParameters->TrainType == dt_EZT ? 2.45 : // Ra 2013-12: Niebugocław mówi, że w EZT podnoszą się przy 2.5
			                                  3.45)) {
				// z EXE Kursa
			...
			if ((false == MoverParameters->Battery) && (false == MoverParameters->ConverterFlag)) {
				pantspeedfactor = 0.0;
			}
			pantspeedfactor = std::max(0.0, pantspeedfactor);
			k = p->fAngleL;
			if ((pantspeedfactor > 0.0) && (i ? MoverParameters->PantRearUp : MoverParameters->PantFrontUp)) // jeśli ma być podniesiony
			{
			...
			if (k != p->fAngleL) { //żeby nie liczyć w kilku miejscach ani gdy nie potrzeba
				if (k + p->fAngleU < M_PI) { // o ile nie został osiągnięty kąt maksymalny
...
*/
	...
	if ((dDoorMoveL < MoverParameters->DoorMaxShiftL) && (true == MoverParameters->DoorLeftOpened)) {
		// open left door
		if ((MoverParameters->TrainType == dt_EZT) || (MoverParameters->TrainType == dt_DMU)) {
			...
			if ((MoverParameters->PlatformMaxShift == 0.0) // no wait if no doorstep
			    ...
			    || (dDoorstepMoveL == 1.0)) {
				dDoorMoveL = std::min(MoverParameters->DoorMaxShiftL, dDoorMoveL + MoverParameters->DoorOpenSpeed * dt1);
			}
		} else {
			dDoorMoveL = std::min(MoverParameters->DoorMaxShiftL, dDoorMoveL + MoverParameters->DoorOpenSpeed * dt1);
		}
	...
	if ((dDoorMoveL > 0.0) && (false == MoverParameters->DoorLeftOpened)) {
		// close left door
		DoorDelayL += dt1;
	...
	if ((dDoorMoveR < MoverParameters->DoorMaxShiftR) && (true == MoverParameters->DoorRightOpened)) {
		// open right door
		if ((MoverParameters->TrainType == dt_EZT) || (MoverParameters->TrainType == dt_DMU)) {
			// multi-unit vehicles typically open door only after unfolding the doorstep
			if ((MoverParameters->PlatformMaxShift == 0.0) // no wait if no doorstep
			    || (MoverParameters->PlatformOpenMethod == 2) // no wait for rotating doorstep
			    || (dDoorstepMoveR == 1.0)) {
				dDoorMoveR = std::min(MoverParameters->DoorMaxShiftR, dDoorMoveR + MoverParameters->DoorOpenSpeed * dt1);
			}
		} else {
			dDoorMoveR = std::min(MoverParameters->DoorMaxShiftR, dDoorMoveR + MoverParameters->DoorOpenSpeed * dt1);
		}
	...
	if ((dDoorMoveR > 0.0) && (false == MoverParameters->DoorRightOpened)) {
		// close right door
		DoorDelayR += dt1;
	...
	// doorsteps
	if ((dDoorstepMoveL < 1.0) && (true == MoverParameters->DoorLeftOpened)) {
		// unfold left doorstep
		dDoorstepMoveL = std::min(1.0, dDoorstepMoveL + MoverParameters->PlatformSpeed * dt1);
	}
	if ((dDoorstepMoveL > 0.0) && (false == MoverParameters->DoorLeftOpened) && (DoorDelayL > MoverParameters->DoorCloseDelay)) {
		// fold left doorstep
		if ((MoverParameters->TrainType == dt_EZT) || (MoverParameters->TrainType == dt_DMU)) {
			// multi-unit vehicles typically fold the doorstep only after closing the door
			if (dDoorMoveL == 0.0) {
				dDoorstepMoveL = std::max(0.0, dDoorstepMoveL - MoverParameters->PlatformSpeed * dt1);
			}
		} else {
			dDoorstepMoveL = std::max(0.0, dDoorstepMoveL - MoverParameters->PlatformSpeed * dt1);
		}
	}
	if ((dDoorstepMoveR < 1.0) && (true == MoverParameters->DoorRightOpened)) {
		// unfold right doorstep
		dDoorstepMoveR = std::min(1.0, dDoorstepMoveR + MoverParameters->PlatformSpeed * dt1);
	}
	if ((dDoorstepMoveR > 0.0) && (false == MoverParameters->DoorRightOpened) && (DoorDelayR > MoverParameters->DoorCloseDelay)) {
		// fold right doorstep
		if ((MoverParameters->TrainType == dt_EZT) || (MoverParameters->TrainType == dt_DMU)) {
			// multi-unit vehicles typically fold the doorstep only after closing the door
			if (dDoorMoveR == 0.0) {
				dDoorstepMoveR = std::max(0.0, dDoorstepMoveR - MoverParameters->PlatformSpeed * dt1);
			}
		} else {
			dDoorstepMoveR = std::max(0.0, dDoorstepMoveR - MoverParameters->PlatformSpeed * dt1);
		}
	...
	if (MoverParameters->Vel > 5.0) {
		...
		if ((dMirrorMoveL < 1.0) && (true == MoverParameters->DoorLeftOpened)) {
			dMirrorMoveL = std::min(1.0, dMirrorMoveL + 1.0 * dt1);
		}
		if ((dMirrorMoveR < 1.0) && (true == MoverParameters->DoorRightOpened)) {
	...
	if ((ctOwner != nullptr ? ctOwner->Controlling()->Battery != SectionLightsActive : SectionLightsActive == true)) { // without controller lights are off. NOTE: this likely mess up the EMU
		toggle_lights();
	}
...
void TDynamicObject::RenderSounds()
{
	...
	if (MoverParameters->ConverterFlag) {
		frequency = (MoverParameters->EngineType == ElectricSeriesMotor ?
	...
	if (m_lastbrakepressure != -1.f) {
		...
		if (quantizedratiochange > 0) {
			...
			.pitch(true == m_brakecylinderpistonadvance.is_combined() ? quantizedratio * 0.01f : m_brakecylinderpistonadvance.m_frequencyoffset + m_brakecylinderpistonadvance.m_frequencyfactor * 1.f)
			.play();
		} else if (quantizedratiochange < 0) {
			m_brakecylinderpistonrecede
			.pitch(true == m_brakecylinderpistonrecede.is_combined() ? quantizedratio * 0.01f : m_brakecylinderpistonrecede.m_frequencyoffset + m_brakecylinderpistonrecede.m_frequencyfactor * 1.f)
			.play();
		}
	}
	...
	if (volume > 0.075f) {
		rsUnbrake.gain(volume).play(sound_flags::exclusive | sound_flags::looping);
	} else {
	...
	if (MoverParameters->Hamulec->GetStatus() & b_rls) {
		sReleaser
		.gain(clamp<float>(MoverParameters->BrakePress * 1.25f, // arbitrary multiplier
		0.f, 1.f))
		.play(sound_flags::exclusive | sound_flags::looping);
	} else {
		sReleaser.stop();
	...
	if (MoverParameters->SlippingWheels) {
		if ((MoverParameters->UnitBrakeForce > 100.0) && (GetVelocity() > 1.0)) {
			auto const velocitydifference{ GetVelocity() / MoverParameters->Vmax };
			rsSlippery.gain(rsSlippery.m_amplitudeoffset + rsSlippery.m_amplitudefactor * velocitydifference).play(sound_flags::exclusive | sound_flags::looping);
		}
	} else {
		rsSlippery.stop();
	}
	...
	(MoverParameters->UnitBrakeForce > 10.0) && (MoverParameters->Vel > 0.05)) {
		brakeforceratio =
		clamp(MoverParameters->UnitBrakeForce / std::max(1.0, MoverParameters->BrakeForceR(1.0, MoverParameters->Vel) / (MoverParameters->NAxles * std::max(1, MoverParameters->NBpA))), 0.0, 1.0);
		rsBrake.pitch(rsBrake.m_frequencyoffset + MoverParameters->Vel * rsBrake.m_frequencyfactor)
	...
	if (MoverParameters->Vel > 2.5) {
		...
		if (volume > 0.075) {
			rsPisk.gain(volume).play(sound_flags::exclusive | sound_flags::looping);
		}
	} else {
		// don't stop the sound too abruptly
...
*/
	...
	if (true == MoverParameters->DoorLeftOpened) {
		...
		if ((dDoorMoveL > 0.0) && (dDoorMoveL < MoverParameters->DoorMaxShiftL)) {
	...
	if (false == MoverParameters->DoorLeftOpened) {
		...
		if ((dDoorstepMoveL > 0.0) && (dDoorstepMoveL < 1.0)) {
			for (auto& door : m_doorsounds) {
				if (door.step_close.offset().x > 0.f) {
					// determine left side doors from their offset
	...
	if (true == MoverParameters->DoorRightOpened) {
		...
		// door sounds
		// due to potential wait for the doorstep we play the sound only during actual animation
		if ((dDoorMoveR > 0.0) && (dDoorMoveR < MoverParameters->DoorMaxShiftR)) {
			for (auto& door : m_doorsounds) {
				if (door.rsDoorClose.offset().x < 0.f) {
	...
	if (false == MoverParameters->DoorRightOpened) {
		...
		// doorstep sounds are played only when the doorstep is moving
		if ((dDoorstepMoveR > 0.0) && (dDoorstepMoveR < 1.0)) {
			for (auto& door : m_doorsounds) {
				if (door.step_close.offset().x < 0.f) {
					// determine left side doors from their offset
					door.step_close.play(sound_flags::exclusive);
	...
	// szum w czasie jazdy
	if ((GetVelocity() > 0.5) && (false == m_bogiesounds.empty()) &&
		...
		auto const normalizer{ (true == bogiesound.is_combined() ? MoverParameters->Vmax * 0.01f : 1.f) };
		frequency = bogiesound.m_frequencyoffset + bogiesound.m_frequencyfactor * MoverParameters->Vel * normalizer;

		// volume calculation
		volume = bogiesound.m_amplitudeoffset + bogiesound.m_amplitudefactor * MoverParameters->Vel;
		...
		// TODO: track quality and/or environment factors as separate subroutine
		volume *= interpolate(0.8, 1.2, clamp(MyTrack->iQualityFlag / 20.0, 0.0, 1.0));
		// for single sample sounds muffle the playback at low speeds
		if (false == bogiesound.is_combined()) {
			volume *= interpolate(0.0, 1.0, clamp(MoverParameters->Vel / 40.0, 0.0, 1.0));
		}
	...
	if (MoverParameters->CategoryFlag == 1) {
		...
		if ((GetVelocity() > 1.0) && (MoverParameters->WheelFlat > 5.0)) {
			m_wheelflat.pitch(m_wheelflat.m_frequencyoffset + std::abs(MoverParameters->nrot) * m_wheelflat.m_frequencyfactor)
	...
	if ((ts.R * ts.R > 1) && (MoverParameters->Vel > 0)) {
		...
		volume = MoverParameters->AccN * MoverParameters->AccN * interpolate(0.0, 1.0, clamp(MoverParameters->Vel / 40.0, 0.0, 1.0));
	} else {
		volume = 0;
	}
	if (volume > 0.05) {
		rscurve.gain(2.5 * volume).play(sound_flags::exclusive | sound_flags::looping);
	} else {
		rscurve.stop();
	}
	...
	if (TestFlag(MoverParameters->SoundFlag, sound::pneumatic)) {
		// pneumatic relay
		dsbPneumaticRelay.gain(true == TestFlag(MoverParameters->SoundFlag, sound::loud) ? 1.0f : 0.8f).play();
	}
	// couplers
	int couplerindex{ 0 };
		...
		if (true == TestFlag(coupler.sounds, sound::bufferclash)) {
			...
			if (true == TestFlag(coupler.sounds, sound::loud)) {
				...
				if (false == couplersounds.dsbBufferClamp_loud.empty()) {
					...
					couplersounds.dsbBufferClamp_loud.gain(1.f).play(sound_flags::exclusive);
				} else {
					// fallback on the standard sound
					couplersounds.dsbBufferClamp.gain(1.f).play(sound_flags::exclusive);
				}
			} else {
				// basic clash
				couplersounds.dsbBufferClamp.gain(0.65f).play(sound_flags::exclusive);
			}
		}
		if (true == TestFlag(coupler.sounds, sound::couplerstretch)) {
			...
			if (true == TestFlag(coupler.sounds, sound::loud)) {
				...
				if (false == couplersounds.dsbCouplerStretch_loud.empty()) {
					...
					couplersounds.dsbCouplerStretch_loud.gain(1.f).play(sound_flags::exclusive);
				} else {
					// fallback on the standard sound
					couplersounds.dsbCouplerStretch.gain(1.f).play(sound_flags::exclusive);
				}
			} else {
				// basic clash
				couplersounds.dsbCouplerStretch.gain(0.65f).play(sound_flags::exclusive);
			}
		}
...
void TDynamicObject::LoadMMediaFile(std::string BaseDir, std::string TypeName, std::string ReplacableSkin)
{
	...
	do {
		...
		if ((token == "models:") || (token == "\xef\xbb\xbfmodels:")) { // crude way to handle utf8 bom potentially appearing before the first token
			// modele i podmodele
			...
			if (i != std::string::npos) { // Ra 2015-01: może szukać przecinka w nazwie modelu, a po przecinku była by liczba tekstur?
				if (i < asModel.length()) {
			...
			if (ReplacableSkin != "none") {
				...
				if ((m_materialdata.replacable_skins[2]) && (GfxRenderer.Material(m_materialdata.replacable_skins[2]).has_alpha)) {
					// tekstura -2 z kanałem alfa - nie renderować w cyklu nieprzezroczystych
					m_materialdata.textures_alpha |= 0x02020002;
				}
				if ((m_materialdata.replacable_skins[3]) && (GfxRenderer.Material(m_materialdata.replacable_skins[3]).has_alpha)) {
					// tekstura -3 z kanałem alfa - nie renderować w cyklu nieprzezroczystych
					m_materialdata.textures_alpha |= 0x04040004;
				}
				if ((m_materialdata.replacable_skins[4]) && (GfxRenderer.Material(m_materialdata.replacable_skins[4]).has_alpha)) {
			...
			if (!MoverParameters->LoadAccepted.empty()) {
				if ((MoverParameters->EnginePowerSource.SourceType == CurrentCollector) && (asLoadName == "pantstate")) {
					...
					if (false == asLoadName.empty()) {
						...
						if ((true == FileExists(specializedloadfilename + ".e3d")) || (true == FileExists(specializedloadfilename + ".t3d"))) {
							mdLoad = TModelsManager::GetModel(specializedloadfilename, true);
						}
						if (mdLoad == nullptr) {
							// if this fails, try generic load model
...
*/
					if (true == pAnimations.empty()) { // jeśli nie ma jeszcze tabeli animacji, można odczytać nowe ilości
						int co = 0;
						iAnimations = 0;
						int ile;
						do { // kolejne liczby to ilość animacj, -1 to znacznik końca
							ile = -1;
					...
					if ((MoverParameters->WheelDiameterL != MoverParameters->WheelDiameter) ||
					    (MoverParameters->WheelDiameterT != MoverParameters->WheelDiameter)) { // obsługa różnych średnic, o ile występują
						while ((i < iAnimType[ANIM_WHEELS]) && (j <= MoverParameters->AxleArangement.length())) { // wersja ze wskaźnikami jest bardziej elastyczna na nietypowe układy
				...
				// Pantografy - Winger 160204
					...
					TSubModel* sm;
					if (pants)
						for (int i = 0; i < iAnimType[ANIM_PANTS]; i++) { // Winger 160204: wyszukiwanie max 2 patykow o nazwie str*
							asAnimName = token + std::to_string(i + 1);
							...
							if (sm) { // w EP09 wywalało się tu z powodu NULL
								...
								if ((sm = pants[i].smElement[0]->ChildGet()) != NULL) { // jeśli ma potomny, można policzyć długość (odległość potomnego od osi obrotu)
									...
									if ((sm = sm->ChildGet()) != NULL) { // jeśli dalej jest ślizg, można policzyć długość górnego ramienia
										...
										} else { // gdy ktoś przesadził ze skalowaniem
											pants[i].fParamPants->fHeight = 0.0; // niech będzie odczyt z pantfactors:
											ErrorLog("Bad model: " + asModel + ", scale of " + (sm->pName) + " is " + std::to_string(100.0 * det) + "%", logtype::model);
										}
									}
								}
							} else {
					...
					parser >> pant1x >> pant2x >> pant1h // wysokość pierwszego ślizgu
					...
					if (pants) {
						for (int i = 0; i < iAnimType[ANIM_PANTS]; ++i) {
							...
							{ // gdy pomiary modelu nie udały się, odczyt podanych parametrów z MMD
								...
								pantograph.fHeight = (i & 1) ? pant2h : pant1h; // wysokość ślizgu jest zapisana w MMD
							}
							pantograph.PantWys = pantograph.fLenL1 * sin(pantograph.fAngleL) + pantograph.fLenU1 * sin(pantograph.fAngleU) + pantograph.fHeight; // wysokość początkowa
                    ...
                    */
					...
					{ // NBMX wrzesien 2003: wyszukiwanie drzwi o nazwie str*
						...
						if (pAnimations[i + j].smAnimated) { //++iAnimatedDoors;
							pAnimations[i + j].smAnimated->WillBeAnimated(); // wyłączenie optymalizacji transformu
							switch (MoverParameters->DoorOpenMethod) { // od razu zapinamy potrzebny typ animacji
								case 1:
...
*/
					...
					{ // NBMX wrzesien 2003: wyszukiwanie drzwi o nazwie str*
						...
						if (pAnimations[i + j].smAnimated) { //++iAnimatedDoors;
							pAnimations[i + j].smAnimated->WillBeAnimated(); // wyłączenie optymalizacji transformu
							switch (MoverParameters->PlatformOpenMethod) { // od razu zapinamy potrzebny typ animacji
								case 1: // shift
...
*/
					...
					{ // NBMX wrzesien 2003: wyszukiwanie drzwi o nazwie str*
						...
						if (pAnimations[i + j].smAnimated) {
							...
							pAnimations[i + j].iNumber = ((pAnimations[i + j].smAnimated->offset().z > 0 ? side::front : side::rear) << 4) + i;
							pAnimations[i + j].fMaxDist = 150 * 150; // drzwi to z daleka widać
							/*
                            // NOTE: no longer used
                            pAnimations[i + j].fSpeed = Random(150); // oryginalny koncept z DoorSpeedFactor
...
*/
			...
			do {
				...
				if (token == "wheel_clatter:") {
					...
					parser.getTokens(1, false);
					parser >> dSDist;
					while (((token = parser.getToken<std::string>()) != "") && (token != "end")) {
						// add another axle entry to the list
						axle_sounds axle{ 0, std::atof(token.c_str()), { sound_placement::external, static_cast<float>(dSDist) } };
					...
					std::sort(std::begin(m_axlesounds), std::end(m_axlesounds), [](axle_sounds const& Left, axle_sounds const& Right) { return (Left.offset < Right.offset); });
				}
					...
					auto const amplitudedivisor =
					static_cast<float>((MoverParameters->EngineType == DieselEngine ? 1 : MoverParameters->EngineType == DieselElectric ? 1 : MoverParameters->nmax * 60 + MoverParameters->Power * 3));
	...
	std::vector<sound_source*> enginesounds = { &sConverter, &sCompressor, &sSmallCompressor };
	...
	if (pants != nullptr) {
		...
		for (auto& pantographsounds : m_pantographsounds) {
			auto const pantographoffset{ glm::vec3(0.f, pants[pantographindex].fParamPants->vPos.y, -pants[pantographindex].fParamPants->vPos.x) };
...
{ // zatrzymanie pojazdu
	if (Mechanik) {
		...
		if ((MoverParameters->SecuritySystem.RadioStop) && (MoverParameters->Radio)) {
...
{ // zapalenie świateł z przodu i z
	...
	if (rear == (light::redmarker_left | light::redmarker_right | light::rearendsignals)) { // jeśli koniec pociągu, to trzeba ustalić, czy
		...
		if (rear == (light::redmarker_left | light::redmarker_right | light::rearendsignals)) // jeśli nadal obydwie możliwości
			if (iInventory[(iDirection ? side::rear : side::front)] & (light::redmarker_left | light::redmarker_right)) {
	...
	{ // jesli pojazd stoi sprzęgiem 0 w stronę czoła
		...
		if (rear >= 0)
			iLights[1] = rear;
	} else { // jak jest odwrócony w składzie (-1), to zapalamy odwrotnie
		if (head >= 0)
			iLights[1] = head;
		if (rear >= 0)
			iLights[0] = rear;
...
{ // ustawienie kierunku w składzie (wykonuje AI)
	...
	if (MyTrack) { // podczas wczytywania wstawiane jest AI, ale może jeszcze nie
		// być toru
		...
		} else if (MoverParameters->Couplers[1].CouplingFlag == ctrain_virtual) // brak pojazdu podpiętego?
...
{ // obliczenie odległości sprzęgów
	...
	} else { // na drodze trzeba uwzględnić wektory ruchu
...
{ // taka proteza:
	...
	if (d->MoverParameters->TrainType & dt_EZT) {
		...
		if ((d->NextConnected != nullptr) && (true == TestFlag(d->MoverParameters->Couplers[1].AllowedFlag, coupling::permanent))) {
			// gdy jest człon od sprzęgu 1, a sprzęg łączony warsztatowo (powiedzmy)
			if ((d->MoverParameters->Power < 1.0) && (d->NextConnected->MoverParameters->Power > 1.0)) {
				// my nie mamy mocy, ale ten drugi ma
				d = d->NextConnected; // będziemy sterować tym z mocą
			}
		} else if ((d->PrevConnected != nullptr) && (true == TestFlag(d->MoverParameters->Couplers[0].AllowedFlag, coupling::permanent))) {
			// gdy jest człon od sprzęgu 0, a sprzęg łączony warsztatowo (powiedzmy)
			if ((d->MoverParameters->Power < 1.0) && (d->PrevConnected->MoverParameters->Power > 1.0)) {
				// my nie mamy mocy, ale ten drugi ma
	...
	switch (what & 0xFF00) {
		case 0x0100: // to np. są drzwi, bity 0..7 określają numer 1..254 albo maskę
			...
			if (what & 1) // na razie mamy lewe oraz prawe, czyli używamy maskę 1=lewe,
				// 2=prawe, 3=wszystkie
				if (MoverParameters->DoorLeftOpened) { // są otwarte
			...
			if (what & 2) // prawe działają niezależnie od lewych
				if (MoverParameters->DoorRightOpened) { // są otwarte
					if (!into) // jeśli zamykanie
					{
						// dźwięk zamykania
					}
...
{ // ustawienie stacji docelowej oraz wymiennej tekstury 4, jeśli istnieje plik
	...
	std::vector<std::string> destinations = { numer + '@' + MoverParameters->TypeName, numer, to + '@' + MoverParameters->TypeName, to, "nowhere" + '@' + MoverParameters->TypeName, "nowhere" };
...
TPowerSource TDynamicObject::ConnectedEnginePowerSource(TDynamicObject const* Caller) const
{
	...
	if ((nullptr != NextConnected) && (NextConnected != Caller) && ((MoverParameters->Couplers[1].CouplingFlag & ctrain_controll) == ctrain_controll)) {
	...
	// ...then rear...
	if ((nullptr != PrevConnected) && (PrevConnected != Caller) && ((MoverParameters->Couplers[0].CouplingFlag & ctrain_controll) == ctrain_controll)) {
...
{
	auto const nullvector{ glm::vec3() };
	std::vector<sound_source*> enginesounds = { &inverter, &motor_relay, &dsbWejscie_na_bezoporow, &motor_parallel, &motor_shuntfield, &rsWentylator, &engine, &engine_ignition, &engine_shutdown,
		&engine_revving, &engine_turbo, &oil_pump, &radiator_fan, &radiator_fan_aux, &transmission, &rsEngageSlippery };
	for (auto sound : enginesounds) {
		if (sound->offset() == nullvector) {
...
void TDynamicObject::powertrain_sounds::render(TMoverParameters const& Vehicle, double const Deltatime)
{
	if (Vehicle.Power == 0) {
		return;
	}
	...
	if (true == Vehicle.OilPump.is_active) {
		oil_pump.pitch(oil_pump.m_frequencyoffset + oil_pump.m_frequencyfactor * 1.f)
	...
	if (engine_state_last != Vehicle.Mains) {
		if (Vehicle.Mains) {
			...
			engine_ignition.pitch(engine_ignition.m_frequencyoffset + engine_ignition.m_frequencyfactor * 1.f)
	...
	if (true == Vehicle.Mains) {
		if ((std::abs(Vehicle.enrot) > 0.01)
			...
			// frequency calculation
			auto const normalizer{ (true == engine.is_combined() ?
				                    ...
				                    1.f) };
			frequency = engine.m_frequencyoffset + engine.m_frequencyfactor * std::abs(Vehicle.enrot) * normalizer;
			...
			switch (Vehicle.EngineType) {
				...
				case DieselElectric: {
					volume =
					engine.m_amplitudeoffset + engine.m_amplitudefactor * (0.25 * (Vehicle.EnginePower / Vehicle.Power) + 0.75 * (Vehicle.enrot * 60) / (Vehicle.DElist[Vehicle.MainCtrlPosNo].RPM));
					break;
				}
				case DieselEngine: {
				...
				default: {
					volume = engine.m_amplitudeoffset + engine.m_amplitudefactor * (Vehicle.EnginePower / 1000 + std::fabs(Vehicle.enrot) * 60.0);
					break;
				}
			}
			...
			if (engine_volume >= 0.05) {
				...
				if ((Vehicle.EngineType == DieselElectric) || (Vehicle.EngineType == DieselEngine)) {
					...
					if (engine_revs_last != -1.f) {
						...
						auto const idlerevolutionsthreshold{ 1.01 * (Vehicle.EngineType == DieselElectric ? Vehicle.DElist[0].RPM : Vehicle.dizel_nmin * 60) };

						engine_revs_change = std::max(0.0, engine_revs_change - 2.5 * Deltatime);
						if ((revolutionsperminute > idlerevolutionsthreshold) && (revolutionsdifference > 1.0 * Deltatime)) {
					...
					auto const enginerevfrequency{ (true == engine_revving.is_combined() ?
					...
					if (enginerevvolume > 0.02) {
						engine_revving.pitch(enginerevfrequency).gain(enginerevvolume).play(sound_flags::exclusive);
					} else {
						engine_revving.stop();
					}
				} // diesel engines
				...
				auto const enginevolume{ (((enginerevvolume > 0.02) && (true == engine_revving.is_combined())) ? std::max(0.0, engine_volume - enginerevvolume) : engine_volume) };
				engine.pitch(frequency).gain(enginevolume).play(sound_flags::exclusive | sound_flags::looping);
			} // enginevolume > 0.05
		} else {
			engine.stop();
		}
	...
	if (Vehicle.TurboTest > 0) {
		...
		((Vehicle.MainCtrlPos >= Vehicle.TurboTest) && (Vehicle.enrot > 0.1)) ? std::max(0.0, (engine_turbo_pitch + engine_turbo.m_amplitudeoffset) * engine_turbo.m_amplitudefactor) : 0.0) };
	...
	if (Vehicle.dizel_engage > 0.1) {
		...
		rsEngageSlippery.pitch(frequency).gain(volume).play(sound_flags::exclusive | sound_flags::looping);
	} else {
	...
	// motor sounds
	volume = 0.0;
	if ((true == Vehicle.Mains) && (false == motors.empty())) {
		if (std::abs(Vehicle.enrot) > 0.01) {
			...
			frequency = motor.m_frequencyoffset + motor.m_frequencyfactor * motorrevolutions * normalizer;

			// base volume calculation
			switch (Vehicle.EngineType) {
				case ElectricInductionMotor: {
					volume = motor.m_amplitudeoffset + motor.m_amplitudefactor * (Vehicle.EnginePower + motorrevolutions * 2);
					break;
				}
				case ElectricSeriesMotor: {
					volume = motor.m_amplitudeoffset + motor.m_amplitudefactor * (Vehicle.EnginePower / 1000 + motorrevolutions * 60.0);
					break;
				}
				default: {
					volume = motor.m_amplitudeoffset + motor.m_amplitudefactor * motorrevolutions * 60.0;
					break;
				}
			}
			...
			motor_momentum = clamp(motor_momentum - 1.0 * Deltatime // smooth out decay
			...
			if (motor_volume >= 0.05) {
				...
				for (auto& motor : motors) {
					motor.pitch(frequency).gain(motor_volume).play(sound_flags::exclusive | sound_flags::looping);
				}
			}
		} else {
	...
	if (Vehicle.EngineType == ElectricInductionMotor) {
		if (Vehicle.InverterFrequency > 0.1) {
			...
			inverter.pitch(inverter.m_frequencyoffset + inverter.m_frequencyfactor * Vehicle.InverterFrequency).gain(volume).play(sound_flags::exclusive | sound_flags::looping);
		} else {
			inverter.stop();
		}
	}
	...
	if (Vehicle.RventRot > 0.1) {
		rsWentylator.pitch(rsWentylator.m_frequencyoffset + rsWentylator.m_frequencyfactor * Vehicle.RventRot)
	...
	// radiator fan sounds
	if ((Vehicle.EngineType == DieselEngine) || (Vehicle.EngineType == DieselElectric)) {
		if (Vehicle.dizel_heat.rpmw > 0.1) {
			...
			radiator_fan.pitch(0.5 + radiator_fan.m_frequencyoffset + radiator_fan.m_frequencyfactor * Vehicle.dizel_heat.rpmw * 0.01)
		...
		if (Vehicle.dizel_heat.rpmw2 > 0.1) {
			// NOTE: fan speed tends to max out at ~100 rpm; by default we try to get pitch range of 0.5-1.5 and volume range of 0.5-1.0
			radiator_fan_aux.pitch(0.5 + radiator_fan_aux.m_frequencyoffset + radiator_fan_aux.m_frequencyfactor * Vehicle.dizel_heat.rpmw2 * 0.01)
	...
	if (TestFlag(soundflags, sound::relay)) {
		...
		if (true == TestFlag(soundflags, sound::shuntfield)) {
			// shunt field
			motor_shuntfield.pitch(true == motor_shuntfield.is_combined() ? Vehicle.ScndCtrlActualPos * 0.01f : motor_shuntfield.m_frequencyoffset + motor_shuntfield.m_frequencyfactor * 1.f)
			.gain(motor_shuntfield.m_amplitudeoffset + (true == TestFlag(soundflags, sound::loud) ? 1.0f : 0.8f) * motor_shuntfield.m_amplitudefactor)
			.play();
			...
			// series mode
			motor_relay.pitch(true == motor_relay.is_combined() ? Vehicle.MainCtrlActualPos * 0.01f : motor_relay.m_frequencyoffset + motor_relay.m_frequencyfactor * 1.f)
			.gain(motor_relay.m_amplitudeoffset + (true == TestFlag(soundflags, sound::loud) ? 1.0f : 0.8f) * motor_relay.m_amplitudefactor)
			.play();
		}
	}

	if (Vehicle.Vel > 0.1) {
		transmission.pitch(transmission.m_frequencyoffset + transmission.m_frequencyfactor * Vehicle.Vel)
...
void vehicle_table::update_traction(TDynamicObject* Vehicle)
{
	...
	for (int pantographindex = 0; pantographindex < Vehicle->iAnimType[ANIM_PANTS]; ++pantographindex) {
		...
		if (true == (pantographindex == side::front ? Vehicle->MoverParameters->PantFrontUp : Vehicle->MoverParameters->PantRearUp)) {
			...
			if (pantograph->hvPowerWire != nullptr) {
				...
				for (int attempts = 0; attempts < 30; ++attempts) {
					...
					auto const fRaParam = -(glm::dot(pantograph->hvPowerWire->pPoint1, vFront) - glm::dot(pant0, vFront)) / glm::dot(pantograph->hvPowerWire->vParametric, vFront);
			...
			if ((pantograph->hvPowerWire == nullptr) && (false == Global.bLiveTraction)) {
