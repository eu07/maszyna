...
void __fastcall TSubModel::SETIDS(AnsiString Name)
{
  ...
  {//renderowanie obiektów OpenGL
   ...
   {if (TextureID<0) // && (ReplacableSkinId!=0))
    {//zmienialne skóry
     ...
     //setclockdigital();                                 // ZEGAR PERONOWY CYFROWY
    }
    else
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// ZMIANA SKORY OBIEKTOW Z REPLACABLESKIN^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

float trans = 1.0f;
bool up = false;
void __fastcall TSubModel::Render2(GLuint ReplacableSkinId, bool blink)
{

if (!up) trans-=0.05;
if (trans < 0.1) up = true;
if (up) trans+=0.05;
if (trans > 1.0) up = false;

  if (Next!=NULL)
      {
       //if ((Next->eType == TP_FREESPOTLIGHT))
       Next->Render2(Global::semlense1, blink);     // WYWOLYWANIE SAMEJ SIEBIE ...
      }

  if ((Visible) && (fSquareDist>=fSquareMinDist) && (fSquareDist<142000))  // SPRAWDZENIE CZY WIDOCZNY I CZY MIESCI SIE POMIEDZY MINDIST A MAXDIST
   {
    glPushMatrix();
...
// SWIATLA SEMAFOROW ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    if (Name == "fspot00" || Name == "fspot01" || Name == "fspot02" || Name == "fspot03" || Name == "fspot04" || Name == "fspot05" || Name == "fspot06" ||
        Name == "light_on00" || Name == "light_on01" || Name == "light_on02" || Name == "light_on03" || Name == "light_on04" || Name == "light_on05" || Name == "light_on06" || Name == "light_on07")
          {
           //glPushMatrix();
           glColor4f(f4Diffuse[0],f4Diffuse[1],f4Diffuse[2],f4Diffuse[3]);
           ...
           glBindTexture(GL_TEXTURE_2D, Global::semlense1);
           glTranslatef(0.0,-0.05,-0.05);
           glBegin(GL_QUADS);
           glNormal3f( 0.0f, 0.0f,-0.5f);
           glTexCoord2f(1.0f, 0.0f); glVertex3f(-0.30f, -0.25f, -0.01f);
           glTexCoord2f(1.0f, 1.0f); glVertex3f(-0.30f,  0.25f, -0.01f);
           glTexCoord2f(0.0f, 1.0f); glVertex3f( 0.30f,  0.25f, -0.01f);
           glTexCoord2f(0.0f, 0.0f); glVertex3f( 0.30f, -0.25f, -0.01f);
           glEnd();

           if (blink)
           {
           glBindTexture(GL_TEXTURE_2D, Global::semlense1);
           glTranslatef(0.0,-0.05,-0.10);
           glBegin(GL_QUADS);
           glNormal3f( 0.0f, 0.0f,-0.5f);
           glTexCoord2f(1.0f, 0.0f); glVertex3f(-0.20f, -0.20f, -0.01f);
           glTexCoord2f(1.0f, 1.0f); glVertex3f(-0.20f,  0.20f, -0.01f);
           glTexCoord2f(0.0f, 1.0f); glVertex3f( 0.20f,  0.20f, -0.01f);
           glTexCoord2f(0.0f, 0.0f); glVertex3f( 0.20f, -0.20f, -0.01f);
           glEnd();
           }
   ...
   //   }
      ...
      if (Child!=NULL) Child->Render2(Global::semlense1, blink);
  ...
  {//renderowanie obiektów OpenGL
   ...
   {if (TextureID<0) // && (ReplacableSkinId!=0))
    {//zmienialne skóry
     glBindTexture(GL_TEXTURE_2D,ReplacableSkinId[-TextureID]);
     setclockdigital();                                                         // ZEGAR PERONOWY CYFROWY
     setdirecttable1();                                                         // TABLICA KIERUNKOWA 2x13
     //TexAlpha=iAlpha&1; //zmiana tylko w przypadku wymienej tekstury
    }
    else
     glBindTexture(GL_TEXTURE_2D,TextureID); //równie¿ 0
...
int __fastcall _GETFILESUBMODELS(AnsiString MODELFILE)
{
 AnsiString TEST, line;
 TStringList *T3D;

 T3D = new TStringList;
 T3D->LoadFromFile(MODELFILE);
 T3D_LINESCOUNT = T3D->Count;
 T3D_SUBS = 0;

 for (int loop = 0; loop < T3D_LINESCOUNT; loop++)
    {
     line = T3D->Strings[loop];
     TEST = line.SubString(1, 4);
...
AnsiString __fastcall readstr(int line, int start, int end)
{
 AnsiString RET;
 AnsiString TEST =  Q3D_FILE->Strings[line];

 RET = TEST.SubString(start,255);

 return RET;
}
inline void setMatrixQ(float4x4& matrix, int x, int y, float val)
{
   matrix(x)[y] = val;
};
...
// TOKENIZER
char explodestring[128] = "100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,100.000,";
char seps[] = ",";
char *token;
float V1, V2, V3, V4, V5, V6, V7, V8, V9, V0, VA, VB, VC, VD, VE, VF;

__fastcall TSubModel::EXPLODESTR(AnsiString line)
{

                    int par;
                    par = 0;
                    strcpy(explodestring, line.c_str());                        // OSMIO PARAMETROWY STRING
                    token = strtok( explodestring, seps );                      // FIRST TOKEN

                    while( (token != NULL) )
                        {
                         // Get nexts token:
                         if (par ==  0) V0 = atof(token);
                         if (par ==  1) V1 = atof(token);
                         if (par ==  2) V2 = atof(token);
                         if (par ==  3) V3 = atof(token);
                         if (par ==  4) V4 = atof(token);
                         if (par ==  5) V5 = atof(token);
                         if (par ==  6) V6 = atof(token);
                         if (par ==  7) V7 = atof(token);
                         if (par ==  8) V8 = atof(token);
                         if (par ==  9) V9 = atof(token);
                         if (par == 10) VA = atof(token);
                         if (par == 11) VB = atof(token);
                         if (par == 12) VC = atof(token);
                         if (par == 13) VD = atof(token);
                         if (par == 14) VE = atof(token);
                         if (par == 15) VF = atof(token);
                         //P[pars] = atof(token);
                         //WriteLog(AnsiString(P[pars]));
...
inline void readMatrixQ3D(float4x4& matrix, int x, int y, float val)
{//Ra: wczytanie transforma
// for (int x=0;x<=3;x++) //wiersze
//  for (int y=0;y<=3;y++) //kolumny
//  {
   matrix(x)[y] = val;;
//   readMatrixQ3D(fMatrix, 0, 1, V1)
//  }
};
...
{//wczytanie submodelu z pliku tekstowego
 WriteLog("MODEL3D: "+AnsiString(FileName));
...
void __fastcall TModel3d::Render2( bool blink, double fSquareDistance,GLuint *ReplacableSkinId,int iAlpha)     // dla semaforow
{
if (mid == 101)
    {
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// RENDEROWANIE LENSOW SEMAFOROW ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

void __fastcall TModel3d::Render2( bool blink, vector3 *vPosition, vector3 *vAngle, GLuint *ReplacableSkinId, int iAlpha)
{
if (mid == 101)
    {
     matrix4x4 Identity;
    ...
    Root->Render2(Global::semlense1, blink);                                         // CALL TO TSubModel::Render2(GLuint ReplacableSkinId, AnsiString nodename);
