...
// McZapkie-060702: zmienialne skory modelu
    ...
    if(eType==smt_Mesh)
    {
        ...
        glVertexPointer(3, GL_FLOAT, sizeof(GLVERTEX), &Vertices[0].Point.x);
        glNormalPointer(GL_FLOAT, sizeof(GLVERTEX), &Vertices[0].Normal.x);
...
#ifdef USE_VERTEX_ARRAYS
        glDrawArrays(GL_TRIANGLES, 0, iNumFaces);
#else
