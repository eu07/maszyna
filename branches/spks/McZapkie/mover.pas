...
TYPE
    ...
    TBrakeSystem = (Individual, Pneumatic, ElectroPneumatic, Electric, Electronic);
    {podtypy hamulcow zespolonych}
    TBrakeSubSystem = (ss_None, ss_W, ss_K, ss_KK, ss_Hik, ss_ESt, ss_KE, ss_LSt, ss_MT);
    TBrakeValve = (W, W_Lu_VI, W_Lu_L, W_Lu_XR, K, Kg, Kp, Kss, Kkg, Kkp, Kks, Hikg1, Hikss, Hikp1, KE1a, SW, ESt3, LSt, ESt4, EP1, EP2, M483, Other);
    TBrakeHandle = (NoHandle, West, FV4a, M394, M254, FVel1, FVel2, D2, Knorr, FD1);
    {typy hamulcow indywidualnych}
    TLocalBrake = (NoBrake, ManualBrake, PneumaticBrake, HydraulicBrake);

    TBrakeDelayTable= array[1..5] of real;
    ...
    TMoverParameters = class(TObject)
               ...
               Pipe,Pipe2,BrakeVes,BrakeCyl:TVessel;
               MainVes,CntrlVes,ValveVes,BrakeAccVes:TVessel;
               ImpulseVes:TVessel;
               IVr:real; { przelozenie komora impulsowa - silownik hamulcowy }
...
{                dizel_auto_min, dizel_auto_max: real; {predkosc obrotowa przelaczania automatycznej skrzyni biegow}
                ...
                EnLiqs: array[0..el_fuel, 0..el_off, 0..el_P] of real;
                ...
                OverCon: real;                      {ogolny stan pojazdu}
                ...
                Sp:real;                            {pole przekroju przewodu glownego}
                ...
                BrakeValves: byte;                  {flagi dadatkowych mozliwosc zaworu}
...
{                function BrakeRatio: real;  }
               ...
               {funkcje dla ukladow pneumatycznych}
                ...
                procedure ValveCheck(dt:real);
                function GetDVc(dt:real):real;
                function GetDVv(dt:real):real;
                function GetDVh(dt:real):real;
                function GetDVb(dt:real):real;
...
{------------------------------------------}
{funkcje pomocnicze}
function GetP(Ves:TVessel):real;
begin
  GetP:=0.1*Ves.Vol/Ves.Cap;
end;

function O2PC(x:real):real;
begin
  O2PC:=1/(1+x);
end;
...
function TMoverParameters.ManualBrakeRatio: real;
var MBR:real;
begin
...
begin
  if (DynamicBrakeType=dbrake_switch) and (MainCtrlPos=0) then
   ...
   {end;
   if (DynamicBrakeType=dbrake_passive) and (TrainType='et42') then
   begin
...
begin
  ...
  if (MainCtrlPosNo>0) and (CabNo<>0) then
   if (TrainType='sp32') and (ScndS) then
    begin
     if (MainCtrlPos>0) then dec(MainCtrlPos);OK:=True;
    end
   else
    begin
      if ((MainCtrlPos<MainCtrlPosNo) {and (ScndCtrlPos=0)} and (EngineType<>DieselElectric)) or((EngineType=DieselElectric)and(MainCtrlPos<MainCtrlPosNo)) then
      begin
 ...
 {            if (EngineType=ElectricSeriesMotor) and (MainCtrlPos=1) then
           ...
           if (CtrlSpeed=1) and (ActiveDir=-1) and (RList[MainCtrlPos].Bn>1) and (TrainType<>'pseudodiesel') then
            begin
                  dec(MainCtrlPos);
                  OK:=false;
          ...
          if (CtrlSpeed>1) and (ActiveDir<>0) {and (ScndCtrlPos=0)} and (TrainType<>'et40') then
           begin
   ...
   OK:=False;
 if ((OK) and (TrainType<>'et40') and (TrainType<>'ep05')) or ((OK) and ((TrainType='et40') or (TrainType='ep05'))and(MainCtrlPos=1)) then LastRelayTime:=0;
  IncMainCtrl:=OK;
end;
...
begin
  ...
  if (MainCtrlPosNo>0) and (CabNo<>0) then
   if (TrainType='sp32') and (ScndS) then
    begin
     if (1.5*MainCtrlPos<MainCtrlPosNo) then begin inc(MainCtrlPos);OK:=True;end;
    end
   else
   begin
      if (MainCtrlPos>0) then
   ...
   OK:=False;
  if ((OK) and (TrainType<>'et40') and (TrainType<>'ep05'))  then LastRelayTime:=0;
  DecMainCtrl:=OK;
end;
...
var //b:byte;
    OK:boolean;
begin
  if(EngineType=DieselElectric)and(AutoRelayFlag)then
    OK:=False
  else
  if (MainCtrlPos=0)and(CabNo<>0)and(TrainType='et42')and(ScndCtrlPos=0)and(DynamicBrakeFlag)then
   begin
    OK:=DynamicBrakeSwitch(False);
   end
  else
 if (ScndCtrlPosNo>0) and (CabNo<>0) and not ((TrainType='et42')and((Imax=ImaxHi)or((DynamicBrakeFlag)and(MainCtrlPos>0)))) then
  begin
     //if (RList[MainCtrlPos].R=0) and (MainCtrlPos>0) and (ScndCtrlPos<ScndCtrlPosNo) and (not CoupledCtrl) then
  if (ScndCtrlPos<ScndCtrlPosNo) and (RList[MainCtrlPos].R=0) and(activedir<>0) and (not CoupledCtrl) and (TrainType<>'et40') and (TrainType<>'ep05') then
    begin
      if CtrlSpeed=1 then
       begin
         inc(ScndCtrlPos);
...
begin
  ...
  if (MainCtrlPos=0)and(CabNo<>0)and(TrainType='et42')and(ScndCtrlPos=0)and not(DynamicBrakeFlag)then
   begin
...
begin
  ...
  else if (ActiveDir=1) and (MainCtrlPos=0) and (TrainType='ezt') then
...
begin
  if (ActiveDir=1) and (MainCtrlPos=0) and (TrainType='ezt') then
...
begin
  ...
  if Abs(ActiveCab+direction)<2 then
   begin
    // if (CabNo+direction=0) then LastCab:=CabNo;
     CabDeactivisation;
     //CabActivisation;
...
function TMoverParameters.BatterySwitch(State:boolean):boolean;
//var b:byte;
begin
...
//       wystarczy spojrzec na Knorra i Oerlikona EP w EN57; mogly ze soba wspolpracowac
...
//            LimPipePress:=0.85;
         ...
         else if(BrakeCtrlPos=0)then
            ActFlowSpeed:=FlowSpeedVal;
//            LimPipePress:=Min0R(0.54,Max0R(0.84-0.68*Max0R(PipePress,0.35),0.5))
...
begin
  if (BrakeCtrlPosNo>0) {and (LocalBrakePos=0)} then
   begin
     if (BrakeCtrlPos>-1) or ((BrakeHandle=FV4a) and (BrakeCtrlPos>-2))then
      begin
        dec(BrakeCtrlPos);
...
//            LimPipePress:=0.85;
            ...
            ActFlowSpeed:=FlowSpeedVal;
//            LimPipePress:=Min0R(0.54,Max0R(0.84-0.68*Max0R(PipePress,0.35),0.5))
//         else if((LimPipePress<PipePress-0.005) and (BrakePressureTable[BrakeCtrlPos].PipePressureVal>PipePress+0.005))or((LimPipePress>PipePress+0.005) and (BrakePressureTable[BrakeCtrlPos].PipePressureVal<PipePress-0.005))then
//            LimPipePress:=PipePress;
...
function TMoverParameters.IncManualBrakeLevel(CtrlSpeed:byte):boolean;
begin
...
UnBrake:=True;
 ...
 function TMoverParameters.DecManualBrakeLevel(CtrlSpeed:byte):boolean;
begin
  ...
  if (BrakeSystem<>Individual) and (BrakeCtrlPosNo>0) then
   begin
     if (not EmergencyBrakeFlag) and Switch then
      ...
      begin
        if (Vel<0.1) and (Switch=False) then  {odblokowanie hamulca bezpieczenistwa tylko po zatrzymaniu}
         begin
           EmergencyBrakeFlag:=Switch;
           EmergencyBrakeSwitch:=True;
         end
...
begin
  if (BrakeCtrlPosNo>-2) and (PipePress<CntrlPipePress) {and {(BrakeStatus=b_on)(not TestFlag(BrakeStatus,b_dmg))} then
   begin
...
begin
  ...
  if (BrakeDelayFlag=bdelay_G)and(GetP(BrakeCyl)>0.06) then
    jakisx:=2
  else
    jakisx:=0;
  case BrakeSubsystem of
    ss_LSt: Speed:=LBDelay;
  else
   begin
    if (BrakeStatus and 3)=3 then
      Speed:=BrakeDelay[2+jakisx];
    if (BrakeStatus and 1)=0 then
      Speed:=BrakeDelay[1+jakisx];
   end;
  end;
...
begin
  if(BrakeValve=EP2)and(BrakePressureTable[BrakeCtrlPos].BrakePressureVal>-1)then
    if(BrakePressureTable[BrakeCtrlPos].BrakePressureVal>LocBrakePress+0.01)and(LocBrakePress<MaxBrakePress*1.1)then
      LocBrakePress:=Max0R(GetP(ImpulseVes),LocBrakePress)+PR(LocBrakePress,0.6)*dt/(LBDelay+2)
    else if(BrakePressureTable[BrakeCtrlPos].BrakePressureVal<LocBrakePress)then
      LocBrakePress:=Max0R(0,LocBrakePress)+PR(LocBrakePress,0)*dt/(LBdelay+2);

  if(BrakeValve=LSt)then
    dpLocalValve:=PR(LocBrakePress,MaxBrakePress*LocalBrakeRatio)*dt/(LBdelay);
  LocBrakePress:=LocBrakePress+dpLocalValve;

if(IVr>0)then
 begin
  if (MaxBrakePress>0) {and (not TestFlag(BrakeStatus,b_dmg))} then
   begin
     if ((BrakeStatus and 3)=3) and (GetP(ImpulseVes)<MaxBrakePress) then
       dpBrake:=1.1*MaxBrakePress*PR(GetP(ImpulseVes),GetP(BrakeVes))*dt/Speed;
     if (BrakeStatus and 1)=0 then
       dpBrake:=1.1*MaxBrakePress*PR(GetP(ImpulseVes),0)*dt/Speed;
     PipeBrakePress:=PipeBrakePress+dpBrake;
     if PipeBrakePress<0 then PipeBrakePress:=0;
   end; {cylindry hamulcowe}
    dpBrake:=Max0R(PipeBrakePress,LocBrakePress)-GetP(ImpulseVes);
  if(dpBrake>0)then
    BrakeVes.Vol:=BrakeVes.Vol-10*dpBrake*ImpulseVes.Cap;
  ImpulseVes.Vol:=ImpulseVes.Vol+10*dpBrake*ImpulseVes.Cap;

  if GetP(ImpulseVes)*IVr-GetP(BrakeCyl)>0.002 then
    dpBrake:=1.1*MaxBrakePress*PR(GetP(BrakeCyl),GetP(BrakeVes))*dt/LBDelay
  else if GetP(ImpulseVes)*IVr-GetP(BrakeCyl)<-0.002 then
    dpBrake:=1.1*MaxBrakePress*PR(GetP(BrakeCyl),0)*dt/LBDelay
  else dpBrake:=0;

  if(dpBrake>0)then
    BrakeVes.Vol:=BrakeVes.Vol-10*dpBrake*BrakeCyl.Cap;
  BrakeCyl.Vol:=BrakeCyl.Vol+10*dpBrake*BrakeCyl.Cap;
 end
else
//  BrakePress:=Max0R(PipeBrakePress,LocBrakePress);
 begin
  if (MaxBrakePress>0) {and (not TestFlag(BrakeStatus,b_dmg))} then
   begin
     if (BrakeStatus and 3)=3 then
       dpBrake:=1.1*MaxBrakePress*PR(PipeBrakePress,GetP(BrakeVes))*dt/Speed;
     if (BrakeStatus and 1)=0 then
       dpBrake:=1.1*MaxBrakePress*PR(PipeBrakePress,0)*dt/Speed;
     PipeBrakePress:=PipeBrakePress+dpBrake;
   end; {cylindry hamulcowe}
   dpBrake:=Max0R(PipeBrakePress,LocBrakePress)-GetP(BrakeCyl);
  if(dpBrake>0)then
    BrakeVes.Vol:=BrakeVes.Vol-10*dpBrake*BrakeCyl.Cap;
  BrakeCyl.Vol:=BrakeCyl.Vol+10*dpBrake*BrakeCyl.Cap;
 end;

if (DynamicBrakeType<>dbrake_switch) and (BrakePress<0.1) and (DynamicBrakeFlag=True) then
 begin
  DynamicBrakeFlag:=False;               {wylaczanie hamulca ED i/albo zalaczanie silnikow}
//  MainCtrlActualPos:=0;
//  LastRelayTime:=0;
...
if (DynamicBrakeType<>dbrake_switch) and (DynamicBrakeType<>dbrake_none) and (BrakePress>0.2) then
  DynamicBrakeFlag:=True;                 {uruchamianie hamulca ED albo odlaczanie silnikow}
end;
//  if (dpBrake*dpBrake>0.00000000001) then Physic_Reactivation;
  ...
  if Couplers[0].Connected<>nil then
   if TestFlag(Couplers[0].CouplingFlag,ctrain_pneumatic) then
     begin
       ...
       dv1:=170*dt*PR(PipePress,c^.PipePress)*Spg;
...
//sprzeg 2
  if Couplers[1].Connected<>nil then
   if TestFlag(Couplers[1].CouplingFlag,ctrain_pneumatic) then
     begin
       ...
       dv2:=170*dt*PR(PipePress,c^.PipePress)*Spg;
  ...
  if(Couplers[1].Connected<>nil)and(Couplers[0].Connected<>nil)then
    if (TestFlag(Couplers[0].CouplingFlag,ctrain_pneumatic))and(TestFlag(Couplers[1].CouplingFlag,ctrain_pneumatic))then
     begin
      dv:=200*dt*PR(Couplers[0].Connected^.PipePress,Couplers[1].Connected^.PipePress)*Spg;
      Couplers[0].Connected.Pipe.Vol:=Couplers[0].Connected.Pipe.Vol+dv;
      Couplers[1].Connected.Pipe.Vol:=Couplers[1].Connected.Pipe.Vol-dv;
     end;
//suma
  GetDVc:=dv2+dv1;
end;

function TMoverParameters.GetDVh(dt:real):real;
const LBDelay=100;
var bcp: shortint;
begin
  dpMainValve:=0;
  dpPipe:=Min0R(BrakePressureTable[BrakeCtrlPos].PipePressureVal,compressor);
  if (BrakeCtrlPosNo>0)and(ActiveCab<>0)then
  case BrakeHandle of
  FV4a: with BrakePressureTable[BrakeCtrlPos] do
   begin

  if(dpPipe<PipePress)or(PipePress>0.33)or(((BrakeStatus and 4)=4)and(BrakeCtrlPos<1))then
   if(BrakeCtrlPos>-2) then
    if(BrakeCtrlPos>-1)or(LimPipePress<0.50)then
     if (BrakeCtrlPos>0)or(LimPipePress<0.49)or(LimPipePress>0.5)then
      LimPipePress:=LimPipePress+PR(LimPipePress,dpPipe)*dt/32
     else if (PipePress>0.54) then
      LimPipePress:=LimPipePress+PR(LimPipePress,dpPipe)*dt/32
     else
    else if (LimPipePress<0.54)and(PipePress>0.65) then
      LimPipePress:=LimPipePress+PR(LimPipePress,dpPipe)*dt/32;

//    if sign(LimPipePress-PipePress)<>sign(ActFlowSpeed) then
//      ActFlowSpeed:=0;
//    if abs(LimPipePress-PipePress)>0.001then
//      ActFlowSpeed:=sign(LimPipePress-PipePress)*9/2;
//    if abs(LimPipePress-PipePress)>0.002 then
//      ActFlowSpeed:=sign(LimPipePress-PipePress)*9;

        ActFlowSpeed:=34*50*sign(LimPipePress-PipePress)*Min0R(0.02,abs(LimPipePress-PipePress));
      if BrakeCtrlPos=-1 then
        if (LimPipePress>0.54) then
          if (PipePress>0.54) then
            ActFlowSpeed:=9*50*sign(0.54-PipePress)*Min0R(0.02,abs(0.54-PipePress))
          else
            ActFlowSpeed:=37*50*sign(0.54-PipePress)*Min0R(0.02,abs(0.54-PipePress))
        else
          ActFlowSpeed:=37*50*sign(0.69-PipePress)*Min0R(0.02,abs(0.69-PipePress));
      if (BrakeCtrlPos=0) then
        if (LimPipePress<0.495) then
          ActFlowSpeed:=15*50*sign(0.6-PipePress)*Min0R(0.02,abs(0.6-PipePress))
        else if (PipePress>LimPipePress) then
          ActFlowSpeed:=2*50*sign(0.50-PipePress)*Min0R(0.02,abs(0.5-PipePress));

  if(dpPipe>PipePress)and(PipePress<0.33)and(((BrakeStatus and 4)=0)or(BrakeCtrlPos>0))or(BrakeCtrlPos=-2)then
          ActFlowSpeed:=0;
      if (BrakeCtrlPos=BrakeCtrlPosNo)or(EmergencyBrakeFlag) then
        ActFlowSpeed:=-37*50*Min0R(0.02,PipePress);
{
//yB: dzialanie kranu w jezdzie
    if (BrakeCtrlPos=0)then
      if(PipePress>0.5)and(LimPipePress<0.51)then
        bcp:=-2
      else
        bcp:=0
    else
      bcp:=BrakeCtrlPos;
//yB: powtarzacz, ustala cisnienia
    with BrakePressureTable[bcp] do
     begin
      if not(PipePressureVal=-1)then
        if(BrakeCtrlPos=-1)then
          if(LimPipePress>0.8)and(PipePress+0.01>Min0R(0.69,compressor))then
            LimPipePress:=0.69
          else
        else
        if (BrakeCtrlPos=0)and(LimPipePress>0.51)and(PipePress+0.005<LimPipePress)then
        else

          LimPipePress:=LimPipePress+PR(LimPipePress,PipePressureVal)*dt*0.5*FlowSpeedVal/LBDelay;
      if sign(LimPipePress-PipePress)<>sign(ActFlowSpeed) then
        ActFlowSpeed:=0;
      if abs(LimPipePress-PipePress)>0.001then
        ActFlowSpeed:=sign(LimPipePress-PipePress)*FlowSpeedVal/2;
      if abs(LimPipePress-PipePress)>0.002 then
        ActFlowSpeed:=sign(LimPipePress-PipePress)*FlowSpeedVal;

     end;
//yB: odluzniacz
(*    if(BrakeCtrlPos=0)then
      if(bcp=-1)then
        ActFlowSpeed:=ActFlowSpeed/2
      else
        if(LimPipePress=BrakePressureTable[-1].PipePressureVal)then
          LimPipePress:=BrakePressureTable[0].PipePressureVal;
//yB: mala fala
    if(BrakeCtrlPos=0)and(LimPipePress>0.51)then
      ActFlowSpeed:=ActFlowSpeed*2.5;
//yB: schodzenie powolne *)
    if(BrakeCtrlPos=-1)then
      if(LimPipePress>0.54)and(LimPipePress<0.7)then
        LimPipePress:=LimPipePress-0.015*dt
      else
//yB: syczenie po fali
      if(LimPipePress<0.541)then
        dpMainValve:=0.05;
//yB: odciecie zasilania kranu po naglym  }
//    if(ActFlowSpeed>0)and(PipePress<0.33)and(((BrakeStatus and 4)=0)or(BrakeCtrlPos>0))then
//      ActFlowSpeed:=0;
//    if EmergencyBrakeFlag then ActFlowSpeed:=-37;
   end; //FV4a
  M394: with BrakePressureTable[BrakeCtrlPos] do
   begin
      if (BrakeCtrlPos=1) then LimPipePress:=PipePress else
        if (PipePressureVal>-1) then
          if (BrakeCtrlPos=0) and (LimPipePress>PipePressureVal) then
            LimPipePress:=LimPipePress+PR(LimPipePress,dpPipe)*dt/75
          else
            LimPipePress:=LimPipePress+PR(LimPipePress,dpPipe)*dt/40*FlowSpeedVal/9;
      if BrakeCtrlPos=BrakeCtrlPosNo then
        ActFlowSpeed:=-30*50*Min0R(0.02,PipePress)
      else if BrakeCtrlPos=-1 then
        ActFlowSpeed:=30
      else if (BrakeCtrlPos=0) and (PipePress>LimPipePress) then
        ActFlowSpeed:=1.5*50*sign(LimPipePress-PipePress)*Min0R(0.02,abs(LimPipePress-PipePress))
      else
        ActFlowSpeed:=8*50*sign(LimPipePress-PipePress)*Min0R(0.02,abs(LimPipePress-PipePress));
   end
  else
   begin
    with BrakePressureTable[BrakeCtrlPos] do
  ...
  else if EmergencyBrakeFlag then ActFlowSpeed:=-37*50*Min0R(0.02,PipePress)
  else ActFlowSpeed:=0;


  dpPipe:=Compressor;
if BrakeHandle=FV4a then
  if (ActFlowSpeed>0) then
    GetDVh:=350*ActFlowSpeed*PR(PipePress,dpPipe)*dt/(LBDelay)
  else
    GetDVh:=350*ActFlowSpeed*PR(0,PipePress)*dt/(LBDelay)
else
  if (ActFlowSpeed>0) then
    GetDVh:=350*ActFlowSpeed*PR(PipePress,dpPipe)*dt/(LBDelay)
  else       //*0.67
  ...
  if(dpMainValve<0.00001)then
  dpMainValve:=PR(Min0R(LimPipePress,dpPipe),PipePress)*dt/15*ActFlowSpeed;
end;



function TMoverParameters.GetDVb(dt:real):real;
var BVP,VVP,BCP,BVM,CVP,temp: real;
begin
  BVP:=GetP(BrakeVes);
  VVP:=GetP(ValveVes);
//  BCP:=GetP(BrakeCyl);
...
//  GetDVb:=PR(BrakeVP,ValvePress)*270*dt*Spg/55;
  if BrakeSubSystem=ss_LSt then
   begin
    BrakeVes.Vol:=10*Compressor*BrakeVes.Cap;
    GetDVb:=0;
   end
  else
  if BrakeSubSystem=ss_ESt then                                                                     //       CntrlPipePress*(1-BCP/MBP)+BCP
// v2    GetDVb:=Max0R(0,PR(BVP,VVP))*BrakeVes.Cap*dt/(0.3+byte(BrakeDelayFlag=bdelay_G))*0.125*(Byte(BVP+0.015<CVP-(BCP-0.02)/BVM)+0.4*Byte((CVP<BVP+0.015)and(VVP<CVP+0.04))+0.05*Byte(VVP>=CVP+0.04))
// v3   GetDVb:=Max0R(-0.01*Byte(BCP<0.02),PR(BVP,VVP))*BrakeVes.Cap*dt/(0.3+1.3*byte(BrakeDelayFlag=bdelay_G))*0.11*(0.5*Byte(BVP+0.03<CntrlPipePress*(1-BCP/MaxBrakePress)+BCP)+0.25*Byte((CVP<BVP+0.03)and(VVP<CVP+0.04))+0.05*Byte(VVP>=CVP+0.04))
// v4    GetDVb:=Max0R(-0.015*Byte(BCP<0.02),PR(BVP,VVP))*BrakeVes.Cap*dt/(1+2.5*byte(BrakeDelayFlag=bdelay_G))*0.15*((0.8-0.25*Byte(BVP+0.06<PipePress))*Byte(VVP>BCP*1.3)+0.4*Byte(BVP+0.01<CVP))
//            /(0.3+byte(BrakeDelayFlag=bdelay_G))*0.125*(Byte(BVP+0.015<CVP-BCP/BVM)+0.4*Byte((CVP<BVP-0.015)and(VVP<CVP+0.04))+0.05*Byte(VVP>=CVP+0.04))
// v1   GetDVb:=Max0R(0,PR(GetP(BrakeVes),GetP(ValveVes)))*BrakeVes.Cap*dt/(0.5+byte(BrakeDelayFlag=bdelay_G))/(1+Byte(GetP(BrakeVes)>GetP(ValveVes)-0.04))/8
   begin
     if(BVP<0.46)and(CVP<0.46)then
       temp:=0.1
     else
     if(BVP<CVP-0.02)then
       temp:=1.1
     else
       if(VVP>CVP+0.04)and(BVP>CVP)then
       temp:=0.01
       else
         if(BVP>CVP+0.01)then
         temp:=0.3
         else
         temp:=0.3;
     if BrakeDelayFlag=bdelay_G then
     temp:=1.14*temp*BrakeDelay[1]/BrakeDelay[3];                      //0.4
    GetDVb:=Max0R(-0.05*Byte(BCP<0.02),PR(BVP,VVP))*BrakeVes.Cap*dt*temp*0.34;
   end
  else if(BrakeStatus and 1)=0 then
    GetDVb:=PR(GetP(BrakeVes),GetP(ValveVes))*BrakeVes.Cap*dt/30
  else
    GetDVb:=0;
//    GetDVb:=(PipePress*PipePress*PipePress*PipePress-BrakeVP*BrakeVP*BrakeVP*BrakeVP)*0*dt*Spg;
end;

function TMoverParameters.GetDVv(dt:real):real;
var BVP,VVP,CVP,BCP,temp: real;
begin
  BVP:=GetP(BrakeVes);
  VVP:=GetP(ValveVes);
//  BCP:=GetP(BrakeCyl);
  ...
  CVP:=GetP(CntrlVes);
//  CVP:=0.5-0.01;
  if (BrakeStatus and 4=4) then
   begin
    GetDVv:=0; CntrlVes.Vol:=CntrlVes.Vol+PR(CVP,0)*CntrlVes.Cap*dt/2;
//    CntrlVes.Vol:=0; BrakeVes.Vol:=0;
   end
  else
  if BrakeSubSystem=ss_LSt then
   begin
    GetDVv:=Byte(BCP<0.02)*PR(CVP,Min0R(VVP,0.5))*CntrlVes.Cap*dt*0.1;
    CntrlVes.Vol:=CntrlVes.Vol+Max0R(0,PR(CVP,compressor))*10*dt*Byte((PipePress-0.01>CVP) and (CVP+0.005<BrakePressureTable[0].PipePressureVal));
    ValveVes.Vol:=ValveVes.Vol+Max0R(0,PR(VVP,compressor))*5*dt*Byte((PipePress-0.01>VVP) and (VVP+0.005<BrakePressureTable[0].PipePressureVal) and (CVP+0.005<BrakePressureTable[0].PipePressureVal));
   end
  else
  if BrakeSubSystem=ss_ESt then
   begin
     if(BVP<0.46)and(CVP<0.46)then
       temp:=0.1
     else
     if(BVP<CVP-0.02)or(BrakeStatus>0)then
       temp:=0
     else
       if(VVP>CVP+0.05)then
       temp:=0.01
       else
         if(BVP>CVP-0.01)then
         temp:=0.3
         ...
         temp:=0.07;
     if BrakeDelayFlag=bdelay_G then
     temp:=1.14*temp*BrakeDelay[1]/BrakeDelay[3];         
      GetDVv:=PR(CVP,VVP)*CntrlVes.Cap*dt*temp*0.34;
//    GetDVv:=PR(CVP,VVP)*CntrlVes.Cap*dt*0.15*Byte(BCP<0.03)*((0.25-0.15*Byte(CVP>BVP))*Byte((VVP>CVP-0.017)and not(CVP<BVP-0.01))+0.5*Byte(CVP<BVP-0.01))/(1+9*Byte(VVP>=CVP+0.04))
   end
  else
    GetDVv:=0;
end;
...
//Vs=(Vp-Vh)*MBP/(0,5-MBP)
//Vp=(X-p/MBP)*Vh
procedure TMoverParameters.ValveCheck(dt:real);
var BVP,VVP,BCP,BVM,CVP: real;
begin
if(BrakeSystem<>Individual)then
 begin
  BVP:=GetP(BrakeVes);
//  VVP:=Min0R(GetP(ValveVes),BVP+0.02);
  VVP:=GetP(ValveVes);
  BrakePress:=GetP(BrakeCyl);
  if IVr>0 then
    BCP:=GetP(ImpulseVes)
  else
    BCP:=GetP(BrakeCyl);

  CVP:=GetP(CntrlVes)-0.005;
//  CVP:=0.5-0.01;
  BVM:=2.73;
  case BrakeSubsystem of
    ss_W:
     begin
       if (BrakeStatus and 1)=1 then
         if(VVP+0.003<BVP)then
           BrakeStatus:=(BrakeStatus or 2)
         else if(VVP>BVP+0.01) then
           BrakeStatus:=(BrakeStatus and 252)
         else if(VVP>BVP) then
           BrakeStatus:=(BrakeStatus and 253)
         else
       else
         if(VVP+0.02<BVP) then
           BrakeStatus:=(BrakeStatus or 3);
     end;
    ss_Kk:
     begin
       if (BrakeStatus and 1)=1 then
         if(VVP+0.003<CVP)then
           BrakeStatus:=(BrakeStatus or 2)
         else if(VVP>CVP+0.005) then
           BrakeStatus:=(BrakeStatus and 252)
         else if(VVP>CVP) then
           BrakeStatus:=(BrakeStatus and 253)
         else
       else
         if(VVP+0.02<CVP) then
           BrakeStatus:=(BrakeStatus or 3)
         else if(VVP<CVP+0.005) then
    ...
    ss_ESt:
     begin
       VVP:=Min0R(GetP(ValveVes),BVP+0.025);     
       if (BrakeValve=ESt4)and((BrakeDelays and bdelay_R)=bdelay_R)then
         if ((BrakeDelayFlag and bdelay_R)=bdelay_R)and(Vel<50)then
           IVr:=0.55
         else if ((BrakeDelayFlag and bdelay_R)=bdelay_R)and(Vel>70)then
           IVr:=1
         else if((BrakeDelayFlag and bdelay_R)=0)then
           IVr:=0.55;

        if(BrakeStatus and 4=4)and(CVP-VVP<0)then
          BrakeStatus:=BrakeStatus and 251;

       if(BrakeValve=EP2)then
         BrakeVes.Vol:=BrakeVes.Cap*5;

       if (BrakeStatus and 1)=1 then
         if(VVP+0.001+BCP/BVM<CVP)then
           BrakeStatus:=(BrakeStatus or 2)
         else if(VVP-0.001+BCP/BVM>CVP) then
           BrakeStatus:=(BrakeStatus and 252)
         else if(VVP+BCP/BVM>CVP) then
           BrakeStatus:=(BrakeStatus and 253)
         else
       else
         if(VVP+0.01<CVP)and(BCP<0.01) then
          begin
           BrakeStatus:=(BrakeStatus or 3);
           Pipe.Vol:=Pipe.Vol*Pipe.Cap/(Pipe.Cap+0.5);
          end
         else if(VVP+BCP/BVM<CVP)and(BCP>0.01) then
           BrakeStatus:=(BrakeStatus or 1);
     end;
    ss_LSt:
     begin
        if(BrakeStatus and 4=4)and(CVP-VVP<0)and(ActiveCab=0)then
          BrakeStatus:=BrakeStatus and 251;

       BVM:=2.88;
       if(BrakeDelayFlag=bdelay_R)and(Vel>55)then
         BVM:=BVM*1.3;

       if (BrakeStatus and 1)=1 then
         if(VVP+0.0004+BCP/BVM<CVP)then
           BrakeStatus:=(BrakeStatus or 2)
         else if(VVP-0.0004+BCP/BVM>CVP) then
           BrakeStatus:=(BrakeStatus and 252)
         else if(VVP+BCP/BVM>CVP) then
           BrakeStatus:=(BrakeStatus and 253)
         else
       else
         if(VVP+0.0004<CVP) then
           BrakeStatus:=(BrakeStatus or 1)
         else if(VVP+BCP/BVM<CVP) then
           BrakeStatus:=(BrakeStatus or 3);
     end;
...
const LBDelay=15.0;
var dv,dv1,pvp,pbp,sm:real;
jakisx:byte;
begin
if (BrakeSubSystem<>ss_None) and (BrakeSystem<>Individual) then
begin

  if (GetDVh(dt)>0) and (GetP(Pipe)>0.02) then
    CompressedVolume:=CompressedVolume-GetDVh(dt)/1000;
  Pipe.Vol:=Pipe.Vol+GetDVh(dt)+GetDVc(dt);
...
//  dpMainValve:=
  ...
  if(BrakeSubsystem=ss_LSt)then
   begin
    if(PR(pvp,GetP(Pipe))>0)then
      sm:=(0.5/BrakeDelay[1+jakisx])
    else if(GetP(ValveVes)<GetP(CntrlVes)-0.16)then
      sm:=0
    else
      sm:=0.5/BrakeDelay[2+jakisx];
   end
  else
    sm:=0.4;
  ...
  dV:=GetDVb(dt);
  dv1:=GetDVv(dt);
  BrakeVes.Vol:=BrakeVes.Vol+dV;
  CntrlVes.Vol:=CntrlVes.Vol+dv1;
  dV:=dV+dv1;


  ValveVes.Vol:=ValveVes.Vol-dV*(1.5*ValveVes.Cap)/(1.5*ValveVes.Cap+Pipe.Cap)+PR(pvp,GetP(Pipe))*dt*sm;
  Pipe.Vol:=Pipe.Vol-PR(pvp,GetP(Pipe))*dt*sm-dV*(Pipe.Cap)/(1.5*ValveVes.Cap+Pipe.Cap)-0.15*(0.2+Byte(GetP(BrakeCyl)<0.02))*dt;
         ...
         begin
           if (Compressor<MaxCompressor) then
            if (EngineType=DieselElectric) and (CompressorPower>0) then
             CompressedVolume:=CompressedVolume+dt*CompressorSpeed*(2*MaxCompressor-Compressor)/MaxCompressor*(1+(DEList[MainCtrlPos].rpm/DEList[MainCtrlPosNo].rpm))/2
            else
...
procedure TMoverParameters.UpdateBatteryVoltage(dt:real);
...
begin
  ...
  if (EngineType=DieselElectric) then
   begin
    if (ConverterAllow=true)and(Mains=true) then
      ConverterFlag:=true;
    if((EnLiqs[el_water][el_act][el_T]<EnLiqs[el_water][el_off][el_T]) and (EnLiqs[el_water][el_off][el_T]>0))
    or((EnLiqs[el_fuel][el_act][el_P]<EnLiqs[el_fuel][el_off][el_P]) and (EnLiqs[el_fuel][el_off][el_P]>0))
    or((EnLiqs[el_oil][el_act][el_P]<EnLiqs[el_water][el_off][el_P]) and (EnLiqs[el_water][el_off][el_P]>0))then
      ConverterFlag:=false;
   end;
end;
...
begin
  ...
  if (VeselVolume>0)and(Compressor>ScndPipePress) then
    USPP:=20*PR(ScndPipePress,Compressor)*dt
  else
    USPP:=0;
  ...
  if Couplers[0].Connected<>nil then
   if TestFlag(Couplers[0].CouplingFlag,ctrain_scndpneumatic) then
    begin
      c:=Couplers[0].Connected; //skrot
      USPP:=-PR(ScndPipePress,c^.ScndPipePress)*240*dt;
...
begin
if (MainCtrlPos=0) and (ScndCtrlPos=0) and (TrainType<>'et40') and Mains then
 begin
...
begin
  ...
  if DynamicBrakeFlag and (TrainType='et42') then { KURS90 azeby mozna bylo hamowac przy opuszczonych pantografach }
...
begin
  ...
  if (EngineType=ElectricSeriesMotor) and (ImaxHi>ImaxLo) then
   begin
     if State and (Imax=ImaxLo) and (RList[MainCtrlPos].Bn<2) and not ((TrainType='et42')and(MainctrlPos>0)) then
      begin
        Imax:=ImaxHi;
     ...
     if (not State) and (Imax=ImaxHi) and not ((TrainType='et42')and(MainctrlPos>0)) then
      begin
        Imax:=ImaxLo;
        MaxCurrentSwitch:=True;
        if CabNo<>0 then
...
begin
  ...
  if ((EngineType=ElectricSeriesMotor) and (IminHi>IminLo)) or (TrainType='ezt') then
   begin
...
begin
//  if ((TrainType='ezt'{) or (TrainType='ET22')}) and (Imin=IminLo)) or ((ActiveDir<0) and (TrainType<>'pseudodiesel')) then
...
if ((not Mains) or (FuseFlag)) and not((DynamicBrakeFlag) and (TrainType='et42')) then
   ...
   if (LastRelayTime>CtrlDelay) and not DelayCtrlFlag then
      ...
      if (((RList[MainCtrlActualPos].R=0) and ((not CoupledCtrl) or ((Imin=IminLo) and (ScndS=True)))) or (MainCtrlActualPos=RListSize))
         ...
         if (not AutoRelayFlag) or (not MotorParam[ScndCtrlActualPos].AutoSwitch) then
           ...
           begin                                                {scnd z samoczynnym rozruchem}
             if (ScndCtrlPos<ScndCtrlActualPos) and (TrainType<>'ezt') then
              begin
                dec(ScndCtrlActualPos);
                OK:=True;
              end
        ...
        begin          {zmieniaj mainctrlactualpos}
          if ((TrainType='ezt') and (Scnds=true) and (Imin=IminLo)) or ((ActiveDir<0) and (TrainType<>'pseudodiesel'))then
          ...
          if (not AutoRelayFlag) or (not RList[MainCtrlActualPos].AutoSwitch) then
           begin                                                {main bez samoczynnego rozruchu}
             ...
             if Rlist[MainCtrlActualPos].Relay<MainCtrlPos then
                if ((Rlist[MainCtrlActualPos].Mn>Rlist[MainCtrlPos].Mn)and (RList[MainCtrlActualPos+1].Mn<Rlist[MainCtrlActualPos].Mn) and (TrainType='et22') and (LastRelayTime<InitialCtrlDelay))then
                 begin
                 Itot:=0;
                 Im:=0;
              ...
              else if (Rlist[MainCtrlActualPos].Relay>MainCtrlPos) and (TrainType='ezt') and (MainCtrlPos>0) then     //K90
              begin
              Exit;      {utkniecie walu kulakowego}
              end
             else if  (TrainType='ezt') and (MainCtrlPos=0) then
              MainCtrlActualPos:=0
              else
              if (Rlist[MainCtrlActualPos].R>0) and (ScndCtrlActualPos>0) and (TrainType<>'ezt') then
           ...
           begin
             OK:=False;
             if (MainCtrlPos<Rlist[MainCtrlActualPos].Relay) and (TrainType<>'ezt') then
             begin
             if not ((Rlist[MainCtrlActualPos].Mn<Rlist[MainCtrlPos].Mn)and (Rlist[MainCtrlActualPos-1].Mn>Rlist[MainCtrlActualPos].Mn) and (TrainType='et22') and (LastRelayTime<InitialCtrlDelay))then
              begin
                dec(MainCtrlActualPos);
             ...
             if (MainCtrlPos<Rlist[MainCtrlActualPos].Relay) and (MainCtrlPos>0) and (TrainType='ezt')  then
              begin
                AutoRelayCheck:=False;
                Exit;    {utkniecie walu}
              end
             else
             if (TrainType='ezt') and (MainCtrlPos=0) then
              begin
                MainCtrlActualPos:=0;
                ScndCtrlActualPos:=0;
              end
     ...
     if ((MainCtrlPos=0) and (TrainType='ezt')) then
      begin
        MainCtrlActualPos:=0;
      ...
      if (MainCtrlPos=1) and (MainCtrlActualPos=0) then
       ...
       if (MainCtrlPos=0) and (MainCtrlActualPos>0) and (TrainType<>'ezt') and (TrainType<>'et40') and (TrainType<>'ep05') then
        begin
         dec(MainCtrlActualPos);
         OK:=true;
        end;
       if (MainCtrlPos=0) and (MainCtrlActualPos>0) and ((TrainType='et40')or(TrainType='ep05')) and (LastRelayTime>(InitialCtrlDelay*2)) then
        begin
         dec(MainCtrlActualPos);
...
begin
  ...
  if (Power>0.01) and ((DynamicBrakeFlag) and (DynamicBrakeType=dbrake_passive)) then
  ResistorsFlagCheck:=false;
end;
...
//youBy: obroty spaliniaka
  if (EngineType=DieselElectric) then
    begin
     tmp := DEList[MainCtrlPos*Byte(not ScndS)].rpm/60;
     ...
     if enrot<>tmp*Byte(ConverterFlag) then
       if ABS(tmp*Byte(ConverterFlag) - enrot) < 0.001 then
         ...
         if (enrot<DEList[0].rpm*0.01) and (ConverterFlag) then
           ...
           enrot := enrot + (tmp*Byte(ConverterFlag) - enrot)*dt;

     PosRatio:=MainCtrlPos/MainCtrlPosNo;
//youBy: temperatury cieczy
     tmp := EnLiqs[el_oil][el_act][el_T];
     EnLiqs[el_oil][el_act][el_T]:=tmp+dt/10*(0.4-tmp/60+(EnLiqs[el_oil][el_max][el_T]-tmp)/30*Byte(PantFrontUp and Mains)+
     +((PosRatio*EnLiqs[el_oil][el_max][el_T]+(1-PosRatio)*EnLiqs[el_oil][el_min][el_T]-tmp)/30*Byte(ConverterFlag)));

     EnLiqs[el_water][el_act][el_T]:=EnLiqs[el_oil][el_act][el_T];


     tmp := EnLiqs[el_fuel][el_act][el_P];
     EnLiqs[el_fuel][el_act][el_P]:=tmp+dt*(-tmp/40+(EnLiqs[el_fuel][el_min][el_P]-tmp)/10*Byte(diel_fuel and Mains)+
     +((PosRatio*EnLiqs[el_fuel][el_max][el_P]+(1-PosRatio)*EnLiqs[el_fuel][el_min][el_P]-tmp)/20*Byte(ConverterFlag)));

     tmp := EnLiqs[el_oil][el_act][el_P];
     EnLiqs[el_oil][el_act][el_P]:=tmp+dt*(-tmp/40+(EnLiqs[el_oil][el_min][el_P]-tmp)/10*Byte(PantRearUp and Mains)+
     +((PosRatio*EnLiqs[el_oil][el_max][el_P]+(1-PosRatio)*EnLiqs[el_oil][el_min][el_P]-tmp)/20*Byte(ConverterFlag)));
...
{        enrot:=Transmision.Ratio*nrot; }
        ...
        if(TrainType='et42')and(Imax=ImaxHi)then
          Voltage:=Voltage/2;
...
//       tmpV:=V*CabNo*ActiveDir;
      ...
      PosRatio:=tmp/DEList[MainCtrlPosNo].genpower;
        ...
        if (ScndS) then Ft:=-sign(V)*Byte(MainS)*Min0R(230000*MainCtrlPos/MainCtrlPosNo,Vel*2300) else
       ...
       if(TrainType='sp32')then    
       RventRot:=RventRot+(Rventnmax*Im/ImaxLo*byte(ScndS)-RventRot)*RVentSpeed*dt;
       if (ScndS) then
         begin
          Voltage:=0;
         end 
         ...
         if (MainCtrlPos = 0) or (ShuntMode) or (ScndS) then
          ...
          41:
            begin 
              ...
              if((MainCtrlPos<MainCtrlPosNo)and(Im>MPTRelay[ScndCtrlPos].Idown))and(ScndCtrlPos>0)then
                dec(ScndCtrlPos); 
            end; 
          45: 
            begin 
              ...
              if (ScndCtrlPos=0) then
                ...
                if (MPTRelay[ScndCtrlPos].Iup<Vel) and (EnginePower<(tmp*0.99))then
                inc(ScndCtrlPos); 
             ...
             begin
              ...
              if (MainCtrlPos>11) and (ScndCtrlPos<ScndCtrlPosNo)then
               if (ScndCtrlPos) mod 2 = 0 then
                ...
                if (MPTRelay[ScndCtrlPos-1].Iup>Im) and (MPTRelay[ScndCtrlPos].Iup<tmpV*3.6) then
                 inc(ScndCtrlPos);

              //malenie
              if (MainCtrlPos<12) and (ScndCtrlPos>0)then
               if (ScndCtrlPos) mod 2 = 0 then
                if (MPTRelay[ScndCtrlPos].Idown<Im)then
                 dec(ScndCtrlPos)
                else
               else
                if (MPTRelay[ScndCtrlPos+1].Idown<Im) and (MPTRelay[ScndCtrlPos].Idown>tmpV*3.6)then
                 dec(ScndCtrlPos);
              if (MainCtrlPos<11)and(ScndCtrlPos>2) then ScndCtrlPos:=2;
              if (MainCtrlPos<8)and(ScndCtrlPos>0) then ScndCtrlPos:=0;
             end;
          47:
             begin
              if (MainCtrlPos<9)then ScndCtrlPos:=0 else
              if (MPTRelay[ScndCtrlPos].Iup<Vel) and (ScndCtrlPos<ScndCtrlPosNo) then
                inc(ScndCtrlPos);
              if (MPTRelay[ScndCtrlPos].Idown<Im) and (ScndCtrlPos>0) then
                dec(ScndCtrlPos);
...
begin
  ...
  if TestFlag(BrakeDelays,bdelay_R)then
   if(not TestFlag(BrakeStatus,b_Ractive)and(BrakeCtrlPosNo=0))then
     ...
     sm:=1.5;
                //0.03
  if BrakePress>(0.03) then         {nie luz}
   begin
...
//     else
      K:=K+(BrakePress-0.01)*P2FTrans;                     {w kN}
     K:=K*BrakeCylNo/(NBrakeAxles*NBpA);            {w kN na os}
   end;
  if (BrakeMethod and 1 = 0) then
    u:=0.6*((1.6*K+100)/(8.0*K+100))*((vel+100)/(5*vel+100))  {wsp. tarcia klockow}
//    u:=60*(0.16*K+100)/((0.8*K+100)*(3*Vel+100)){wsp. tarcia}
 else
    u:=0.5*(0.4*K+100)*(Vel+80)/((0.8*K+100)*(2*Vel+80)); {wsp. tarcia tarcz}
...
//    end;
  ...
  {  FStand:=0; }
   ...
   // begin
     if (CabNo=0) and (Vel<0.0001) and (abs(AccS)<0.0001) and (TrainType<>'ezt') then
      begin
...
function TMoverParameters.DoorBlockedFlag:Boolean;
begin
...
var //k:integer;
    ...
    bcmss: byte;
const
    DefBrakeTable:array[1..8]of Byte=(15,4,25,40,10,3,5,2);
begin
  OK:=True;
...
if (BrakeSubSystem<>ss_None)and(BrakeSystem<>Individual) then
begin
   BrakeCyl.Cap:=1000*BrakeCyl.Cap;
 if(BrakeVes.Cap=0)and(MaxBrakePress>0)then
   BrakeVes.Cap:=MaxBrakePress/(CntrlPipePress-MaxBrakePress)*BrakeCyl.Cap;
 if BrakeVes.Cap=0 then BrakeVes.Cap:=1;
 if BrakeVes.Cap<0 then BrakeVes.Cap:=BrakeCyl.Cap*5;
end;

 {to dac potem do init}
  if ReadyFlag then     {gotowy do drogi}
   begin
//     BrakeVes.Vol:=BrakeVes.Cap*HighPipePress*10;
...
//       Volume:=BrakeVVolume*MinCompressor*10;
     ...
     MainSwitch(true);     
     ActiveDir:=1;
     LimPipePress:=HighPipePress;
   end
  else
   begin                {zahamowany}
//     Volume:=BrakeVVolume*(MaxBrakePress+HighPipePress)*5;
...
//yB: jesli pojazdy nie maja zadeklarowanych czasow, to wsadz z przepisow +-16,(6)%
  ...
  if(BrakeSystem<>Individual)then
  begin
  Sp:=Spg*(1+0.5*byte((BrakeCtrlPosNo>0)and(Power>1)and(BrakeSystem=Pneumatic)));
  Pipe.Cap    := Sp*(Dim.L+0.5);
  Pipe2.Cap   := Spg*(Dim.L+0.5);
  Pipe.Vol    := PipePress*Pipe.Cap*10;
  Pipe2.Vol   := Pipe2.Cap*10*ScndPipePress;
  MainVes.Cap := VeselVolume*1000;
  MainVes.Vol := CompressedVolume*1000;
//  BrakeCyl.Cap:= BrakeVolume;
//  BrakeVes.Cap:= BrakeVVolume;
  BrakeCyl.Vol:= BrakePress*BrakeCyl.Cap*10;
//  BrakeVes.Vol:= Volume*0.1/BrakeVVolume;
  BrakeVes.Vol:= 10*BrakeVes.Cap*HighPipePress-BrakeCyl.Vol;
  CntrlVes.Cap:= 10;
  CntrlVes.Vol:= 50-3*Byte(GetP(BrakeCyl)>0.07);
  ValveVes.Cap:= 0.2;
  ValveVes.Vol:= PipePress*10*ValveVes.Cap;
  if(BrakeValve=ESt4)and((BrakeDelays and bdelay_R)=bdelay_R)or(BrakeValve=EP2)then
   begin
    IVr:=1;
    ImpulseVes.Cap:=3;
    ImpulseVes.Vol:=BrakeCyl.Vol/BrakeCyl.Cap*ImpulseVes.Cap;
   end
  else
   begin
    IVr:=0;
 ...
 if (PantFrontUp<>State)  then
  ...
  if (State=true) then
   ...
   begin
      ...
      if (TrainType='ezt') and (ActiveCab=1) then
       begin
        PantRearUp:=false;
        PantRearStart:=1;
...
begin
  case BrakeValve of
    ...
    ESt3,ESt4,EP2,EP1: BrakeSubsystem:=ss_ESt;
        ...
        begin
          if Pos('Param.',lines)>0 then      {stale parametry}
            begin
              ...
              s:=ExtractKeyWord(lines,'Type=');
              TrainType:=DUE(s);
              if TrainType='ezt' then
              begin
              IminLo:=1;IminHi:=2;Imin:=1;
              end;
              if TrainType='sp32' then
               RVentnmax:=50;
            end;
...
//              if(BrakeSystem<>Individual)then              
               ...
               begin
                 ...
                 if BrakeCylNo>0 then
                  begin
                   ...
                   BCMFlag:=2;
                   BrakeCyl.Cap:=Pi*SQR(BrakeCylRadius)*BrakeCylDist*BrakeCylNo;
                  end
          ...
          if Pos('Doors:',lines)>0 then      {drzwi}
            begin
              ...
              s:=ExtractKeyWord(lines,'FloorR=');
              DoorMaxShiftL:=Max0R(DoorMaxShiftL,s2r(DUE(s)));
              s:=ExtractKeyWord(lines,'WallWheelD=');
              DoorMaxShiftR:=Max0R(DoorMaxShiftR,s2r(DUE(s))/2);              
...
{                 if AutoRelayType=0 then
          ...
          if Pos('EngineLiquids:',lines)>0 then
            for k:=el_oil to el_fuel do
            begin
                read(  fin, EnLiqs[k][el_min][el_P], EnLiqs[k][el_max][el_P], EnLiqs[k][el_off][el_P]);
                readln(fin, EnLiqs[k][el_min][el_T], EnLiqs[k][el_max][el_T], EnLiqs[k][el_off][el_T]);
            end;
//youBy
