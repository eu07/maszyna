...
{                        VelDesired:=Min0R(VelDesired,VelReduced+VelNext); }
                      ...
                      if ActualProximityDist>MinProximityDist then
                       begin                                //25.92 - skorektowane, zeby ladniej hamowal
                         if (VelNext>Vel-20) then
                           AccDesired:=(SQR(VelNext)-SQR(Vel))/(25.92*ActualProximityDist+0.1) {hamuj proporcjonalnie}
                         else
                           AccDesired:=(SQR(VelNext)-SQR(Vel))/(21.53*ActualProximityDist+0.1); {hamuj proporcjonalnie}
...
//                      if ((BrakeSubsystem=ss_LSt) and (PipePress<0.55) and (LimPipePress<0.55)) or (BrakeSubsystem<>ss_LSt) then
                ...
                if ((AbsAccS<AccDesired-0.1)and(Vel+20<VelDesired))or((Vel<VelDesired*0.7-VelMargin) or ((Vel<VelDesired*0.94-VelMargin) and (AbsAccS<0)))
       ...
       {          if not MaxVelFlag then }
                ...
                if ((Vel>VelDesired) or ((Vel+VelMargin>VelDesired*0.95) and (AbsAccS>0)))
                or ((AccDesired<0) and (AbsAccS>AccDesired)) then
       {          if not MinVelFlag then   }
                   begin
                      if not DecSpeed then         // or ((Vel-VelMargin)>(VelMargin+VelDesired))
                       if (AccDesired<{-0.1*Byte(VelNext>Vel-20)}-0.1+AbsAccS) then
