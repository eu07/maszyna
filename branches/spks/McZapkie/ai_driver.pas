...
//       Pipe.Vol:=Pipe.Cap*5;
//       PipePress:=0.5;
     if (FuseFlag) and (MainCtrlPos=0) and (ScndCtrlPos=0) then
     FuseFlag:=False;
     Battery:=true;
     if (EngineType=DieselElectric) then
      begin
       Enliqs[el_oil][el_act][el_T]:=Enliqs[el_oil][el_min][el_T]+10;
       PantFront(true);
       PantRear(True);
       diel_fuel:=true;
      end;
...
{                else
       ...
       //         AbsAccs:=Accs*sign(V); {czy sie rozpedza czy hamuje}
                AbsAccs:=(Vel-LastVel)/(3.6*dt); {czy sie rozpedza czy hamuje}
                if BrakeCtrlPos>0 then ReactionTime:=0.5;
...
//                     if (Vel<VelReduced+VelNext) and (ActualProximityDist>(+SQR(Vel)-SQR(VelNext))/3) then {dojedz do semafora/przeszkody}
                      begin
...
//                         VelDesired:=Min0R(VelDesired,VelReduced+VelNext);
                      ...
                      if ActualProximityDist>MinProximityDist then
                       begin                                //25.92 - skorektowane, zeby ladniej hamowal
                         if (VelNext>Vel-30) then
                          if ((VelNext>0)and(VelNext>Vel-10)) then
                            AccDesired:=0
                          else
                            if (VelNext=0)and(Vel*Vel<0.4*ActualProximityDist) then
                              if Vel<30 then
                                AccDesired:=0.5
                              else
                                AccDesired:=0
                           ...
                           AccDesired:=(SQR(VelNext)-SQR(Vel))/(20.73*ActualProximityDist+0.1); {hamuj proporcjonalnie}
                         if AccPreferred<AccDesired then
                          AccDesired:=AccPreferred;
//                         ReactionTime:=3+(BrakeDelay[2+2*BrakeDelayFlag]-3)/2; {aby szybkosc hamowania zalezala od przyspieszenia i opoznienia hamulcow}
                       end
                      else
                ...
                if (VelDesired>=0) and (Vel>VelDesired) then
                 if (VelDesired=0) then
                     AccDesired:=-0.5
                 else
                 if (Vel<VelDesired+10)then
...
//                  Accdesired:=-AccPreferred;
                ...
                if (BrakeSystem=Pneumatic) or (BrakeSystem=ElectroPneumatic) then  {napelnianie uderzeniowe}
//yB                 if BrakeSubsystem=Oerlikon then
                  begin
                    if (Ready=False)and(AccDesired>0)then
                     if(LocalBrakePos>0)then DecLocalBrakeLevel(10) else
                     if(BrakeCtrlPos=0)then
                      if (PipePress<0.51)and not((PipePress>0.49) and (BrakePress<0.05)) then
//                        BrakeCtrlPos:=0
                       begin
                        DecBrakeLevel;
//                        BrakeReleaser();
                     ...
                     else if(BrakeCtrlPos=-2)and(ScndPipePress>BrakePressureTable[-1].PipePressureVal) then
                      IncBrakeLevel
                     else if Need_BrakeRelease then
                       if (PipePress<0.34)and(ScndPipePress>BrakePressureTable[-1].PipePressureVal)then
                        begin
//                         PipePress:=0.37;
                         Pipe.Vol:=Pipe.Cap*3.7;
                         Need_BrakeRelease:=false;
                        end
                       else
                      Need_BrakeRelease:=false
                     else if (BrakeCtrlPos=-1)and((PipePress>0.59)or(BrakePress<0.03))then IncBrakeLevel
                     else
                    else if (BrakeCtrlPos<0)and(Vel<1)and(VelNext<1)and(PipePress>HighPipePress) then
                      IncBrakeLevel;

                    if (Ready=True) then
                     if BrakeCtrlPos<0 then
//                      if ((BrakeSubsystem=ss_LSt) and (PipePress<0.55) and (LimPipePress<0.55)) or (BrakeSubsystem<>ss_LSt) then
                       IncBrakeLevel;
                  end;

                if(AccDesired>=0) then while DecBrake do;
                if(AccDesired<=0)or(Vel+VelMargin>VelDesired*0.95) then while DecSpeed do;
...
//                    IncMainCtrl(1); {zwieksz nastawnik skoro mozesz - tak aby sie ustawic na bezoporowej}
       ...
       {          if not MinVelFlag then   }
                ...
                if ((AccDesired<-0.05) and (AbsAccS<AccDesired-0.2)) then
                 begin DecBrake; ReactionTime:=(BrakeDelay[1+2*BrakeDelayFlag])/3; end;
//                   end;
//Mietek-end1
...
//                 end;
             ...
             if (v=0) and ((VelDesired=0) or (AccDesired=0)) then
              if (BrakeCtrlPos<1) or (not DecBrake) then
               IncLocalBrakeLevel(5);
...
' Time [s]   Velocity [m/s]  Acceleration [m/ss]   Coupler.Dist[m]  Coupler.Force[N]  TractionForce [kN]  FrictionForce [kN]   BrakeForce [kN]    BrakePress [MPa]   PipePress [MPa]   MotorCurrent [A]    MCP SCP BCP LBP DmgFlag Command CVal1 CVal2');
  ...
  ScanMe:=False;
  VelMargin:=Controlling^.Vmax*0.02; //0,005
  WarningDuration:=-1;
