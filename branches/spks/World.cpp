...
__fastcall TWorld::Init(HWND NhWnd, HDC hDC)
{
    ...
    WriteLog(GLEW_ARB_texture_compression ? "Texture Compression" : "No texture compression");
...
//        SetCursorPos(Global::iWindowWidth/2,Global::iWindowHeight/2);
   ...
   if ((Pressed(VK_RBUTTON)) || (Pressed(VK_F4)))
    ...
    {Ground.Update(dt,n);Ground.Update(dt,n);Ground.Update(dt,n);Ground.Update(dt,n);}
    //blablabla
    //Sleep(50);
...
//    glBlendFunc(GL_SRC_ALPHA_SATURATE,GL_ONE);
    ...
    glDisable(GL_FOG);
    float nnn=(0.18f*sqrt(1-lightsum));
    glColor4f(nnn,nnn,nnn,0.25f);
    ...
    if (DebugModeFlag)
     {
       ...
       OutText1+= (" t: ") + FloatToStrF(((z+=dt)>100?z-=100:z),ffFixed,6,2);
...
//          OutText2+= AnsiString(" AnPos: ")+FloatToStrF(tmp->MoverParameters->AnPos,ffFixed,5,2)+AnsiString(". ");
          OutText2+= AnsiString(" UWX: ")+FloatToStrF(tmp->uwx,ffFixed,5,2)+AnsiString(". ");
...
//          OutText3+= FloatToStrF(tmp->GetLastDynamic(1)->MoverParameters->BrakePress,ffFixed,5,2)+AnsiString(", ");
          OutText3+= AnsiString("Pipe press: ")+FloatToStrF(tmp->MoverParameters->Pipe.Vol/tmp->MoverParameters->Pipe.Cap,ffFixed,5,2)+AnsiString(", ");
...
////          OutText3+=FloatToStrF(tmp->MoverParameters->EnLiqs[el_oil][el_act][el_P],ffFixed,5,2)+AnsiString(", ");
////          OutText3+=FloatToStrF(tmp->MoverParameters->EnLiqs[el_oil][el_act][el_T],ffFixed,5,2)+AnsiString(", ");
////          OutText3+=FloatToStrF(tmp->MoverParameters->EnLiqs[el_fuel][el_act][el_P],ffFixed,5,2)+AnsiString(", ");
          +FloatToStrF(tmp->MoverParameters->BrakeVes.Vol/tmp->MoverParameters->BrakeVes.Cap,ffFixed,5,2)+AnsiString(", ");
          OutText3+=FloatToStrF(tmp->MoverParameters->CntrlVes.Vol/tmp->MoverParameters->CntrlVes.Cap,ffFixed,5,2)+AnsiString(", ");
          OutText3+=FloatToStrF(tmp->MoverParameters->ValveVes.Vol/tmp->MoverParameters->ValveVes.Cap,ffFixed,5,2)+AnsiString(", ");
