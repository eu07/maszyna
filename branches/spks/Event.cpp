...
bool __fastcall TEvent::Load(cParser* parser)
{
    ...
    *parser >> token;
    ...
    if (str==AnsiString("load"))
        Type= tp_Load;
    else
    if (str==AnsiString("unload"))
        Type= tp_RideUnload;
    else
        Type= tp_Unknown;
...
//            Params[3].asdouble=Parser->GetNextSymbol().ToDouble(); //polozenie X,Y,Z - do poprawy!
        ...
        case tp_Load:
            parser->getTokens();
            *parser >> token;
            Params[1].asText= new char[255];
            strcpy(Params[1].asText,token.c_str());
            parser->getTokens(2);
            *parser >> Params[2].asInt >> token;
            Params[3].asText= new char[255];
            strcpy(Params[3].asText,token.c_str());
            parser->getTokens(2);
            *parser >> Params[4].asInt >> Params[5].asInt;
            parser->getTokens(); *parser >> token;
        break;
        case tp_RideUnload:
            parser->getTokens(5);
            *parser >> Params[1].asdouble >> Params[2].asdouble >> Params[3].asdouble >> Params[4].asdouble >> token;
        break;
        case tp_Multiple:
            i= 0;
            Params[8].asInt= 0;
