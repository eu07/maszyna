...
void TDynamicObject::VEP()
{ //Ustalanie cisnienia w hamulcach po calym skladzie
  //taki Virtual EP dla AI

   TDynamicObject* temp=this;
   int coupler_nr=1;

   while(temp->MoverParameters->Couplers[coupler_nr].CouplingFlag&2)
   {
      if (coupler_nr==0)
         {
          if (temp->PrevConnectedNo==coupler_nr)
      ...
      temp->MoverParameters->Pipe.Vol=10*temp->MoverParameters->Pipe.Cap*MoverParameters->PipePress;
      if(temp->Mechanik==NULL)
      temp->MoverParameters->BrakeCyl.Vol=MoverParameters->BrakeCyl.Vol/MoverParameters->BrakeCyl.Cap*temp->MoverParameters->BrakeCyl.Cap;
      temp->MoverParameters->BrakeVes.Vol=5*temp->MoverParameters->BrakeVes.Cap-temp->MoverParameters->BrakeCyl.Vol;
      temp->MoverParameters->CntrlVes.Vol=4.9*temp->MoverParameters->CntrlVes.Cap;            
   }
...
//*************************************************************/// koniec wezykow
   ...
   if(ObjSqrDist<1e6)
    {
     if (smAnimatedWall[0])
       smAnimatedWall[0]->SetTranslate(vector3(0,uwx,0));
     if (smAnimatedWall[1])
       smAnimatedWall[1]->SetRotateXYZ(vector3(0,asin(uwx<MoverParameters->DoorMaxShiftL?uwx/MoverParameters->DoorMaxShiftL:1)*57.3,0));
     if (smAnimatedWall[2])
       smAnimatedWall[2]->SetRotateXYZ(vector3(0,asin(uwx<MoverParameters->DoorMaxShiftL?uwx/MoverParameters->DoorMaxShiftL:1)*57.3,0));
    }
...
//        double scandist=300+random(MoverParameters->Vmax*5); //fabs(Mechanik->ProximityDist);
        double scandist=MoverParameters->Vmax+100+MoverParameters->Vel*MoverParameters->Vel/12.5;
...
//                    Mechanik->ProximityDist=fabs(scandist);
                   }
                  }
                }
              }
...
if ((MoverParameters->EnginePowerSource.SourceType==CurrentCollector)||(MoverParameters->TrainType=="ezt"))
{
    if (Global::bLiveTraction)
     ...
     if ((MoverParameters->PantFrontVolt) || (MoverParameters->PantRearVolt) ||
      ...
      tmpTraction.TractionVoltage=3350;
     }
    else
    tmpTraction.TractionVoltage=3300;
...
tmpTraction.TractionVoltage=3250;
     tmpTraction.TractionFreq=0;
     tmpTraction.TractionMaxCurrent=7500;
     tmpTraction.TractionResistivity=0.1;   //0.01
...
////            if((Controller!=Humandriver)&&(MoverParameters->BrakeCtrlPosNo>0)) VEP();
           }
    }
//    else
...
if ((tmpTraction.TractionVoltage==0)&&(MoverParameters->EngineType!=DieselElectric))
    {
...
//yB: ladunek
...

 {
  ...
  double dl;
//  dt=0.1;
  dl=(MoverParameters->Vel*dt1*0.28);

  if(MoverParameters->Load<1) {MoverParameters->Load=0;MoverParameters->LoadType=NULL;mdLoad=NULL;}
  mtr=Axle2.GetTrack();
  if (MoverParameters->V>0)
  {
   if (mtr->Event2!=NULL)
    if (mtr->Event2->Type==tp_RideUnload)
     {
      if(uwA!=mtr->Event2->Params[1].asdouble) uwLDact=0;
      uwA = mtr->Event2->Params[1].asdouble;
      uwL = mtr->Event2->Params[2].asdouble;
      if(uwLD<uwLDact)
        Duwx= uwA/uwL*dl;
      else
        uwLDact+= dl;
      if (uwx>0.5)
       {
        MoverParameters->Load-=1;
       }
       uwx+=Duwx;
//       uwx=10*dl;

      if(uwx<mtr->Event2->Params[3].asdouble)
        uwx=mtr->Event2->Params[3].asdouble;
      if(uwx>mtr->Event2->Params[4].asdouble)
        uwx=mtr->Event2->Params[4].asdouble;

     }
    else if (mtr->Event2->Type==tp_Load)
     {
     if((MoverParameters->LoadType==mtr->Event2->Params[1].asText)||(MoverParameters->LoadType==NULL))
       switch(mtr->Event2->Params[2].asInt)
        {
         case  1:
                  MoverParameters->Load= Max0R(MoverParameters->Load,mtr->Event2->Params[4].asInt);
                  MoverParameters->LoadType=mtr->Event2->Params[1].asText;
         break;
         case  0:
                  MoverParameters->Load= mtr->Event2->Params[4].asInt;
                  MoverParameters->LoadType=mtr->Event2->Params[1].asText;
         break;
         case -1:
                  MoverParameters->Load= Min0R(MoverParameters->Load,mtr->Event2->Params[4].asInt);
         break;
        }
     }
    else uwA=uwL=0;
  ...
  {
   if (mtr->Event1!=NULL)
    if (mtr->Event1->Type==tp_RideUnload)
     {
      if(uwA!=mtr->Event1->Params[1].asdouble) uwLDact=0;
      uwA = mtr->Event1->Params[1].asdouble;
      uwL = mtr->Event1->Params[2].asdouble;
      if(uwLD<uwLDact)
        Duwx= uwA/uwL*dl;
      else
        uwLDact+= dl;
...
//      uwx=10*dl;
      ...
      if(uwx<mtr->Event1->Params[3].asdouble)
        uwx=mtr->Event1->Params[3].asdouble;
      if(uwx>mtr->Event1->Params[4].asdouble)
        uwx=mtr->Event1->Params[4].asdouble;
     }
    else if (mtr->Event1->Type==tp_Load)
     {
     if((MoverParameters->LoadType==mtr->Event2->Params[1].asText)||(MoverParameters->LoadType==NULL))
       switch(mtr->Event1->Params[2].asInt)
        {
         case  1:
                  MoverParameters->Load= Max0R(MoverParameters->Load,mtr->Event1->Params[4].asInt);
                  MoverParameters->LoadType=mtr->Event1->Params[1].asText;
         break;
         case  0:
                  MoverParameters->Load= mtr->Event1->Params[4].asInt;
                  MoverParameters->LoadType=mtr->Event1->Params[1].asText;
         break;
         case -1:
                  MoverParameters->Load= Min0R(MoverParameters->Load,mtr->Event1->Params[4].asInt);
         break;
        }
     }
    else uwA=uwL=0;
  ...
  {uwA=uwL=0;
   if (mtr->Event0!=NULL)
    if (mtr->Event0->Type==tp_Load)
     if((MoverParameters->LoadType==mtr->Event2->Params[1].asText)||(MoverParameters->LoadType==NULL))
     {
       switch(mtr->Event2->Params[2].asInt)
        {
         case  1:
                  MoverParameters->Load= Max0R(MoverParameters->Load,mtr->Event0->Params[4].asInt);
                  MoverParameters->LoadType= mtr->Event0->Params[1].asText;
         break;
         case  0:
                  MoverParameters->Load= mtr->Event0->Params[4].asInt;
                  MoverParameters->LoadType= mtr->Event0->Params[1].asText;
         break;
         case -1:
                  MoverParameters->Load= Min0R(MoverParameters->Load,mtr->Event0->Params[4].asInt);
         break;
        }
     }
  }
...
//            McZapkie-250302 - natezenie zalezne od obrotow i mocy
       ...
       if (enginevolume<0.01)
         rsSilnik.Stop();
       if ((MoverParameters->EngineType==ElectricSeriesMotor)||(MoverParameters->EngineType==DieselElectric) && rsWentylator.AM!=0)
        {
...
//       vol+=(50000/ts.R*ts.R);
     ...
     if (vol>0.001)
      {
       rscurve.Play(vol,DSBPLAY_LOOPING,MechInside,GetPosition());
      }
     else
       rscurve.Stop();
    ...
    if (rsRunningNoise.AM!=0)
     {
       if (MoverParameters->Vel>0)
        {
          freq= rsRunningNoise.FM*MoverParameters->Vel+rsRunningNoise.FA;
          vol= rsRunningNoise.AM*MoverParameters->Vel+rsRunningNoise.AA;
           switch (MyTrack->eEnvironment)
            {
               case e_tunnel:
                {
          ...
          if (fabs(MoverParameters->nrot)>0.01)
           vol*=1+MoverParameters->UnitBrakeForce/(1+MoverParameters->MaxBrakeForce);
          vol= vol*(20.0+MyTrack->iDamageFlag)/21;
//          vol= vol*(random(11)+30.0f)/40.0f;
          rsRunningNoise.AdjFreq(freq,0);
          rsRunningNoise.Play(vol, DSBPLAY_LOOPING, MechInside, GetPosition());
        }
       else
        rsRunningNoise.Stop();
     }
...
//    eng_vol_act=enginevolume;
    ...
    if((MoverParameters->EngineType==DieselElectric)&&(MoverParameters->Mains))
    {
      if(MoverParameters->PantRearUp)
        sOilPomp.TurnOn(MechInside,GetPosition());
      else
        sOilPomp.TurnOff(MechInside,GetPosition());
      sOilPomp.Update(MechInside,GetPosition());

      if(MoverParameters->diel_fuel)
        sFuelPomp.TurnOn(MechInside,GetPosition());
      else
        sFuelPomp.TurnOff(MechInside,GetPosition());
      sFuelPomp.Update(MechInside,GetPosition());
    }
...
//Winger 040304 - ladowanie przedsionkow dla EZT
    ...
    //McZapkie-050402: wyszukiwanie kol o nazwie str*
            ...
            if (str==AnsiString("animwallprefix:"))              //prefiks obracanych sie kol
             {
              str= Parser->GetNextSymbol();
              asAnimName="";
              for (int i=1; i<=5; i++)
               {
    //McZapkie-050402: wyszukiwanie scian o nazwie str*
                asAnimName=str+i;
                smAnimatedWall[i-1]= mdModel->GetFromName(asAnimName.c_str());
               }
              uwx=0;
             }
            else
    ...
    //NBMX wrzesien 2003: wyszukiwanie drzwi o nazwie str*
        ...
        if (str==AnsiString("sounds:"))                            //dzwieki
         while (!Parser->EndOfFile && str!=AnsiString("endsounds"))
         {
          ...
          if (str==AnsiString("outernoise:"))    //plik z dzwiekiem wentylatora, mnozniki i ofsety amp. i czest.
           {
            str= Parser->GetNextSymbol();
            rsRunningNoise.Init(str.c_str(),Parser->GetNextSymbol().ToDouble(),GetPosition().x,GetPosition().y,GetPosition().z);
            rsRunningNoise.AM=Parser->GetNextSymbol().ToDouble()/MoverParameters->Vmax;
            rsRunningNoise.AA=Parser->GetNextSymbol().ToDouble();
            rsRunningNoise.FM=Parser->GetNextSymbol().ToDouble()/MoverParameters->Vmax;
          ...
          if (str==AnsiString("fuelpomp:"))                      //pliki z pompa paliwa
           {
            sFuelPomp.Load(Parser,GetPosition());
           }
          if (str==AnsiString("oilpomp:"))                      //pliki z pompa oleju
           {
            sOilPomp.Load(Parser,GetPosition());
           }
         }
        else
