...
//        tmp->Init(nv);
    ...
    if ((tmp->iType==TP_EVLAUNCH)&&((tmp->EvLaunch->hh<25)||(tmp->EvLaunch->DeltaTime>0)))
    {
        tmp->Next= RootTEL;
        RootTEL= tmp;
    }
...
bool __fastcall TGround::InitLaunchers()
{
    ...
    for (Current= RootTEL; Current!=NULL; Current= Current->Next)
    {
           EventLauncher= Current->EvLaunch;
           if (EventLauncher->iCheckMask!=0)
            if (EventLauncher->asMemCellName!=AnsiString("none"))
...
//            if (tmp)
  //              tmp->Render();
    ...
    for (node= RootTEL; node!=NULL; node= node->Next)
    {
        node->Render();
    }
