...
{funkcje logiczne}
function TestFlag(Flag:byte; Value:byte):boolean;
function  SetFlag(var Flag:byte; Value:byte):boolean;

function  FuzzyLogic(Test,Threshold:real):boolean;
...
begin
  if Test>Threshold then
   FuzzyLogic:=(Random>Threshold/Test) {im wiekszy Test tym wieksza szansa}
...
begin
  s:='';
  nextword:=False;
  while (not eoln(infile)) and (not nextword) do
   begin
     read(infile,c);
     if c=' ' then
      if s<>'' then
       nextword:=True;
     if c in _FileName+['='] then
      s:=s+c;
   end;
  ReadWord:=s;
end;
...
procedure ComputeALine(X0,Y0,Xn,Yn,L,R:real; var Xout,Yout:real);
var dX,dY,pX,pY,gamma,alfa:real;
begin
  dX:=Xn-X0;
  dY:=Yn-Y0;
  if dX<>0 then gamma:=arctan(dY/dX)
