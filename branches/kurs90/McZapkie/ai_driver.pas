...
{                        VelDesired:=Min0R(VelDesired,VelReduced+VelNext); }
                      ...
                      if ActualProximityDist>MinProximityDist then
                       begin
                         ...
                         ReactionTime:=BrakeDelay[2+2*BrakeDelayFlag]/(1+abs(AccDesired)); {aby szybkosc hamowania zalezala od przyspieszenia i opoznienia hamulcow}
                       end
                      else
                ...
                if (VelDesired>=0) and (Vel>(VelDesired+2)) then
                ...
                if BrakeSystem=Pneumatic then  {napelnianie uderzeniowe}
                 if BrakeSubsystem=Oerlikon then
                  begin
                    if BrakeCtrlPos<0 then
                     IncBrakeLevel;
                    if (BrakeCtrlPos=0) and (AccDesired>0) then
                     if FuzzyLogicAI(HighPipePress-PipePress,0.01,1) then
                      DecBrakeLevel
                     else
       ...
       {          if not MinVelFlag then   }
                   begin
                      if not DecSpeed then
                       if (AccDesired<-0.1+AbsAccS) or ((Vel-VelMargin)>(VelMargin+VelDesired)) then
