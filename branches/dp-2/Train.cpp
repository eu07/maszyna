...
double went1rottime = 0;
double went2rottime = 0;
...
//        if (!GetAsyncKeyState(VK_SHIFT)<0)         // bez shifta
         {
          ...
          if (cKey==Global::Keys[k_MechUp])
              pMechOffset.y+=0.05; //McZapkie-120302 - wstawanie
          else
          if (cKey==Global::Keys[k_MechDown])
              pMechOffset.y-=0.05; //McZapkie-120302 - siadanie
         }
       }
...
// ZEGAREK NORMALNY
    ...
    if (ClockGaugeMin_q.SubModel!=NULL)
     {
      ClockGaugeMin_q.UpdateValue(GlobalTime->mm);
      ClockGaugeMin_q.Update();
      ClockGaugeHou_q.UpdateValue(GlobalTime->hh+GlobalTime->mm/60.0);
      ClockGaugeHou_q.Update();
      ClockGaugeSec_q.UpdateValue(int(GlobalTime->mr));
      ClockGaugeSec_q.Update();
     }


// WENTYLATOR 1

    if (Ventilator1_q.SubModel!=NULL)
     {
     if (Global::ventilator1on == true)
         {
          //double dtx = Timer::GetDeltaTime();

          if (Global::ventilator1acc < 25) Global::ventilator1acc += 0.1;
          went1rottime += Global::ventilator1acc; //dtx * 354;
          Ventilator1_q.Update();
          Ventilator1_q.SubModel->SetRotate(float3(0,0,1), went1rottime);
          if (went1rottime > 360) went1rottime = 0;
         }
     if (Global::ventilator1on == false)
         {
          //double dtx = Timer::GetDeltaTime();

          if (Global::ventilator1acc > 0) Global::ventilator1acc -= 0.05;
          went1rottime += Global::ventilator1acc; //dtx * 354;
          Ventilator1_q.Update();
          Ventilator1_q.SubModel->SetRotate(float3(0,0,1), went1rottime);
          if (went1rottime > 360) went1rottime = 0;
...
// WENTYLATOR 2
    ...
    if (Ventilator2_q.SubModel!=NULL)
     {
     if (Global::ventilator2on == true)
         {
          //double dtx = Timer::GetDeltaTime();

          if (Global::ventilator2acc < 25) Global::ventilator2acc += 0.1;
          went2rottime += Global::ventilator2acc; //dtx * 354;
          Ventilator2_q.Update();
          Ventilator2_q.SubModel->SetRotate(float3(0,0,1),went2rottime);
          if (went2rottime > 360) went2rottime = 0;
         }
     if (Global::ventilator2on == false)
         {
          //double dtx = Timer::GetDeltaTime();

          if (Global::ventilator2acc > 0) Global::ventilator2acc -= 0.05;
          went2rottime += Global::ventilator2acc; //dtx * 354;
          Ventilator2_q.Update();
          Ventilator2_q.SubModel->SetRotate(float3(0,0,1),went2rottime);
          if (went2rottime > 360) went2rottime = 0;
...
//          btLampkaSHP.TurnOff();
    ...
    if ( Pressed(Global::Keys[k_Horn]) )
     {
      if (Global::bCONSOLE) return true;
      if (Pressed(VK_SHIFT))
         {
...
//    Parser->LoadStringToParse(asFile);
    ...
    // TUTAJ NIEZALEZNE OD WPISOW W MMD
    if (DynamicObject->mdClock1) ClockGaugeSec_q.Init(DynamicObject->mdClock1->GetFromName("clocks"),gt_Rotate,0.01667,0,0);
    if (DynamicObject->mdClock1) ClockGaugeMin_q.Init(DynamicObject->mdClock1->GetFromName("clockm"),gt_Rotate,0.01667,0,0);
    if (DynamicObject->mdClock1) ClockGaugeHou_q.Init(DynamicObject->mdClock1->GetFromName("clockh"),gt_Rotate,0.08333,0,0);

    if (DynamicObject->mdVentilator1) Ventilator1_q.Init(DynamicObject->mdVentilator1->GetFromName("cooler"),gt_Rotate,0.01667,0,0);  //QUEUEDEU: ventylator 1
    if (DynamicObject->mdVentilator2) Ventilator2_q.Init(DynamicObject->mdVentilator2->GetFromName("cooler"),gt_Rotate,0.01667,0,0);  //QUEUEDEU: ventylator 1
...
/*
   else if (DynamicObject->mdClock1 && str==AnsiString("qclocktimer:"))                                    //QUEUEDEU: zegarek analogowy
         {
          if (Parser->GetNextSymbol()==AnsiString("qanalog"))
           {
            ClockGaugeSec_q.Init(DynamicObject->mdClock1->GetFromName("clocks"),gt_Rotate,0.01667,0,0);
            ClockGaugeMin_q.Init(DynamicObject->mdClock1->GetFromName("clockm"),gt_Rotate,0.01667,0,0);
            ClockGaugeHou_q.Init(DynamicObject->mdClock1->GetFromName("clockh"),gt_Rotate,0.08333,0,0);
           }
         }
   else if (DynamicObject->mdVentilator1 && str==AnsiString("qventilato1:"))
         {
          if (Parser->GetNextSymbol()==AnsiString("dirr"))
              Ventilator1_q.Init(DynamicObject->mdVentilator1->GetFromName("cooler"),gt_Rotate,0.01667,0,0);}  //QUEUEDEU: ventylator 1

   else if (DynamicObject->mdVentilator2 && str==AnsiString("qventilato2:"))
         {
          if (Parser->GetNextSymbol()==AnsiString("dirr"))
              Ventilator2_q.Init(DynamicObject->mdVentilator2->GetFromName("cooler"),gt_Rotate,0.01667,0,0);}  //QUEUEDEU: ventylator 2

   else if (DynamicObject->mdFotel1 && str==AnsiString("qfotel-a-l:"))
         {
          if (Parser->GetNextSymbol()==AnsiString("dirr"))
               fotelAL.Init(DynamicObject->mdFotel1->GetFromName("fotel_rot"),gt_Rotate,0.01667,0,0);}  //QUEUEDEU: fotel w kabibinie A lewy

   else if (DynamicObject->mdFotel2 && str==AnsiString("qfotel-a-r:"))
         {
          if (Parser->GetNextSymbol()==AnsiString("dirr"))
            fotelAR.Init(DynamicObject->mdFotel2->GetFromName("fotel_rot"),gt_Rotate,0.01667,0,0); }  //QUEUEDEU: fotel w kabibinie A prawy
 */
