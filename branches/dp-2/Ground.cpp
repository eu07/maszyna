...
{//renderowanie kwadratu kilometrowego (DL), jeœli jeszcze nie zrobione
 if (iLastDisplay!=iFrameNumber)
 {
  ...
  {//³¹czenie trójk¹tów w jedn¹ listê - trochê wioska
   ...
   nRender->Render(); //nieprzezroczyste trójk¹ty kwadratu kilometrowego
  }
...
{//przyczepienie obiektu do sektora, wstêpna kwalifikacja na listy renderowania
 ...
 {case TP_SOUND: //te obiekty s¹ sprawdzanie niezale¿nie od kierunku patrzenia
  ...
  case TP_TRACK: //TODO: tory z cieniem (tunel, canyon) te¿ daæ bez ³¹czenia?
   ...
   Global::asCurrentNodeName = Node->asName;
...
{//renderowanie nieprzezroczystych (DL)
 ...
 for (node=nRender;node;node=node->nNext3)
  node->Render(); //nieprzezroczyste obiekty (oprócz pojazdów)
...
void __fastcall TSubRect::Render2()
{//renderowanie nieprzezroczystych

 TGroundNode *node;
 RaAnimate(); //przeliczenia animacji torów w sektorze
 for (node=nRender;node;node=node->nNext3)
  node->Render2(); //nieprzezroczyste obiekty (oprócz pojazdów)                 // call to void __fastcall TGroundNode::Render2()
 for (node=nRenderMixed;node;node=node->nNext3)
  node->Render2(); //nieprzezroczyste z mieszanych modeli                       // call to void __fastcall TGroundNode::Render2()
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// POTRZEBNE DO  RENDEROWANIA SWIATEL SEMAFOROW COBY KOLEJNOSC BYLA OK ^^^^^^^^^
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

void __fastcall TGroundNode::Render2()     // CALL
{
 if (Global::bsemlenses)
    {
    //double mgn= SquareMagnitude(pCenter-Global::pCameraPosition);
    double mgn= SquareMagnitude(pCenter-Global::pCameraPosition);
   ...
   //     return;
    ...
    switch (iType)
    {
     //case TP_MODEL: return Model->Render2(&pCenter);                             // FROM ANIMMODEL.CPP   bool __fastcall TAnimModel::Render2(vector3 pPosition, double fAngle, AnsiString nodename)
     //break;
    }
  }
}
...
//i jezeli tak to odpowiedni GL_GREATER w przeciwnym wypadku standardowy 0.04
 ...
 {
     Global::problend = true;
     if ((Global::problend) ) // odpalac? no ?no ;)    //Q: 13122011
          {
               glDisable(GL_BLEND);
               glEnable(GL_ALPHA_TEST);
               glAlphaFunc(GL_GREATER,0.72);     // CZEMU 0.64? dla testu, poki co wartosc jest wzieta z dupy a mimo to jest dobra , ze niby im wieksza to mniejsza ramka dookola? cos w tym stylu, wieksza wartosc bardziej zzera alphe metoda zerojedynkowa , no tez wczoraj robilem testy i wydawalo sie okej pozniej cos popsulem i juz sie nie udalo powtorzyc efektu jesli chodzi o wartosci, to mozna je testowac w programie glsandbox, masz opengl live i edytujesz wartosci hmhm, ok brnijmy w to dalej
               glDepthFunc(GL_LEQUAL);
          }
         else
          {
...
TGroundNode* __fastcall TGround::FindDynamicQ( AnsiString asNameToFind )
    {
  ...
  GetWindowText(hwnd, buffer, textlen+1);
...
{//wczytanie wpisu typu "node"
 ...
 AnsiString g___APPDIR;
 AnsiString tracktofind;
 AnsiString prevnode;
 ...
 Global::rtim = (rtim) / 1000 ;

 Global::gtc2 = GetTickCount();
 Global::lsec = Global::gtc2 - Global::gtc1;

 //if (dorefresh)
 if (Global::rtim > 4.9)    { gtc1 =  GetTickCount(); rtim = 0; Global::rtim = 0; W->RenderLoader(77, AnsiString(Global::lsec/1000 ).c_str());}
 //if (dorefresh) { gtc1 =  GetTickCount(); rtim = 0; Global::rtim = 0; W->RenderLoader(77, AnsiString(Global::lsec/1000 ).c_str());}
/*
HWND okno = GetForegroundWindow();
GetWindowString(okno, title);
caption = title.c_str();
if (caption != Global::asWINTITLE) {dorefresh = true; xx = true;}// WriteLog(title.c_str());    // JEZELI INNE OKNO AKTYWNE
else
{
...
if (caption == Global::asWINTITLE)
{
 if (xx) SetForegroundWindow(Global::hWnd);
 if (xx) W->RenderLoader(77, AnsiString(Global::lsec/1000 ).c_str());
 xx = false;
}
 */

 //ADDFILE = new TStringList;
 //g___APPDIR = ExtractFilePath(ParamStr(0));

 int bytespassed = parser->bytespassed;
 int bytestotal = parser->bytestotal;
 Global::iPARSERBYTESPASSED = bytespassed;
 Global::iPARSERBYTESTOTAL = bytestotal;


 parser->getTokens(2);           // DISTANCE, PARAM 1 i 2
 *parser >> r >> rmin;

 parser->getTokens();            // NODENAME, PARAM 4
 *parser >> token;
 asNodeName=AnsiString(token.c_str());

 Global::asCurrentNodeName = asNodeName;
...
if (str=="dynamic")
    {
                WriteLog("   ");
                WriteLog("-----------------------");
                //W->RenderLoader(77, "WCZYTYWANIE POJAZDU: " + tmp->asName);
...
//            tmp->iState=(Parser->GetNextSymbol().LowerCase()=="loop"?DSBPLAY_LOOPING:0);
  ...
  case TP_DYNAMIC :
                ...
                WriteLog(AnsiString(asTrainName.c_str()) + ", " + tmp->asName.c_str());                                  // TRAINNAME, NAME

                directtable = asTrainName.c_str();
             //   WriteLog(tmp->asName.c_str());                                  // NAME
                parser->getTokens();
                *parser >> token;
                str1 = AnsiString(token.c_str());                               //McZapkie-131102: model w .mmd    // PATH
                ...
                *parser >> token;
                ...
                parser->getTokens();
                *parser >> token;
                str3=AnsiString(token.c_str());                                 // CHK
                if (bTrainSet)
                {//jeœli pojazd jest umieszczony w sk³adzie
                str=asTrainSetTrack;                                            // TRACK
                parser->getTokens();
                ...
                *parser >> token;
                ...
                parser->getTokens();
                *parser >> tf1;                                                 // DISTANCE //Ra: -1 oznacza odwrotne wstawienie
                ...
                *parser >> token;
                ...
                {//jeœli tor znaleziony
                 ...
                 DYNOBJ->directtabletoken = directtable;


                 W->RenderLoader(77, "INICJALIZACJA POJAZDU: " + AnsiString(asNodeName));
                 ...
                 // WCZYTYWANIE PLIKU .ADD DLA POJAZDU  ^^^^^^^^^^^^^^^^^^^^^^^^^


                 W->RenderLoader(77, "PRZETWARZANIE PLIKU .ADD POJAZDU");

                 AnsiString ADDFILENAME = "0";

                 READADDFILE(ADDFILENAME, DYNOBJ);
  ...
  case TP_MODEL :
   //lrb++;
   //Global::loaderrefresh = 150;
   //Global::postep = (Global::iPARSERBYTESPASSED * 100 / Global::iNODES ); // PROCENT

   //if (lrb > Global::loaderrefresh) W->RenderLoader(77, "WCZYTYWANIE MODELU: " + tmp->asName);
   //if (lrb > Global::loaderrefresh) lrb = 0;

   //if ((tmp->asName != "")) WriteLog(AnsiString(" "));
   //if ((tmp->asName != "")) WriteLog(AnsiString("NODE: ") + asNodeName);
   Global::asCurrentNodeName = tmp->asName;
   parser->getTokens(3);
   *parser >> tmp->pCenter.x >> tmp->pCenter.y >> tmp->pCenter.z;
   parser->getTokens();
   *parser >> tf1;
...
//   str=Parser->GetNextSymbol().LowerCase();
   ...
   *parser >> token;
   ...
   // PROBLEND Q: 13122011
           PROBLEND = false;     // DEFAULTOWO WYLACZONY PROBLEND //a nie robisz else problend = false;? na jedno wyjdzie mhm ide sie wylac bo nie moge na to paptzrec masz jakies zaburzenia prostaty, lejesz co parenascie minut
           tmp->PROBLEND = false;  // co to daje? odwolanie do tgroundnode, bo renderowanie jest w tej klasie
           if (str.Pos("#") > 0)     //       SPRAWDZANIE CZY WCIAGU ZNAJDUJE SIE ZNAK @  , WLASCIWIE TO SPRAWDZA POZYCJE DANEGO ZNAKU W STRINGU
               {
                PROBLEND = true;     // WLACZAMY PROBLEND DLA TEGO TRIANGLE
                tmp->PROBLEND = true;
               }
...
{//ustalanie zale¿noœci na scenerii przed wczytaniem pojazdów
 ...
 bInitDone=true;
 W->RenderLoader(77, AnsiString("Normals init..." ).c_str());
 ...
 WriteLog("InitEvents");
 W->RenderLoader(77, AnsiString("Init Events..." ).c_str());
 InitEvents();
 ...
 W->RenderLoader(77, AnsiString("Init global time..." ).c_str());
...
{//g³ówne wczytywanie scenerii
 ...
 //pTrain=NULL;
    ...
    //W->RenderLoader(77, AnsiString(" " ).c_str());
    ...
    //PlaySound("data\\sounds\\menu.WAV", NULL, SND_ASYNC);

    while (token!="") //(!Parser->EndOfFile)
    {

     //Global::iPARSERBYTESPASSED = parser.bytespassed;
     //Global::bignoreinclude = false;
        ...
        if (str==AnsiString("node"))
        {
         ...
         {
          Error("Scene parse error near "+AnsiString(token.c_str()) + " " + LastNode->asName);
          break;
         }
        }
        else
    ...
    //            break;
	 ...
	 glLightfv(GL_LIGHT0,GL_DIFFUSE,Global::diffuseDayLight);  //kolor padaj¹cy
  	 ...
  	 glLightfv(GL_LIGHT0,GL_SPECULAR,Global::specularDayLight); //kolor odbity
        ...
        {//youBy - niebo z pliku
         W->RenderLoader(77, AnsiString("Scenery sky definitions..." ).c_str());
        ...
        if (str==AnsiString("skyh"))
        {
            ...
            AnsiString SkyTemp;
            SkyTemp = AnsiString(token.c_str());
            //if (Global::asSkyL=="1")          // Q: ZAHASZOWALEM COBY ZAWSZE BRALO
              Global::asSkyH=SkyTemp;
            do
             {//po¿arcie dodatkowych parametrów
               parser.getTokens(); parser >> token;
             } while (token.compare("endsky") != 0);
             WriteLog(Global::asSkyH.c_str());
        }
...
//blablabla
{
   ...
   {
    Global::bnearestengaged = false;                                            // QUEUEDEU - 010407  RESETOWANIE INFO O NAJBLIZSZYM POJEZDZIE !!
    Global::bnearestobjengaged = false;
    Global::asnearestobj = "x0";                                                // RESET NODENAME COBY PO ODDALENIU SIE OD OBJEKTU NIE BYLO MOZLIWOSCI STEROWANIA NIM
    Global::asnearestdyn = "x0";
...
*/
 ...
 for (i=0;i<iRendered;i++)
  pRendered[i]->Render(); //renderowanie nieprzezroczystych
 return true;
}
...
bool __fastcall TGround::Render2(vector3 pPosition)
{

    //glColor3f(1.0f,1.0f,1.0f);
    register int tr,tc;
    register TGroundNode *node,*oldnode;
 int n=2*iNumSubRects; //(2*==2km) promieñ wyœwietlanej mapy w scektorach
 int c=GetColFromX(pPosition.x);
 int r=GetRowFromZ(pPosition.z);
    register TSubRect *tmp,*tmp2;
...
*/
 ...
 for (i=0;i<iRendered;i++)
  pRendered[i]->Render2(); //renderowanie nieprzezroczystych
 return true;
}
