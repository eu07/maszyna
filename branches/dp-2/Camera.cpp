...
//gora-dol
        if (Pressed(VK_CONTROL) && Pressed(VK_NUMPAD9)) Pos.y+=0.05;
        if (Pressed(VK_CONTROL) && Pressed(VK_NUMPAD3)) Pos.y-=0.05;
...
bool __fastcall TCamera::SetMatrix()
{
    ...
    if (Type==tp_Follow){ gluLookAt(Pos.x,Pos.y,Pos.z,LookAt.x,LookAt.y,LookAt.z,vUp.x,vUp.y,vUp.z);  } //Ra: pOffset is zero

    if (Type==tp_Satelite) Pitch= M_PI*0.5;

    if (Type!=tp_Follow) glTranslated(-Pos.x,-Pos.y,-Pos.z); //nie zmienia kierunku patrzenia
