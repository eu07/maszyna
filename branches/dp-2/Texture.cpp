...
TTexturesManager::Names::iterator TTexturesManager::LoadFromFile(std::string fileName,int filter)
{
    ...
    char buf[255];
    AnsiString BF = "TEXTURE: " ;             // + Global::g___APPDIR
    AnsiString assuf1 = "...ok";
    AnsiString assuf2 = "...be";
    //int x;
...
//  WriteLog(cFileName);
    ...
    AlphaValue texinfo;
    BF += AnsiString(realFileName.c_str());                 //1512
    //BF += AnsiString(cFileName);
         ...
         if (hash) // #: calkowity brak filtracji - pikseloza
          {
           ...
           //if (Global::problend)
           glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER, GL_LINEAR);     // WYMUSZONE ROZMYWANIE   131211
           glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER, GL_LINEAR);     // WYMUSZONE ROZMYWANIE   131211
          }
         else
          {
           glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
{//Ra: u¿ywane tylko dla TGA i TEX
 ...
 glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, maxaniso);
   ...
   // glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);                 // Q
   // glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);   // Q
