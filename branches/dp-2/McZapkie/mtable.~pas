...
var
xss, xh, xm, xs : integer;    //dodanie czasu (deltaT) w sekundach, z przeliczeniem godziny
...
begin
  ...
  while hh>23 do //przeliczenie godzin do w³aœciwego przedzia³u
   ...
   then
   begin
   if (hh < 10) then shh := '0' + IntToStr(hh) else shh := IntToStr(hh);
   if (mm < 10) then smm := '0' + IntToStr(mm) else smm := IntToStr(mm);
   if (ss < 10) then sss := '0' + IntToStr(ss) else sss := IntToStr(ss);
   h1 := StrToInt(copy(shh,1,1));
   h2 := StrToInt(copy(shh,2,1));
   m1 := StrToInt(copy(smm,1,1));
   m2 := StrToInt(copy(smm,2,1));
   s1 := StrToInt(copy(sss,1,1));
   s2 := StrToInt(copy(sss,2,1));
   bysec := 0;
   end;
   bysec:=bysec+ deltaT;
