 ...
 if (iFlags&2) //jeœli tor
 {
  ...
  {fVelNext=trTrack->VelocityGet(); //aktualizacja prêdkoœci (mo¿e byæ zmieniana eventem)
   ...
   {//jeœli skrzy¿owanie, ograniczyæ prêdkoœæ przy skrêcaniu
    if (abs(i)>0x10000000) //±1 to jazda na wprost, ±2 nieby te¿, ale z przeciêciem g³ównej drogi - chyba ¿e jest równorzêdne...
     fVelNext=30.0; //uzale¿niæ prêdkoœæ od promienia; albo niech bêdzie ograniczona w skrzy¿owaniu (velocity z ujemn¹ wartoœci¹)
    if ((iFlags&32)==0) //jeœli nie wjecha³
     if (trTrack->iNumDynamics>0) //a skrzy¿owanie zawiera pojazd
      fVelNext=0.0; //to zabroniæ wjazdu (chyba ¿e ten z przodu te¿ jedzie prosto)
   }
...
{//ustalenie parametrów, zwraca typ komendy, jeœli sygna³ podaje prêdkoœæ do jazdy
 ...
 iDrivigFlags&=~(moveTrackEnd|moveSwitchFound); //te flagi s¹ ustawiane tutaj, w razie potrzeby
...
//       iStationStart=TrainParams->StationIndex; //zaktualizowaæ wyœwietlanie rozk³adu
     ...
     {//w trybie manewrowym: skanowaæ od niego wstecz, stan¹æ po wyjechaniu za sygnalizator i zmieniæ kierunek
    ...
    {//zawalidrogi nie ma (albo pojazd jest samochodem), sprawdziæ sygna³
     ...
     {//jeœli podana prêdkoœæ manewrowa
      ...
      {//jeœli tryb poci¹gowy a tarcze ma ShuntVelocity 0 0
       ...
       if (go<=cm_Ready) //jeœli jeszcze nie ma komendy
        if (v!=0.0) //komenda jest tylko gdy ma jechaæ, bo stoi na podstawie tabelki
        {//jeœli nie by³o komendy wczeœniej - pierwsza siê liczy - ustawianie VelSignal
         go=cm_ShuntVelocity; //w trybie poci¹gowym tylko jeœli w³¹cza tryb manewrowy (v!=0.0)
         //Ra 2014-06: (VelSignal) nie mo¿e byæ tu ustawiane, bo Tm mo¿e byæ daleko
         //VelSignal=v; //nie do koñca tak, to jest druga prêdkoœæ
         if (VelSignal==0.0) VelSignal=v; //aby stoj¹cy ruszy³
      ...
      if (go<=cm_Ready) //jeœli nie by³o komendy wczeœniej - pierwsza siê liczy - ustawianie VelSignal
       if (v<0.0?true:v>=1.0) //bo wartoœæ 0.1 s³u¿y do hamowania tylko
       {go=cm_SetVelocity; //mo¿e odjechaæ
        //Ra 2014-06: (VelSignal) nie mo¿e byæ tu ustawiane, bo semafor mo¿e byæ daleko
        //VelSignal=v; //nie do koñca tak, to jest druga prêdkoœæ; -1 nie wpisywaæ...
        if (VelSignal==0.0) VelSignal=v; //aby stoj¹cy ruszy³
        if (sSpeedTable[i].fDist<0.0) //jeœli przejechany
        {VelSignal=v; //!!! ustawienie, gdy przejechany jest lepsze ni¿ wcale, ale to jeszcze nie to
       ...
       {//jeœli prêdkoœæ jest zerowa, a komórka zawiera komendê
        ...
        if (iDrivigFlags&moveStopHere) //jeœli ma staæ, dostaje komendê od razu
         go=cm_Command; //komenda z komórki, do wykonania po zatrzymaniu
        else if (sSpeedTable[i].fDist<=20.0) //jeœli ma doci¹gn¹æ, to niech doci¹ga (moveStopCloser dotyczy doci¹gania do W4, nie semafora)
         go=cm_Command; //komenda z komórki, do wykonania po zatrzymaniu
       }
...
{//zamiana kodu rozkazu na opis
 if (Order&Change_direction)     return "Change_direction"; //mo¿e byæ na³o¿ona na inn¹ i wtedy ma priorytet
...
//   mvOccupied->BrakeLevelSet((mvOccupied->BrakeHandle==FVel6)?4:-2); //odciêcie na zaworze maszynisty, FVel6 po drugiej stronie nie luzuje
  ...
  {//jeœli zmieniony zosta³ pojazd prowadzony
   ...
   ControllingSet(); //utworzenie po³¹czenia do sterowanego pojazdu (mo¿e siê zmieniæ) - silnikowy dla EZT
  }
  if (mvControlling->EngineType==DieselEngine) //dla 2Ls150 - przed ustawieniem kierunku - mo¿na zmieniæ tryb pracy
   if (mvControlling->ShuntModeAllow)
    mvControlling->CurrentSwitch((OrderList[OrderPos]&Shunt)||(fMass>224000.0)); //do tego na wzniesieniu mo¿e nie daæ rady na liniowym
  //Ra: to prze³¹czanie poni¿ej jest tu bez sensu
  mvOccupied->ActiveCab=iDirection; //aktywacja kabiny w prowadzonym poje¿dzie (silnikowy mo¿e byæ odwrotnie?)
  ...
  if (p->Mechanik) //jeœli ma obsadê
   if (p->Mechanik!=this) //ale chodzi o inny pojazd, ni¿ aktualnie sprawdzaj¹cy
    if (p->Mechanik->iDrivigFlags&movePrimary) //a tamten ma priorytet
     if ((iDrivigFlags&movePrimary)&&(mvOccupied->DirAbsolute)&&(mvOccupied->BrakeCtrlPos>=-1)) //jeœli rz¹dzi i ma kierunek
    ...
    {//wyliczanie optymalnego przyspieszenia do jazdy na widocznoœæ (Ra: na pewno tutaj?)
     ...
     {if (pVehicles[0]->fTrackBlock<fMaxProximityDist) //porównianie z minimaln¹ odleg³oœci¹ kolizyjn¹
      ...
      {//jeœli tamten jedzie szybciej, to nie potrzeba modyfikowaæ przyspieszenia
       double d=(pVehicles[0]->fTrackBlock-0.5*fabs(mvOccupied->V)-fMaxProximityDist); //bezpieczna odleg³oœæ za poprzednim
       ...
       if (d>0) //bo jak ujemne, to zacznie przyspieszaæ, aby siê zderzyæ
        k=(k*k-mvOccupied->Vel*mvOccupied->Vel)/(25.92*d); //energia kinetyczna dzielona przez masê i drogê daje przyspieszenie
       else
  ...
  if (!iDirection) //jeœli nie ma ustalonego kierunku
   ...
   {//ustalenie kierunku, gdy stoi
    ...
    if (!iDirection) //jeœli nie ma ustalonego kierunku
     ...
     {if (mvOccupied->Couplers[1].CouplingFlag==ctrain_virtual) //jeœli z ty³u nie ma nic
       iDirection=-1; //jazda w kierunku sprzêgu 1
      if (mvOccupied->Couplers[0].CouplingFlag==ctrain_virtual) //jeœli z przodu nie ma nic
...
{//zwiêkszenie prêdkoœci; zwraca false, jeœli dalej siê nie da zwiêkszaæ
 ...
 switch (mvOccupied->EngineType)
 {
       ...
       {//przekroczone 30km/h, mo¿na wejœæ na jazdê równoleg³¹
        if (mvControlling->ScndCtrlPos) //jeœli ustawiony bocznik
         if (mvControlling->MainCtrlPos<mvControlling->MainCtrlPosNo-1) //a nie jest ostatnia pozycja
          mvControlling->DecScndCtrl(2); //to bocznik na zero po chamsku (ktoœ mia³ to poprawiæ...)
  ...
  case DieselEngine:
   if (mvControlling->ShuntModeAllow)
   {//dla 2Ls150 mo¿na zmieniæ tryb pracy, jeœli jest w liniowym i nie daje rady (wymaga zerowania kierunku)
...
//    else
    ...
    if (Ready||(iDrivigFlags&movePress)) //o ile mo¿e jechaæ
     ...
     {//procedura wje¿d¿ania na ekstremalne wzniesienia
      if (fabs(mvControlling->Im)>0.85*mvControlling->Imax) //a pr¹d jest wiêkszy ni¿ 85% nadmiarowego
       ...
       {//w³¹czenie wysokiego rozruchu; (I*U)[A*V=W=kg*m*m/sss]/(m[kg]*a[m/ss])=v[m/s]; 2.8m/ss=10km/h
        ...
        if (mvControlling->Imax<mvControlling->ImaxHi) //jeœli da siê na wysokim
         mvControlling->CurrentSwitch(true); //rozruch wysoki (za to mo¿e siê œlizgaæ)
        if (ReactionTime>0.1)
      ...
      if ((fabs(mvControlling->Im)>0.96*mvControlling->Imax)||mvControlling->SlippingWheels) //jeœli pr¹d jest du¿y (mo¿na 690 na 750)
       if (mvControlling->ScndCtrlPos>0) //je¿eli jest bocznik
...
*/
  ...
  // if (mvOccupied->CabNo*mvOccupied->V*NewValue1<0) //jeœli zadana prêdkoœæ niezgodna z aktualnym kierunkiem jazdy
  ...
  {iCoupler=floor(fabs(NewValue2)); //jakim sprzêgiem
   ...
   {//bêdzie odczepianie, ale jeœli wagony s¹ z przodu, to trzeba najpierw zmieniæ kierunek
    ...
    {iDirectionOrder=-iDirection; //zmiana na ci¹gniêcie
     ...
     if (mvOccupied->Couplers[mvOccupied->DirAbsolute>0?1:0].CouplingFlag>0) //z ty³u coœ
...
//   mvOccupied->PipePress=0.5; //yB: w SPKS s¹ poprawnie zrobione pozycje
 ...
 if ((LastReactionTime>Min0R(ReactionTime,2.0)))
 {
  ...
  //dla hamowania -0.2 [m/ss] droga wynosi 0.389*Vel*Vel [km/h], czyli 600m dla 40km/h, 3.8km dla 100km/h i 9.8km dla 160km/h
  //dla hamowania -0.4 [m/ss] droga wynosi 0.096*Vel*Vel [km/h], czyli 150m dla 40km/h, 1.0km dla 100km/h i 2.5km dla 160km/h
  ...
  if (fMass>1000000.0) fBrakeDist*=2.0; //korekta dla ciê¿kich, bo prze¿ynaj¹ - da to coœ?
    ...
    {if (iVehicleCount>=0) //jeœli by³a podana iloœæ wagonów
     {
      ...
      {//3. faza odczepiania.
       ...
       if ((mvControlling->MainCtrlPos>0)||(mvOccupied->BrakeSystem==ElectroPneumatic)) //jeœli jazda
       {
      ...
      {//2. faza odczepiania: zmieñ kierunek na przeciwny i dociœnij
       ...
       double p=mvOccupied->BrakePressureActual.PipePressureVal; //tu mo¿e byæ 0 albo -1 nawet
       ...
       {//jeœli w miarê zosta³ zahamowany (ciœnienie mniejsze ni¿ podane na pozycji 3, zwyle 0.37)
        if (mvOccupied->BrakeSystem==ElectroPneumatic)
         mvOccupied->BrakeLevelSet(0); //wy³¹czenie EP, gdy wystarczy (mo¿e nie byæ potrzebne, bo na pocz¹tku jest)
      ...
      {//4. faza odczepiania: zwolnij i zmieñ kierunek
       ...
       {//ponowna zmiana kierunku
        ...
        DirectionForward(mvOccupied->ActiveDir<0); //zmiana kierunku jazdy na w³aœciwy
...
/*
     ...
     {//jeœli jest wybrany kierunek jazdy, mo¿na ustaliæ prêdkoœæ jazdy
      ...
      //hm, kiedyœ semafory wysy³a³y SetVelocity albo ShuntVelocity i ustaw³y tak VelSignal - a teraz jak to zrobiæ?
      TCommandType comm=TableUpdate(mvOccupied->Vel,VelDesired,ActualProximityDist,VelNext,AccDesired); //szukanie optymalnych wartoœci
     }
*/
   //break;
   case Shunt:
   ...
   case Change_direction|Connect: //zmiana kierunku podczas pod³¹czania
    ...
    //if ((VelSignal==0.0)&&(WaitingTime>WaitingExpireTime)&&(mvOccupied->RunningTrack.Velmax!=0.0))
    ...
    {//automatyczne ruszanie po odstaniu albo spod SBL
     if ((VelSignal==0.0)&&(WaitingTime>0.0)&&(mvOccupied->RunningTrack.Velmax!=0.0))
...
*/
      ...
      if (iCoupler?true:(iDrivigFlags&moveStopHere)==0) //Ra: tu jest coœ nie tak, bo bez tego warunku rusza³o w manewrowym !!!!
...
*/
    ...
    if (AIControllFlag)
     if (OrderList[OrderPos]&Change_direction) //mo¿e byæ zmieszane z jeszcze jak¹œ komend¹
...
*/
    ...
    {//jeœli jest wybrany kierunek jazdy, mo¿na ustaliæ prêdkoœæ jazdy
     ...
     //if (VelSignal!=VelDesired) //je¿eli prêdkoœæ zalecana jest inna (ale tryb te¿ mo¿e byæ inny)
     switch (comm)
     {//ustawienie VelSignal - trochê proteza = do przemyœlenia
      case cm_Ready: //W4 zezwoli³ na jazdê
       ...
       if (iDrivigFlags&moveStopCloser)
        VelSignal=VelDesired; //niech jedzie, jak W4 puœci³o - nie, ma czekaæ na sygna³ z sygnalizatora!
      case cm_SetVelocity: //od wersji 357 semafor nie budzi wy³¹czonej lokomotywy
       if (!(OrderList[OrderPos]&~(Obey_train|Shunt))) //jedzie w dowolnym trybie albo Wait_for_orders
        if (fabs(VelSignal)>=1.0) //0.1 nie wysy³a siê do samochodow, bo potem nie rusz¹
         PutCommand("SetVelocity",VelSignal,VelNext,NULL); //komenda robi dodatkowe operacje
      break;
      ...
      case cm_Command: //komenda z komórki
       if (!(OrderList[OrderPos]&~(Obey_train|Shunt))) //jedzie w dowolnym trybie albo Wait_for_orders
        if (mvOccupied->Vel<0.1) //dopiero jak stanie
         //iDrivigFlags|=moveStopHere moveStopCloser) //chyba ¿e stan¹³ za daleko (SU46 w WK staje za daleko)
     ...
     if (VelNext==0.0)
      ...
      {//je¿eli wolnej drogi nie ma, a jest w trybie manewrowym albo oczekiwania
       //if ((OrderList[OrderPos]&Connect)?pVehicles[0]->fTrackBlock>ActualProximityDist:true) //pomiar odleg³oœci nie dzia³a dobrze?
        //w trybie Connect skanowaæ do ty³u tylko jeœli przed kolejnym sygna³em nie ma taboru do pod³¹czenia
       //Ra 2F1H: z tym (fTrackBlock) to nie jest najlepszy pomys³, bo lepiej by by³o porównaæ z odleg³oœci¹ od sygnalizatora z przodu
       if ((OrderList[OrderPos]|Connect)?pVehicles[0]->fTrackBlock>2000:true)
        ...
        {//nale¿y sprawdzaæ odleg³oœæ od znalezionego sygnalizatora,
         ...
         if (comm==cm_Command) //jeœli komenda Shunt
          iDrivigFlags|=moveStopHere; //to j¹ odbierz bez przemieszczania siê (np. odczep wagony po dopchniêciu do koñca toru)
         iDirectionOrder=-iDirection; //zmiana kierunku jazdy
          OrderList[OrderPos]=TOrders(OrderList[OrderPos]|Change_direction); //zmiana kierunku bez psucia kolejnych komend
        }
      }
     double vel=mvOccupied->Vel; //prêdkoœæ w kierunku jazdy
     if (iDirection*mvOccupied->V<0) vel=-vel; //ujemna, gdy jedzie w przeciwn¹ stronê, ni¿ powinien
...
#if LOGVELOCITY
     //WriteLog("VelDesired="+AnsiString(VelDesired)+", VelSignal="+AnsiString(VelSignal));
       ...
       {//jedzie szybciej, ni¿ trzeba na koñcu ActualProximityDist, ale jeszcze jest daleko
        ...
        {//jeœli jedzie wolniej ni¿ VelNext+35km/h //Ra: 40, ¿eby nie kombinowa³ na zwrotnicach
         ...
         {//jeœli wiêksze przekroczenie ni¿ fVelPlus [km/h], ale ma jechaæ
          //Ra 2F1I: jak by³o (VelNext+fVelPlus) tu, to hamowa³ zbyt póŸno przed 40, a potem zbyt mocno i zwalnia³ do 30
...
//        if ((mvOccupied->BrakeCtrlPos<0)&&(mvOccupied->BrakePress<0.3))//{(CntrlPipePress-(Volume/BrakeVVolume/10)<0.01)})
...
#if LOGVELOCITY
      WriteLog("Dist="+FloatToStrF(ActualProximityDist,ffFixed,7,1)+", VelDesired="+FloatToStrF(VelDesired,ffFixed,7,1)+", AccDesired="+FloatToStrF(AccDesired,ffFixed,7,3)+", VelSignal="+AnsiString(VelSignal)+", VelNext="+AnsiString(VelNext));
#endif
      if (AccDesired>0.1)
      ...
      //Ra 2F1H: jest konflikt histerezy pomiêdzy nastawion¹ pozycj¹ a uzyskiwanym przyspieszeniem - utrzymanie pozycji powoduje przekroczenie przyspieszenia
...
*/
      ...
      if (mvControlling->SlippingWheels)
      {
       ...
       if (mvOccupied->BrakeCtrlPos==mvOccupied->BrakeCtrlPosNo) //jeœli ostatnia pozycja hamowania
 ...
 if (AIControllFlag)
 {
  ...
  if (mvOccupied->Vel>=3.0) //jesli jedzie, mo¿na odblokowaæ tr¹bienie, bo siê wtedy nie w³¹czy
...
{//wykonanie kolejnej komendy z tablicy rozkazów
 if (OrderList[OrderPos]!=Wait_for_orders)
 {
  if (OrderList[OrderPos]&Change_direction) //jeœli zmiana kierunku
   ...
   {OrderList[OrderPos]=TOrders(OrderList[OrderPos]&~Change_direction); //usuniêcie zmiany kierunku z innej komendy
    OrderCheck();
    return;
   }
...
{//sprawdzanie zdarzeñ semaforów z ty³u pojazdu, zwraca komendê
 ...
 if ((OrderList[OrderPos]&~(Shunt|Connect)))
  return cm_Unknown; //skanowanie sygna³ów tylko gdy jedzie w trybie manewrowym albo czeka na rozkazy
 vector3 sl;
   ...
   {//jeœli jest jakiœ sygna³ na widoku
    ...
    {//przes³aæ info o zbli¿aj¹cym siê semaforze
     ...
     if (e->Command()==cm_SetVelocity)
      if ((vmechmax==0.0)?(OrderCurrentGet()&(Shunt|Connect)):(OrderCurrentGet()&Connect)) //przy podczepianiu ignorowaæ wyjazd?
...
#endif
         //SetProximityVelocity(scandist,vmechmax,&sl);
         return (iDrivigFlags&moveTrackEnd)?cm_ChangeDirection:cm_Unknown; //jeœli jedzie na W5 albo koniec toru, to mo¿na zmieniæ kierunek
        }
       }
...
{//przejêcie kontroli przez AI albo oddanie
 ...
 {//teraz AI prowadzi
  ...
  if (OrderCurrentGet()) //jeœli coœ robi
   ...
   if (pVehicle->iLights[mvOccupied->CabNo<0?1:0]&21) //któreœ ze œwiate³ zapalone?
