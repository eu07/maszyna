...
{TODO: sprawdzanie innego czlonu                  if not FuseFlagCheck() then }
                    if (BrakePress<=0.05*MaxBrakePress) then
                      begin
                        OK:=IncMainCtrl(1);
                      end;
              end;
...
{                        VelDesired:=Min0R(VelDesired,VelReduced+VelNext); }
                ...
                if BrakeSystem=Pneumatic then  {napelnianie uderzeniowe}
                 if BrakeSubsystem=Oerlikon then //yb: cos mi sie tu nie podoba, bo AI przeladowuje...
                  begin
                    if BrakeCtrlPos<0 then
                     BrakeCtrlPos:=0;
                     IncBrakeLevel;
                    if (BrakeCtrlPos=0) and (AccDesired>0) then
                     if PipePress<0.4 then
                      DecBrakeLevel
                     else
...
//                       end;
       ...
       {          if not MaxVelFlag then }
                ...
                {ladne odhamowywanie}
                if ((Vel>VelDesired) and (AbsAccS<0) and (BrakeCtrlPos>0)) then
                  if abs((Vel-VelDesired)/3.6)<abs(0.5*AbsAccS*(1+BrakeDelay[1+2*BrakeDelayFlag])*BrakePress/MaxBrakePress) then
                    BrakeCtrlPos:={-1}0 //yB: nie, bo przeladujesz
                  else
                else
       ...
       {          if not MinVelFlag then   }
                   begin
                      if not DecSpeed then
                       if (AccDesired<-0.1+AbsAccS) or (Vel>VelDesired) then
