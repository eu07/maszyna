...
TYPE
    ...
    TEngineTypes = (None, Dumb, WheelsDriven, ElectricSeriesMotor, DieselEngine, SteamEngine); { EZT); }
    ...
    TMoverParameters = class(TObject)
               ...
               CompressorPower: integer; {0: bezp. z obwodow silnika, 1: z przetwornicy}
...
begin
  if Abs(CabNo+direction)<2 then
   begin
     ...
     if (CabNo<>LastCab) and (CabNo<>0) then
      begin
       c:=PantFrontUp;
...
begin
 if State=true then
  if ((CompressorPower=1) and (not ConverterAllow)) then
   State:=false;
 if (CompressorAllow<>State) then
 begin
...
begin
    if (DynamicBrakeType<>dbrake_switch) and (DynamicBrakeType<>dbrake_none) and ((BrakePress>0.2) or (PipePress<0.36)) then
     begin
...
begin
    ...
    if (DynamicBrakeType<>dbrake_switch) and ((BrakePress<0.1) and (PipePress>0.36)) then
       ...
       if ((BrakeCtrlPosNo>0) and (BrakePressureTable[BrakeCtrlPos].BrakeType=Pneumatic)) or
        ...
        begin
          ...
          if BrakePress>Rate*MaxBrakePress then
            begin
              dpBrake:=(1-Rate)*MaxBrakePress*dt*3/LBDelay;
        ...
        if PipePressureVal<>-1 then
         begin
           ...
           if (dpMainValve>0) then
            CompressedVolume:=CompressedVolume-10*dpMainValve*BrakeVolume*((CompressedVolume-2.5)/3)*6;
...
{        if Mains and (MainCtrlPos>1) then}
 ...
 {        if (not CompressorFlag) and (Compressor<MinCompressor) and Mains and (LastSwitchingTime>CtrlDelay) then}
         if (not CompressorFlag) and (Compressor<MinCompressor) and Mains and CompressorAllow and (LastSwitchingTime>CtrlDelay) then
          begin
            CompressorFlag:=True;
            LastSwitchingTime:=0;
          end;
         if (Compressor>MaxCompressor) or not CompressorAllow or not Mains then
   ...
   begin
    if Moment>dizel_engageMaxForce*dizel_engage*dizel_engageDia*friction*2 then {zerwanie przyczepnosci sprzegla}
     ...
     begin
       ...
       enrot:=n;                          {jest przyczepnosc tarcz}
     end
   end
  else
   begin
   ...
   begin
     ...
     if dizel_nmax_cutoff>0 then
      begin
       ...
       if enrot>nreg then
        realfill:=realfill*(3.9-3*enrot/nreg);
       if enrot>dizel_nmax_cutoff then
       realfill:=realfill*(9.8-9*enrot/dizel_nmax_cutoff);
       if enrot<dizel_nmin then
        realfill:=realfill*(1+(dizel_nmin-enrot)/dizel_nmin);
      end;
   end;
  if realfill<0 then
   realfill:=0;
...
var PosRatio,dmoment,dtrans: real;
...
{        enrot:=Transmision.Ratio*nrot; }
   ...
   DieselEngine: begin
                   ...
                   if MainCtrlPos>1 then
                    dmoment:=dmoment-dizel_Mstand*(0.2*abs(enrot)/nmax);  {dodatkowe opory z powodu sprezarki}
...
var newdist,distDelta,CF,dV,absdv,Fmax,BetaAvg:real; CNext:byte;
const MaxDist=205.0; {ustawione +5 m, bo skanujemy do 400 m }
      MaxCount=1000;
begin
  CF:=0;
  distDelta:=0;
    ...
    begin
      ...
      if (Couplers[CouplerN].CouplingFlag=ctrain_virtual) and (newdist>0) then
       begin
         ...
         inc(ScanCounter);
         if (newdist>MaxDist) or (ScanCounter>MaxCount) then
       ...
       begin
         ...
         if Dist>0 then
          begin
            ...
            if not Connected^.PhysicActivation then {uaktywanianie fizyki podlaczonego skladu}
             if (Dist>DmaxC*0.0001) or (abs(dV)>0.0001) then
              Connected^.Physic_ReActivation;
         ...
         if Dist<0 then
          begin
            ...
            if not Connected^.PhysicActivation then {uaktywanianie fizyki podlaczonego skladu}
             if (Dist<-DmaxB*0.0001) or (abs(dV)>0.0001) then
              Connected^.Physic_ReActivation;
...
begin
     with Couplers[CouplerN] do
      begin
        ...
        VprevC:=Connected^.V;
        case CouplerN of
         0 : CCF:=ComputeCollision(V,Connected^.V,TotalMass,Connected^.TotalMass,(beta+Connected^.Couplers[CouplerNr[CouplerN]].beta)/2,VirtualCoupling)/(dt+0.01);
         1 : CCF:=ComputeCollision(Connected^.V,V,Connected^.TotalMass,TotalMass,(beta+Connected^.Couplers[CouplerNr[CouplerN]].beta)/2,VirtualCoupling)/(dt+0.01);
        end;
    ...
    begin
     if (CabNo=0) and (Vel<0.0001) and (abs(AccS)<0.0001) and (TrainType<>'ezt') then
      begin
        ...
        if LastSwitchingTime>10+Random(100) then
...
{      if (EngineType=ElectricSeriesMotor) and (CValue1=1) and
  ...
  else if command='PantRear' then         {Winger 160204}
   begin
     if (TrainType='ezt') then
     begin
       if (CValue1=1) then
...
{              SetFlag(OKFlag,wheels_ok);}
          ...
          if Pos('Brake:',lines)>0 then      {hamulce}
            begin
              ...
              s:=ExtractKeyWord(lines,'CompressorPower=');
              if s='Converter' then
               CompressorPower:=1
              else
               CompressorPower:=0 ;
