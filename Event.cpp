...
TEvent::export_as_text( std::ostream &Output ) const {
    ...
    // NOTE: for flexibility condition check and export is performed for all event types rather than only for these which support it currently
    auto const isconditional { ( conditional_trackoccupied | conditional_trackfree | conditional_propability | conditional_memcompare ) };
    if( ( ( m_condition.flags & isconditional ) != 0 ) ) {
        Output << "condition ";
        ...
        if( ( m_condition.flags & conditional_memcompare ) != 0 ) {
            Output
                << "memcompare "
     ...
     && ( Event->fDelay == 0.0 ) ) {
        ...
        if( ( false == Event->m_ignored )
         && ( true == EventConditon( Event ) ) ) { // teraz mogą być warunki do tych eventów
            ...
            WriteLog( "EVENT EXECUTED" + ( Owner ? ( " by " + Owner->asName ) : "" ) + ": " + Event->asName );
            WriteLog( "Type: AddValues & Track command - ["
                + Event->m_update.data_text + "] ["
                + to_string( Event->m_update.data_value_1, 2 ) + "] ["
                + to_string( Event->m_update.data_value_2, 2 ) + "]" );
            // TODO: dump status of target cells after the operation
            for( auto &target : Event->m_targets ) {
                auto *memcell = static_cast<TMemCell *>( std::get<scene::basic_node *>( target ) );
                if( memcell == nullptr ) { continue; }
                memcell->UpdateValues(
                    Event->m_update.data_text,
                    Event->m_update.data_value_1,
                    Event->m_update.data_value_2,
                    Event->m_update.flags );
                if( memcell->Track == nullptr ) { continue; }
                // McZapkie-100302 - updatevalues oprocz zmiany wartosci robi putcommand dla wszystkich 'dynamic' na danym torze
                auto const location { memcell->location() };
                for( auto dynamic : memcell->Track->Dynamics ) {
                    memcell->PutCommand(
                        dynamic->Mechanik,
                        &location );
                }
    ...
    { // eventy są posortowana wg czasu wykonania
        ...
        if( ( false == m_workevent->m_ignored ) && ( true == m_workevent->bEnabled ) ) {
            ...
            case tp_CopyValues: {
                ...
                auto const *datasource = static_cast<TMemCell *>( std::get<scene::basic_node *>( m_workevent->m_update.data_source ) );
                m_workevent->m_update.data_text = datasource->Text();
                m_workevent->m_update.data_value_1 = datasource->Value1();
                m_workevent->m_update.data_value_2 = datasource->Value2();
            }
            case tp_AddValues: // różni się jedną flagą od UpdateValues
            case tp_UpdateValues: {
                if( false == EventConditon( m_workevent ) ) { break; }
                WriteLog( "Type: " + std::string( m_workevent->Type == tp_AddValues ? "AddValues & Track command" : "UpdateValues" ) + " - ["
                    + m_workevent->m_update.data_text + "] ["
                    + to_string( m_workevent->m_update.data_value_1, 2 ) + "] ["
                    + to_string( m_workevent->m_update.data_value_2, 2 ) + "]" );
                // TODO: dump status of target cells after the operation
                for( auto &target : m_workevent->m_targets ) {
                    ...
                    targetcell->UpdateValues(
                        m_workevent->m_update.data_text,
                        m_workevent->m_update.data_value_1,
                        m_workevent->m_update.data_value_2,
                        m_workevent->m_update.flags );
            ...
            case tp_GetValues: {
                ...
                if( m_workevent->Activator ) {
                    ...
                    m_workevent->m_update.data_cell()->PutCommand(
                        m_workevent->Activator->Mechanik,
                        &(m_workevent->m_update.data_cell()->location()) );
                }
                break;
            }
            case tp_PutValues: {
                ...
                if (m_workevent->Activator) {
                    ...
                    loc.X = -m_workevent->m_update.location.x;
                    loc.Y =  m_workevent->m_update.location.z;
                    loc.Z =  m_workevent->m_update.location.y;
                    if( m_workevent->Activator->Mechanik ) {
                        // przekazanie rozkazu do AI
                        m_workevent->Activator->Mechanik->PutCommand(
                            m_workevent->m_update.data_text,
            ...
            case tp_Lights: {
                for( auto &target : m_workevent->m_targets ) {
                    ...
                    // event effect code
                    for( i = 0; i < iMaxNumLights; ++i ) {
                        if( m_workevent->m_lights[ i ] == -2.f ) {
                            // processed all supplied values, bail out
                            break;
                        }
                        if( m_workevent->m_lights[ i ] >= 0.f ) {
                            // -1 zostawia bez zmiany
                            targetmodel->LightSet(
                                i,
                                m_workevent->m_lights[ i ] );
                        }
                    }
                }
                break;
            ...
            case tp_Visible: {
                for( auto &target : m_workevent->m_targets ) {
                    ...
                    // event effect code
                    targetnode->visible( m_workevent->m_visible );
                }
                break;
            }
            case tp_Sound: {
                for( auto &target : m_workevent->m_sounds ) {
                    auto *targetsound = std::get<sound_source *>( target );
                    if( targetsound == nullptr ) { continue; }
                    // event effect code
                    switch( m_workevent->m_soundmode ) {
                        // trzy możliwe przypadki:
                        case 0: {
                            targetsound->stop();
                            break;
                        ...
                        case 1: {
                            if( m_workevent->m_soundradiochannel > 0 ) {
                                simulation::radio_message(
                                    targetsound,
                                    m_workevent->m_soundradiochannel );
                            }
                            else {
            ...
            case tp_Animation: {
                ...
                if( m_workevent->m_animationtype == 4 ) {
                    ...
                    for( auto &target : m_workevent->m_targets ) {
                        ...
                        targetmodel->AnimationVND(
                            m_workevent->m_animationfiledata,
                            m_workevent->m_animationparams[ 0 ], // tu mogą być dodatkowe parametry, np. od-do
                            m_workevent->m_animationparams[ 1 ],
                            m_workevent->m_animationparams[ 2 ],
                            m_workevent->m_animationparams[ 3 ] );
                    }
                }
                else {
                    // other animation modes target specific submodels
                    for( auto *targetcontainer : m_workevent->m_animationcontainers ) {
                        switch( m_workevent->m_animationtype ) {
                            case 1: { // rotate
                                targetcontainer->SetRotateAnim(
                                    glm::make_vec3( m_workevent->m_animationparams.data() ),
                                    m_workevent->m_animationparams[ 3 ] );
                                break;
                            }
                            case 2: { // translate
            ...
            case tp_Switch: {
                for( auto &target : m_workevent->m_targets ) {
                    ...
                    // event effect code
                    targettrack->Switch(
                        m_workevent->m_switchstate,
                        m_workevent->m_switchmoverate,
                        m_workevent->m_switchmovedelay );
                }
                if( Global.iMultiplayer ) {
                    // dajemy znać do serwera o przełożeniu
                    multiplayer::WyslijEvent( m_workevent->asName, "" ); // wysłanie nazwy eventu przełączajacego
            ...
            case tp_TrackVel: {
                WriteLog( "Type: TrackVel - [" + to_string( m_workevent->m_velocity, 2 ) + "]" );
                for( auto &target : m_workevent->m_targets ) {
                    auto *targettrack = static_cast<TTrack *>( std::get<scene::basic_node *>( target ) );
                    if( targettrack == nullptr ) { continue; }
                    // event effect code
                    targettrack->VelocitySet( m_workevent->m_velocity );
                    if( DebugModeFlag ) {
            ...
            case tp_Multiple: {
                auto const condition { EventConditon( m_workevent ) };
                if( ( true == condition )
                 || ( true == m_workevent->m_condition.has_else ) ) {
                    // warunek spelniony albo było użyte else
                    WriteLog("Type: Multi-event");
                    for( auto &childevent : m_workevent->m_children ) {
                        auto *childeventdata { std::get<TEvent*>( childevent ) };
                    ...
                    if( Global.iMultiplayer ) {
                        // dajemy znać do serwera o wykonaniu
                        if( false == m_workevent->m_condition.has_else ) {
                            // jednoznaczne tylko, gdy nie było else
                            if( m_workevent->Activator ) {
                                multiplayer::WyslijEvent( m_workevent->asName, m_workevent->Activator->name() );
                            }
            ...
            case tp_WhoIs: {
                for( auto &target : m_workevent->m_targets ) {
                    ...
                    if( targetcell == nullptr ) { continue; }
                    // event effect code
                    if( m_workevent->m_update.flags & update_load ) {
                        // jeśli pytanie o ładunek
                        if( m_workevent->m_update.flags & update_memadd ) {
                            // jeśli typ pojazdu
                            // TODO: define and recognize individual request types
                            auto const owner = (
                                ( ( m_workevent->Activator->Mechanik != nullptr ) && ( m_workevent->Activator->Mechanik->Primary() ) ) ?
                            ...
                            targetcell->UpdateValues(
                                ...
                                consistbrakelevel,
                                collisiondistance,
                                m_workevent->m_update.flags & ( update_memstring | update_memval1 | update_memval2 ) );

                            WriteLog(
                                "Type: WhoIs (" + to_string( m_workevent->m_update.flags ) + ") - "
                                + "[name: " + m_workevent->Activator->MoverParameters->TypeName + "], "
                                + "[consist brake level: " + to_string( consistbrakelevel, 2 ) + "], "
                                + "[obstacle distance: " + to_string( collisiondistance, 2 ) + " m]" );
                        }
                    ...
                    else if( m_workevent->m_update.flags & update_memadd ) { // jeśli miejsce docelowe pojazdu
                        targetcell->UpdateValues(
                            m_workevent->Activator->asDestination, // adres docelowy
                            m_workevent->Activator->DirectionGet(), // kierunek pojazdu względem czoła składu (1=zgodny,-1=przeciwny)
                            m_workevent->Activator->MoverParameters->Power, // moc pojazdu silnikowego: 0 dla wagonu
            ...
            case tp_LogValues: { // zapisanie zawartości komórki pamięci do logu
                if( m_workevent->m_targets.empty() ) {
                    // lista wszystkich
                    simulation::Memory.log_all();
                }
                else {
            ...
            case tp_Voltage: { // zmiana napięcia w zasilaczu (TractionPowerSource)
                WriteLog( "Type: Voltage [" + to_string( m_workevent->m_voltage, 2 ) + "]" );
                for( auto &target : m_workevent->m_targets ) {
                    ...
                    targetpowersource->VoltageSet( m_workevent->m_voltage );
                }
                break;
            }
            case tp_Friction: { // zmiana tarcia na scenerii
                // na razie takie chamskie ustawienie napięcia zasilania
                WriteLog("Type: Friction");
                Global.fFriction = (m_workevent->m_friction);
                break;
            }
            case tp_Message: { // wyświetlenie komunikatu
                break;
...
event_manager::InitEvents() {
    ...
    for( auto *event : m_events ) {
        ...
        case tp_UpdateValues: { // zmiana wartości
            ...
            event->init_targets( simulation::Memory, "memory cell" );
            // conditional data
            event->init_conditions();
            break;
        }
        case tp_LogValues: {
            // skojarzenie z memcell
        ...
        case tp_GetValues: {
            ...
            event->m_update.data_source = event->m_targets.front();
            break;
        }
        case tp_WhoIs: {
            event->init_targets( simulation::Memory, "memory cell" );
        ...
        case tp_CopyValues: {
            ...
            event->init_targets( simulation::Memory, "memory cell" );
            // source cell
            std::get<scene::basic_node *>( event->m_update.data_source ) = simulation::Memory.find( std::get<std::string>( event->m_update.data_source ) );
            if( std::get<scene::basic_node *>( event->m_update.data_source ) == nullptr ) {
                event->m_ignored = true; // deaktywacja
                ErrorLog( "Bad event: copyvalues event \"" + event->asName + "\" cannot find memcell \"" + std::get<std::string>( event->m_update.data_source ) + "\"" );
            }
            break;
        }
        case tp_Animation: {
            // animacja modelu
            event->init_targets( simulation::Instances, "model instance" );
            // custom target initialization code
            if( event->m_animationtype == 4 ) {
                // vmd animations target whole model
                break;
            }
            // locate and set up animated submodels
            event->m_animationcontainers.clear();
            for( auto &target : event->m_targets ) {
                auto *targetmodel { static_cast<TAnimModel *>( std::get<scene::basic_node *>( target ) ) };
                if( targetmodel == nullptr ) { continue; }
                auto *targetcontainer { targetmodel->GetContainer( event->m_animationsubmodel ) };
                if( targetcontainer == nullptr ) {
                    event->m_ignored = true;
                    ErrorLog( "Bad event: animation event \"" + event->asName + "\" cannot find submodel " + event->m_animationsubmodel + " in model instance \"" + targetmodel->name() + "\"" );
                    break;
                }
                targetcontainer->WillBeAnimated(); // oflagowanie animacji
                if( targetcontainer->Event() == nullptr ) {
                    // nie szukać, gdy znaleziony
                    targetcontainer->EventAssign( FindEvent( targetmodel->name() + '.' + event->m_animationsubmodel + ":done" ) );
                }
                event->m_animationcontainers.emplace_back( targetcontainer );
            }
            break;
        }
        case tp_Lights: {
        ...
        case tp_Visible: {
            ...
            for( auto &target : event->m_targets ) {
                ...
                if( targetnode == nullptr ) {
                    event->m_ignored = true; // deaktywacja
                    ErrorLog( "Bad event: event \"" + event->asName + "\" [" + event->type_to_string() + "] cannot find item \"" + std::get<std::string>( target ) + "\"" );
                }
            }
            break;
        }
        case tp_Switch: {
            // przełożenie zwrotnicy albo zmiana stanu obrotnicy
            event->init_targets( simulation::Paths, "track" );
            // custom target initialization code
            for( auto &target : event->m_targets ) {
                auto *targettrack = static_cast<TTrack *>( std::get<scene::basic_node *>( target ) );
                if( targettrack == nullptr ) { continue; }
                ...
                if( ( event->m_switchstate == 0 )
                 && ( event->m_switchmovedelay >= 0.0 ) ) {
                    // jeśli przełącza do stanu 0 & jeśli jest zdefiniowany dodatkowy ruch iglic
                    // przesłanie parametrów
                    targettrack->Switch(
                        event->m_switchstate,
                        event->m_switchmoverate,
                        event->m_switchmovedelay );
                }
            }
            break;
        }
        case tp_Sound: {
            // odtworzenie dźwięku
            for( auto &target : event->m_sounds ) {
                std::get<sound_source *>( target ) = simulation::Sounds.find( std::get<std::string>( target ) );
                if( std::get<sound_source *>( target ) == nullptr ) {
                    event->m_ignored = true; // deaktywacja
                    ErrorLog( "Bad event: event \"" + event->asName + "\" [" + event->type_to_string() + "] cannot find static sound \"" + std::get<std::string>( target ) + "\"" );
                }
            }
            break;
        }
        ...
        case tp_Multiple: {
            // conditional data
            event->init_targets( simulation::Memory, "memory cell", false );
            if( event->m_ignored ) {
                event->m_condition.flags &= ~( conditional_memstring | conditional_memval1 | conditional_memval2 );
                event->m_ignored = false;
            }
            event->init_conditions();
        ...
        case tp_Voltage: {
            // zmiana napięcia w zasilaczu (TractionPowerSource)
            event->init_targets( simulation::Powergrid, "power source" );
            break;
        }
        case tp_Message: {
            // wyświetlenie komunikatu
...
event_manager::EventConditon( TEvent *Event ) {
    ...
    if( Event->m_condition.flags == 0 ) {
        return true; // bezwarunkowo
    }
    // if there's conditions, check them
    if( Event->m_condition.flags & conditional_propability ) {
        auto const rprobability { static_cast<float>( Random() ) };
        WriteLog( "Test: Random integer - [" + std::to_string( rprobability ) + "] / [" + std::to_string( Event->m_condition.probability ) + "]" );
        if( rprobability > Event->m_condition.probability ) {
            return false;
        }
    }
    if( Event->m_condition.flags & conditional_trackoccupied ) {
        for( auto *track : Event->m_condition.tracks ) {
            if( true == track->IsEmpty() ) {
                return false;
            }
        }
    }
    if( Event->m_condition.flags & conditional_trackfree ) {
        for( auto *track : Event->m_condition.tracks ) {
    ...
    if( Event->m_condition.flags & conditional_memcompare ) {
        // porównanie wartości
        for( auto &target : m_workevent->m_targets ) {
            auto *targetcell = static_cast<TMemCell *>( std::get<scene::basic_node *>( target ) );
            ...
            auto const comparisonresult =
                targetcell->Compare(
                    Event->m_condition.match_text,
                    Event->m_condition.match_value_1,
                    Event->m_condition.match_value_2,
                    Event->m_condition.flags );
            ...
            comparisonlog += (
                TestFlag( Event->m_condition.flags, conditional_memstring ) ?
                    "[" + std::string( m_workevent->m_condition.match_text ) + "]" :
                    "[*]" );
            comparisonlog += (
                TestFlag( m_workevent->m_condition.flags, conditional_memval1 ) ?
                    " [" + to_string( m_workevent->m_condition.match_value_1, 2 ) + "]" :
                    " [*]" );
            comparisonlog += (
                TestFlag( m_workevent->m_condition.flags, conditional_memval2 ) ?
                    " [" + to_string( m_workevent->m_condition.match_value_2, 2 ) + "]" :
                    " [*]" );
