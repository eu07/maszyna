...
TEvent::TEvent(std::string const& m) : asNodeName(m)
{
	if (false == m.empty()) {
	...
	switch (Type) {
		...
		case tp_PutValues:
			...
			*parser >> Params[3].asdouble >> Params[4].asdouble >> Params[5].asdouble; // położenie
			// X,Y,Z
			if (!(org == Math3D::vector3())) { // przesunięcie
		...
		case tp_Animation:
			...
			*parser >> token;
			...
			{ // animacja z pliku VMD
				{
					std::ifstream file(szModelPath + token, std::ios::binary | std::ios::ate);
		...
		case tp_Switch:
			...
			*parser >> token;
			...
			if (token != "endevent") {
				Params[2].asdouble = atof(token.c_str()); // dodatkowy ruch drugiej iglicy (zamknięcie nastawnicze)
				parser->getTokens();
				*parser >> token;
			} else
		...
		case tp_Multiple: {
			...
			*parser >> token;
			...
			while ((token != "endevent") && (token != "condition") && (token != "randomdelay")) {
				if (token != "else") {
...
void TEvent::export_as_text(std::ostream& Output) const
{
	if (Type == tp_Unknown) {
		return;
	}
	...
	Output << asName << ' ';
	// type
	std::vector<std::string> const types{ "unknown", "sound", "exit", "disable", "velocity", "animation", "lights", "updatevalues", "getvalues", "putvalues", "switch", "dynvel", "trackvel",
		"multiple", "addvalues", "copyvalues", "whois", "logvalues", "visible", "voltage", "message", "friction" };
	Output << types[Type] << ' ';
	// delay
	Output << fDelay << ' ';
	// associated node
	...
	switch (Type) {
		case tp_AddValues:
		case tp_UpdateValues: {
			Output << ((iFlags & update_memstring) == 0 ? "*" : Params[0].asText) << ' ' << ((iFlags & update_memval1) == 0 ? "*" : to_string(Params[1].asdouble)) << ' '
		...
		case tp_CopyValues: {
			...
			Output << (Params[9].asMemCell == nullptr ? "none" : Params[9].asMemCell->name()) << ' ' << (iFlags & (update_memstring | update_memval1 | update_memval2)) << ' ';
			break;
		}
		case tp_PutValues: {
			Output
			// location
			<< Params[3].asdouble << ' ' << Params[4].asdouble << ' ' << Params[5].asdouble
			<< ' '
			// command
			<< Params[0].asText << ' ' << Params[1].asdouble << ' ' << Params[2].asdouble << ' ';
			break;
		}
		case tp_Multiple: {
		...
		case tp_Switch: {
			Output << Params[0].asInt << ' ';
			if ((Params[1].asdouble != -1.0) || (Params[2].asdouble != -1.0)) {
		...
		case tp_Lights: {
			auto lightidx{ 0 };
			while ((lightidx < iMaxNumLights) && (Params[lightidx].asdouble > -2.0)) {
		...
		case tp_Animation: {
			// animation type
			Output << (Params[0].asInt == 1 ? "rotate" :
			                                  Params[0].asInt == 2 ? "translate" :
			                                                         // NOTE: .vmd animation doesn't preserve file name, can't be exported. TODO: fix this
			                                  Params[0].asInt == 8 ? "digital" : "none")
			       << ' ';
			// submodel
			Output << (Params[9].asAnimContainer != nullptr ? Params[9].asAnimContainer->nameGet() : "none") << ' ';
			// animation parameters
			Output << Params[1].asdouble << ' ' << Params[2].asdouble << ' ' << Params[3].asdouble << ' ' << Params[4].asdouble << ' ';
			break;
		}
		case tp_Sound: {
			// playback mode
		...
		case tp_WhoIs: {
			Output << (iFlags & (update_memstring | update_memval1 | update_memval2)) << ' ';
			break;
		}
		default: {
			break;
	...
	if (((iFlags & isconditional) != 0)) {
		...
		if ((iFlags & conditional_trackoccupied) != 0) {
			...
			Output << "propability " << Params[10].asdouble << ' ';
		} else if ((iFlags & conditional_memcompare) != 0) {
			Output << "memcompare " << ((iFlags & conditional_memstring) == 0 ? "*" : Params[10].asText) << ' ' << ((iFlags & conditional_memval1) == 0 ? "*" : to_string(Params[11].asdouble)) << ' '
			       << ((iFlags & conditional_memval2) == 0 ? "*" : to_string(Params[12].asdouble)) << ' ';
		}
	}
	if (fRandomDelay != 0.0) {
		Output << "randomdelay " << fRandomDelay << ' ';
	}
	// footer
	Output << "endevent"
	       << "\n";
...
void TEvent::AddToQuery(TEvent* Event, TEvent*& Start)
{
	...
	while ((Event->fStartTime >= target->fStartTime) && (target->evNext != nullptr)) {
		previous = target;
...
bool event_manager::insert(TEvent* Event)
{
	if (Event->Type == tp_Unknown) {
		return false;
	}
	...
	if (lookup != m_eventmap.end()) {
		...
		// tymczasowo wyjątki:
		else if ((size > 8) && (Event->asName.substr(0, 9) == "lineinfo:")) {
			// tymczasowa utylizacja duplikatów W5
			return false;
		} else if ((size > 8) && (Event->asName.substr(size - 8) == "_warning")) {
			// tymczasowa utylizacja duplikatu z trąbieniem
			return false;
		} else if ((size > 4) && (Event->asName.substr(size - 4) == "_shp")) {
	...
	if (lookup == m_eventmap.end()) {
		...
		if ((Event->m_ignored != true) && (Event->asName.find("onstart") != std::string::npos)) {
...
TEvent* event_manager::FindEvent(std::string const& Name)
{
	if (Name.empty()) {
		return nullptr;
	}
	...
	return (lookup != m_eventmap.end() ? m_events[lookup->second] : nullptr);
}

// legacy method, inserts specified event in the event query
bool event_manager::AddToQuery(TEvent* Event, TDynamicObject* Owner, double delay)
{
	if (true == Event->bEnabled) {
		...
		{ // kolejka eventów jest posortowana względem (fStartTime)
			...
			if ((Event->Type == tp_AddValues) && (Event->fDelay == 0.0)) {
				...
				if ((false == Event->m_ignored) && (true == EventConditon(Event))) { // teraz mogą być warunki do tych eventów
					...
					if (Event->Params[6].asTrack) { // McZapkie-100302 - updatevalues oprocz zmiany wartosci robi putcommand dla
						...
						for (auto dynamic : Event->Params[6].asTrack->Dynamics) {
							Event->Params[5].asMemCell->PutCommand(dynamic->Mechanik, Event->Params[4].asLocation);
						}
						//if (DebugModeFlag)
						WriteLog("EVENT EXECUTED" + (Owner ? (" by " + Owner->asName) : "") + ": AddValues & Track command - [" + std::string{ Event->Params[0].asText } + "] [" +
						         to_string(Event->Params[1].asdouble, 2) + "] [" + to_string(Event->Params[2].asdouble, 2) + " ]");
					}
					//else if (DebugModeFlag)
					WriteLog("EVENT EXECUTED" + (Owner ? (" by " + Owner->asName) : "") + ": AddValues - [" + std::string(Event->Params[0].asText) + "] [" + to_string(Event->Params[1].asdouble, 2) +
				...
				} while ((Event != nullptr) && ((false == Event->bEnabled) || (Event->iQueued > 0)));
			}
...
bool event_manager::CheckQuery()
{
	TLocation loc;
	int i;
	while ((QueryRootEvent != nullptr) && (QueryRootEvent->fStartTime < Timer::GetTime())) { // eventy są posortowana wg czasu wykonania
		...
		if ((false == m_workevent->m_ignored) && (true == m_workevent->bEnabled)) {
			...
			switch (m_workevent->Type) {
				case tp_CopyValues: // skopiowanie wartości z innej komórki
					m_workevent->Params[5].asMemCell->UpdateValues(m_workevent->Params[9].asMemCell->Text(), m_workevent->Params[9].asMemCell->Value1(), m_workevent->Params[9].asMemCell->Value2(),
				...
				case tp_UpdateValues:
					if (EventConditon(m_workevent)) { // teraz mogą być warunki do tych eventów
						if (m_workevent->Type != tp_CopyValues) // dla CopyValues zrobiło się wcześniej
							m_workevent->Params[5].asMemCell->UpdateValues(m_workevent->Params[0].asText, m_workevent->Params[1].asdouble, m_workevent->Params[2].asdouble, m_workevent->iFlags);
						if (m_workevent->Params[6].asTrack) { // McZapkie-100302 - updatevalues oprocz zmiany wartosci robi putcommand dla
							// wszystkich 'dynamic' na danym torze
							for (auto dynamic : m_workevent->Params[6].asTrack->Dynamics) {
								m_workevent->Params[5].asMemCell->PutCommand(dynamic->Mechanik, m_workevent->Params[4].asLocation);
							}
							//if (DebugModeFlag)
							WriteLog("Type: UpdateValues & Track command - [" + m_workevent->Params[5].asMemCell->Text() + "] [" + to_string(m_workevent->Params[5].asMemCell->Value1(), 2) + "] [" +
							         to_string(m_workevent->Params[5].asMemCell->Value2(), 2) + "]");
						} else //if (DebugModeFlag)
							WriteLog("Type: UpdateValues - [" + m_workevent->Params[5].asMemCell->Text() + "] [" + to_string(m_workevent->Params[5].asMemCell->Value1(), 2) + "] [" +
							         to_string(m_workevent->Params[5].asMemCell->Value2(), 2) + "]");
					}
					break;
				case tp_GetValues: {
					if (m_workevent->Activator) {
						...
						m_workevent->Params[9].asMemCell->PutCommand(m_workevent->Activator->Mechanik, m_workevent->Params[8].asLocation);
					}
				...
				case tp_PutValues: {
					if (m_workevent->Activator) {
						...
						if (m_workevent->Activator->Mechanik) {
							...
							m_workevent->Activator->Mechanik->PutCommand(m_workevent->Params[0].asText, m_workevent->Params[1].asdouble, m_workevent->Params[2].asdouble, loc);
						} else {
							// przekazanie do pojazdu
							m_workevent->Activator->MoverParameters->PutCommand(m_workevent->Params[0].asText, m_workevent->Params[1].asdouble, m_workevent->Params[2].asdouble, loc);
						}
						WriteLog("Type: PutValues - [" + std::string(m_workevent->Params[0].asText) + "] [" + to_string(m_workevent->Params[1].asdouble, 2) + "] [" +
					...
					if (m_workevent->Params[9].asModel) {
						for (i = 0; i < iMaxNumLights; ++i) {
							...
							if (m_workevent->Params[i].asdouble >= 0) {
								...
								m_workevent->Params[9].asModel->lightSet(i, m_workevent->Params[i].asdouble);
							}
						}
					}
					break;
					...
					switch (m_workevent->Params[0].asInt) {
						...
						case 1: {
							if (m_workevent->Params[1].asdouble > 0.0) {
								Global.pWorld->radio_message(m_workevent->Params[9].tsTextSound, static_cast<int>(m_workevent->Params[1].asdouble));
							} else {
				...
				case tp_Animation: {
					switch (m_workevent->Params[0].asInt) {
						case 1: {
							...
							Math3D::vector3{ m_workevent->Params[1].asdouble, m_workevent->Params[2].asdouble, m_workevent->Params[3].asdouble }, m_workevent->Params[4].asdouble);
							break;
						}
						case 2: {
						...
						case 4: {
							m_workevent->Params[9].asModel->animationVND(m_workevent->Params[8].asPointer,
							m_workevent->Params[1].asdouble, // tu mogą być dodatkowe parametry, np. od-do
							m_workevent->Params[2].asdouble, m_workevent->Params[3].asdouble, m_workevent->Params[4].asdouble);
							break;
						}
						default: {
							break;
				...
				case tp_Switch: {
					if (m_workevent->Params[9].asTrack) {
						m_workevent->Params[9].asTrack->Switch(m_workevent->Params[0].asInt, m_workevent->Params[1].asdouble, m_workevent->Params[2].asdouble);
					}
					if (Global.iMultiplayer) {
				...
				case tp_TrackVel:
					if (m_workevent->Params[9].asTrack) {
						...
						WriteLog("Type: TrackVel - [" + to_string(m_workevent->Params[0].asdouble, 2) + "]" +
						         (DebugModeFlag ? ", actual [ " + to_string(m_workevent->Params[9].asTrack->VelocityGet(), 2) + "]" : ""));
					}
					break;
				case tp_DynVel:
				...
				case tp_Multiple: {
					...
					if ((true == condition) || (true == m_workevent->m_conditionalelse)) {
						...
						for (auto& childevent : m_workevent->m_children) {
							...
							if (std::get<bool>(childevent) != condition) {
								continue;
							}
				...
				case tp_WhoIs: {
					...
					if (m_workevent->iFlags & update_load) {
						...
						if (m_workevent->iFlags & update_memadd) {
							...
							auto const owner =
							(((m_workevent->Activator->Mechanik != nullptr) && (m_workevent->Activator->Mechanik->Primary())) ? m_workevent->Activator->Mechanik : m_workevent->Activator->ctOwner);
							auto const consistbrakelevel = (owner != nullptr ? owner->fReady : -1.0);
							auto const collisiondistance = (owner != nullptr ? owner->TrackBlock() : -1.0);

							m_workevent->Params[9].asMemCell->UpdateValues(m_workevent->Activator->MoverParameters->TypeName, // typ pojazdu
							consistbrakelevel, collisiondistance, m_workevent->iFlags & (update_memstring | update_memval1 | update_memval2));

							WriteLog("Type: WhoIs (" + to_string(m_workevent->iFlags) + ") - " + "[name: " + m_workevent->Activator->MoverParameters->TypeName + "], " +
							         "[consist brake level: " + to_string(consistbrakelevel, 2) + "], " + "[obstacle distance: " + to_string(collisiondistance, 2) + " m]");
						} else {
							// jeśli parametry ładunku
							m_workevent->Params[9].asMemCell->UpdateValues(m_workevent->Activator->MoverParameters->LoadType, // nazwa ładunku
							...
							WriteLog("Type: WhoIs (" + to_string(m_workevent->iFlags) + ") - " + "[load type: " + m_workevent->Activator->MoverParameters->LoadType + "], " + "[current load: " +
							         to_string(m_workevent->Activator->MoverParameters->Load, 2) + "], " + "[max load: " + to_string(m_workevent->Activator->MoverParameters->MaxLoad, 2) + "]");
						}
					} else if (m_workevent->iFlags & update_memadd) { // jeśli miejsce docelowe pojazdu
						m_workevent->Params[9].asMemCell->UpdateValues(m_workevent->Activator->asDestination, // adres docelowy
						...
						WriteLog("Type: WhoIs (" + to_string(m_workevent->iFlags) + ") - " + "[destination: " + m_workevent->Activator->asDestination + "], " +
						         "[direction: " + to_string(m_workevent->Activator->DirectionGet()) + "], " + "[engine power: " + to_string(m_workevent->Activator->MoverParameters->Power, 2) + "]");
					} else if (m_workevent->Activator->Mechanik)
						if (m_workevent->Activator->Mechanik->Primary()) { // tylko jeśli ktoś tam siedzi - nie powinno dotyczyć pasażera!
							m_workevent->Params[9].asMemCell->UpdateValues(m_workevent->Activator->Mechanik->TrainName(),
				...
				case tp_LogValues: {
					...
					if (m_workevent->Params[9].asMemCell) {
						...
						WriteLog("Memcell \"" + m_workevent->asNodeName + "\": [" + m_workevent->Params[9].asMemCell->Text() + "] [" + to_string(m_workevent->Params[9].asMemCell->Value1(), 2) +
				...
				case tp_Voltage: // zmiana napięcia w zasilaczu (TractionPowerSource)
					if (m_workevent->Params[9].psPower) { // na razie takie chamskie ustawienie napięcia zasilania
		...
		switch (event->Type) {
			...
			case tp_WhoIs: {
				...
				if (cell != nullptr) {
					...
					event->Params[9].asMemCell = cell;
					if ((event->Type == tp_GetValues) && (cell->IsVelocity())) {
					...
					if (event->Params[0].asInt == 4) {
						...
						if (event->Params[9].asAnimContainer) {
							...
							if (event->Params[9].asAnimContainer->event() == nullptr) {
								...
								event->Params[9].asAnimContainer->eventAssign(FindEvent(event->asNodeName + "." + cellastext + ":done"));
							}
						}
					}
				} else {
			...
			case tp_Switch: {
				...
				if (track != nullptr) {
					...
					if ((event->Params[0].asInt == 0) && (event->Params[2].asdouble >= 0.0)) {
						// jeśli przełącza do stanu 0 & jeśli jest zdefiniowany dodatkowy ruch iglic
						// przesłanie parametrów
						event->Params[9].asTrack->Switch(event->Params[0].asInt, event->Params[1].asdouble, event->Params[2].asdouble);
					}
				} else {
...
bool event_manager::EventConditon(TEvent* Event)
{
	...
	if (Event->iFlags & conditional_trackoccupied)
		...
		m_workevent->Params[9].asMemCell->Compare((Event->Params[10].asText != nullptr ? Event->Params[10].asText : ""), Event->Params[11].asdouble, Event->Params[12].asdouble, Event->iFlags);

		std::string comparisonlog = "Type: MemCompare - ";

		comparisonlog +=
		"[" + Event->Params[9].asMemCell->Text() + "]" + " [" + to_string(Event->Params[9].asMemCell->Value1(), 2) + "]" + " [" + to_string(m_workevent->Params[9].asMemCell->Value2(), 2) + "]";

		comparisonlog += (true == comparisonresult ? " == " : " != ");

		comparisonlog += (TestFlag(Event->iFlags, conditional_memstring) ? "[" + std::string(m_workevent->Params[10].asText) + "]" : "[*]");
		comparisonlog += (TestFlag(m_workevent->iFlags, conditional_memval1) ? " [" + to_string(m_workevent->Params[11].asdouble, 2) + "]" : " [*]");
		comparisonlog += (TestFlag(m_workevent->iFlags, conditional_memval2) ? " [" + to_string(m_workevent->Params[12].asdouble, 2) + "]" : " [*]");
