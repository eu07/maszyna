...
{//(cKey) to kod klawisza, cyfrowe i literowe siê zgadzaj¹
 ...
 {//podczas pauzy klawisze nie dzia³aj¹
  ...
  if (cKey==VkKeyScan('`')) { if (TWorld::bCONSOLEMODE == false) TWorld::bCONSOLEMODE = true;  else TWorld::bCONSOLEMODE = false; }
  if (TWorld::bCONSOLEMODE && TWorld::cph > 990) TWorld::CONSOLE__addkey(cKey);
  if (TWorld::bCONSOLEMODE && TWorld::cph > 990)  if (Pressed(VK_RETURN)) TWorld::CONSOLE__execute();
  if (TWorld::bCONSOLEMODE && cKey==Global::Keys[k_MechForward]) CONSOLE__fastnextcmd();
  if (TWorld::bCONSOLEMODE && cKey==Global::Keys[k_MechBackward]) CONSOLE__fastprevcmd();
  if (TWorld::bCONSOLEMODE) return;  
