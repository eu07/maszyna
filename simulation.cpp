...
namespace simulation
{
	...
	bool state_manager::deserialize(std::string const& Scenariofile)
	{
		...
		if (false == scenarioparser.ok()) {
			return false;
		}

		deserialize(scenarioparser, importscratchpad);
		if ((false == importscratchpad.binary.terrain) && (Scenariofile != "$.scn")) {
	...
	void state_manager::deserialize(cParser& Input, scene::scratch_data& Scratchpad)
	{
		...
		std::vector<std::pair<std::string, deserializefunction>> functionlist = { { "atmo", &state_manager::deserialize_atmo }, { "camera", &state_manager::deserialize_camera },
			{ "config", &state_manager::deserialize_config }, { "description", &state_manager::deserialize_description }, { "event", &state_manager::deserialize_event },
			{ "lua", &state_manager::deserialize_lua }, { "firstinit", &state_manager::deserialize_firstinit }, { "light", &state_manager::deserialize_light },
			{ "node", &state_manager::deserialize_node }, { "origin", &state_manager::deserialize_origin }, { "endorigin", &state_manager::deserialize_endorigin },
			{ "rotate", &state_manager::deserialize_rotate }, { "sky", &state_manager::deserialize_sky }, { "test", &state_manager::deserialize_test }, { "time", &state_manager::deserialize_time },
			{ "trainset", &state_manager::deserialize_trainset }, { "endtrainset", &state_manager::deserialize_endtrainset } };
		...
		auto timelast{ std::chrono::steady_clock::now() }, timenow{ timelast };
	...
	void state_manager::deserialize_atmo(cParser& Input, scene::scratch_data& Scratchpad)
	{
		...
		// fog range
		Input.getTokens(2);
		Input >> Global.fFogStart >> Global.fFogEnd;
	...
	void state_manager::deserialize_camera(cParser& Input, scene::scratch_data& Scratchpad)
	{
		...
		if (into < 10) { // przepisanie do odpowiedniego miejsca w tabelce
			...
			Global.FreeCameraInitAngle[into] = Math3D::vector3(glm::radians(abc.x), glm::radians(abc.y), glm::radians(abc.z));
...
*/
	...
	void state_manager::deserialize_config(cParser& Input, scene::scratch_data& Scratchpad)
	{
		// config parameters (re)definition
		Global.ConfigParse(Input);
	}

	void state_manager::deserialize_description(cParser& Input, scene::scratch_data& Scratchpad)
	{
		// legacy section, never really used;
		skip_until(Input, "enddescription");
	}

	void state_manager::deserialize_event(cParser& Input, scene::scratch_data& Scratchpad)
	{
		...
		(Scratchpad.location.offset.empty() ? Math3D::vector3() : Math3D::vector3(Scratchpad.location.offset.top().x, Scratchpad.location.offset.top().y, Scratchpad.location.offset.top().z));
	...
	void state_manager::deserialize_firstinit(cParser& Input, scene::scratch_data& Scratchpad)
	{
...
*/
			...
			if (nodedata.range_min < 0.0) {
				...
				if (false == Scratchpad.binary.terrain) {
					...
					for (auto i = 1; i < cellcount; ++i) {
						...
						simulation::Region->insert_shape(scene::shape_node().convert(submodel), Scratchpad, false);
		...
		} else if ((nodedata.type == "triangles") || (nodedata.type == "triangle_strip") || (nodedata.type == "triangle_fan")) {
			if (false == Scratchpad.binary.terrain) {
				simulation::Region->insert_shape(scene::shape_node().import(Input, nodedata), Scratchpad, true);
			} else {
				// all shapes were already loaded from the binary version of the file
				skip_until(Input, "endtri");
			}
		} else if ((nodedata.type == "lines") || (nodedata.type == "line_strip") || (nodedata.type == "line_loop")) {
			if (false == Scratchpad.binary.terrain) {
				simulation::Region->insert_lines(scene::lines_node().import(Input, nodedata), Scratchpad);
			} else {
...
*/
	...
	void state_manager::deserialize_origin(cParser& Input, scene::scratch_data& Scratchpad)
	{
		glm::dvec3 offset;
		Input.getTokens(3);
	...
	void state_manager::deserialize_endorigin(cParser& Input, scene::scratch_data& Scratchpad)
	{
	...
	void state_manager::deserialize_sky(cParser& Input, scene::scratch_data& Scratchpad)
	{
		// sky model
		Input.getTokens(1);
		Input >> Global.asSky;
	...
	void state_manager::deserialize_test(cParser& Input, scene::scratch_data& Scratchpad)
	{
		// legacy section, no longer supported;
		skip_until(Input, "endtest");
	}

	void state_manager::deserialize_time(cParser& Input, scene::scratch_data& Scratchpad)
	{
	...
	void state_manager::deserialize_trainset(cParser& Input, scene::scratch_data& Scratchpad)
	{
		...
		Input.getTokens(4);
		Input >> Scratchpad.trainset.name >> Scratchpad.trainset.track >> Scratchpad.trainset.offset >> Scratchpad.trainset.velocity;
	}

	void state_manager::deserialize_endtrainset(cParser& Input, scene::scratch_data& Scratchpad)
	{
		if ((false == Scratchpad.trainset.is_open) || (true == Scratchpad.trainset.vehicles.empty())) {
		...
		for (auto* vehicle : Scratchpad.trainset.vehicles) {
			...
			if (vehicleindex > 0) {
				...
				Scratchpad.trainset.vehicles[vehicleindex - 1]->AttachPrev(vehicle, Scratchpad.trainset.couplings[vehicleindex - 1]);
			}
			++vehicleindex;
		}
		...
		if (Scratchpad.trainset.driver != nullptr) {
			...
			controller->PutCommand("Timetable:" + Scratchpad.trainset.name, Scratchpad.trainset.velocity, 0, nullptr);
		}
	...
	TTrack* state_manager::deserialize_path(cParser& Input, scene::scratch_data& Scratchpad, scene::node_data const& Nodedata)
	{
	...
	TTraction* state_manager::deserialize_traction(cParser& Input, scene::scratch_data& Scratchpad, scene::node_data const& Nodedata)
	{
	...
	TTractionPowerSource* state_manager::deserialize_tractionpowersource(cParser& Input, scene::scratch_data& Scratchpad, scene::node_data const& Nodedata)
	{
		if (false == Global.bLoadTraction) {
	...
	TDynamicObject* state_manager::deserialize_dynamic(cParser& Input, scene::scratch_data& Scratchpad, scene::node_data const& Nodedata)
	{
		...
		if ((offset != -1.0) && (std::abs(offset) > 0.5)) { // maksymalna odległość między sprzęgami - do przemyślenia
		...
		auto const length = vehicle->Init(Nodedata.name, datafolder, skinfile, mmdfile, path, (offset == -1.0 ? Scratchpad.trainset.offset : Scratchpad.trainset.offset - offset), drivertype, velocity,
		Scratchpad.trainset.name, loadcount, loadtype, (offset == -1.0), params);
		...
		if (length != 0.0) { // zero oznacza błąd
			...
			if ((coupling != 0) && (vehicle->MoverParameters->Couplers[(offset == -1.0 ? side::front : side::rear)].AllowedFlag & coupling::permanent)) {
	...
	void state_manager::skip_until(cParser& Input, std::string const& Token)
	{
		std::string token{ Input.getToken<std::string>() };
	...
	glm::dvec3 state_manager::transform(glm::dvec3 Location, scene::scratch_data const& Scratchpad)
	{
